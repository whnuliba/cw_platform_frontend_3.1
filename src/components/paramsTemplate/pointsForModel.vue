          ...
          <el-tabs v-model="activeName" class='param-tab-style'>
           <!--保护参数-->
            ...
            >
              ...
              <el-col :span="24" class="text-capacity-compensation">
                <el-form-item label="补偿值">
                    <el-input type="textarea" v-model="processContent.pp.capacityCompensation"  :disabled="isFlowDts" :rows="1"/>
                </el-form-item>
              </el-col>
              <el-col :span="12">
            ...
            <!-- DCR参数设定 -->
              ...
              <el-col :span="24">
                <div :style="{display:'flex',fontWeight:'bolder'}">
                  ...
                  <p :style="{marginLeft:'10px',marginRight:'10px',marginTop:'7px'}">{{ "步" }}</p>
                  <!-- <span> -->
              ...
              <el-col :span="24" class="text-capacity-compensation">
                <el-form-item label="补偿值">
                    <el-input type="textarea" v-model="processContent.pp.DCRCapacityCompensation"  :disabled="isFlowDts" :rows="1"/>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                ...
                >
                  ...
                  >
                    <template slot-scope="scope" :style="{width: '100%',height: '100%'}">
                      <el-input style="font-size: 10px"  oninput ="value = value.replace(/[^0-9.-]/g,'')"
                      @input="handleInput(scope,'row','stepTime')" v-model="scope.row.stepTime" :disabled="isFlowDts"/>
                    </template>
                  </el-table-column>
                  <el-table-column
                    prop="stepCurrent"
                  ...
                  <!-- width="120" -->
                    <template slot-scope="scope" :style="{width: '100%',height: '100%'}">
                      <el-input type="number" v-if="scope.row.show5" v-model="scope.row.stepEndCurrent" :disabled="isFlowDts"/>
                    </template>
                  </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      nowAdd() {
        ...
        // console.log(JSON.stringify(this.processContent))
      ...
      stopInput(chargeStep, index, row) {
        if (chargeStep === 'DC') { // 放电
          if (index == 4) {
            ...
            this.processContent.sp[row.rowIndex]['stepEndCurrent'] = 0
        ...
        if (chargeStep === 'Sleep') {  // 休眠
          if (index == 3 || index == 4 || index == 5) { // || index == 6
            this.processContent.sp[row.rowIndex]['show' + index] = false
            this.processContent.sp[row.rowIndex]['stepCurrent'] = 0
            this.processContent.sp[row.rowIndex]['stepVoltage'] = 0
            this.processContent.sp[row.rowIndex]['stepEndCurrent'] = 0
            return 'background:#7d7d7d;color:red'
          } else if (index >= 2) {
            this.processContent.sp[row.rowIndex]['show' + index] = true
...
<style lang="less" scoped>
  #chargeParamTemp1 {
    ...
    //   }
    // }
    ::v-deep .text-capacity-compensation .el-form-item--medium .el-form-item__content{
          width: 42.5vw; //90%;
    }
    .colHeight {
      height: 40px;
