...
export default {
 ...
 props: {
    ...
    mounted () {
      ...
      this.initRact(this.codeStyle)

      // Object.assign(codemirror_s.style,this.codeStyle)
...
// 内部真实的内容
  ...
  methods: {
      initRact(style){
        ...
        // const codemirror_s = el.getElementsByClassName('CodeMirror-scroll')[0]
        if(style){
        ...
        // codemirror_s.style.height=this.height
       // Object.assign(codemirror.style,this.codeStyle)
      },
      initScript(val){
          this.code = val
      },
      tablenameinitScript(val){
        this.code = val ? val : ''
      },
        // 初始化
      _initialize () {
