<template>
    <div class="context-menu-div" :id="rightclickInfo.menuId">
        <div class="context-menu-list" v-for="(item) in rightclickInfo.menulists" :key="item.btnName"
        ...
        watch: {
            ...
            "rightclickInfo.position"(val) {
                this.$nextTick(() =>{
                    ...
                    let mainDesignerMainClientWidth = document.querySelector('.main_designer_main1') ? document.querySelector('.main_designer_main1').clientWidth : 0;
                    let mainDesignerMainClientHeight = document.querySelector('.main_designer_main1') ? document.querySelector('.main_designer_main1').clientHeight : 0;
                    let mainDesignerLeftClientWidth = document.querySelector('.main_designer_left') ? document.querySelector('.main_designer_left').clientWidth : 0;
                    let innerWidth = mainDesignerMainClientWidth + mainDesignerLeftClientWidth; // 获取页面可是区域宽度
                    let innerHeight = mainDesignerMainClientHeight; // 获取可视区域高度
                    ...
                    // 菜单的位置计算 不超过可视区域
                    menu.style.top = (innerHeight ? (y + menuHeight > innerHeight ? innerHeight - menuHeight : y) : y) + "px";
                    menu.style.left = (innerWidth ? (x + menuWidth > innerWidth ? innerWidth - menuWidth : x) : x) + "px";
