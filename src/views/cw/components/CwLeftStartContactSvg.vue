...
export default {
  ...
  methods: {
    ...
    createSvgPath(createCount = 6, length = 30) {
      ...
      let contact_w = 1;
      let contact_h = 4;
      // let outBox = `M 0 ${boxSize} h ${length} v ${-boxSize} h ${width-length} v ${length} h ${-width} z  M  ${width} ${boxSize} l ${-interval+boxSize} ${(length-boxSize*2)/2} l ${interval-boxSize} ${(length-boxSize*2)/2} h ${-boxSize} l  ${-interval+boxSize} ${-(length-boxSize*2)/2} l ${interval-boxSize} ${-(length-boxSize*2)/2} z`

      let outBox = `M 0 ${boxSize} v ${length - boxSize} h ${width}  v ${-length} h ${-width + length
        } v ${boxSize} h ${-length} z  M  ${width} ${boxSize} l ${-interval + boxSize} ${(length - boxSize * 2) / 2
