...
Inspector.showIfPresent = function(data, propname) {
  // if (propname === "ports") data.data.ports = JSON.stringify(data.data.ports);
...
Inspector.prototype.inspectObject = function(object) {
  ...
  if (!inspectedObjects || !this.multipleSelection) {
    ...
    for (var names in declaredProperties) {
      let desc = declaredProperties[names];
      if (!this.canShowProperty(names, desc, inspectedObject)) continue;
      let val = this.findValue(names, desc, data);
      tbody.appendChild(this.buildPropertyRow(names, val));
    }
    ...
    if (data) { // initial pass to set shared and all
      ...
      if (this.includesOwnProperties) {
        for (var ks in data) {
          if (ks === '__gohashid') continue; // skip internal GoJS hash property
          if (this._inspectedProperties[ks]) continue; // already exists
          if (declaredProperties[ks] && !this.canShowProperty(ks, declaredProperties[ks], inspectedObject)) continue;
          shared.add(ks, data[ks]);
          all.add(ks, data[ks]);
        }
      }
    }
    ...
    while (it.next() && (this.showSize < 1 || nodecount <= this.showSize)) { // grabs all the properties from the other selected objects
      ...
      if (inspectedObject) {
        ...
        if (data) {
          ...
          for (var neckName in declaredProperties) {
            let desc = declaredProperties[neckName];
            if (!this.canShowProperty(neckName, desc, inspectedObject)) continue;
            let val = this.findValue(neckName, desc, data);
            if (val === '' && desc && desc.type === 'checkbox') {
              properties.add(neckName, false);
            } else {
              properties.add(neckName, val);
            }
          }
          // Go through all the properties on the model data and add them to properties to add, if appropriate:
          if (this.includesOwnProperties) {
            for (var ak in data) {
              if (ak === '__gohashid') continue; // skip internal GoJS hash property
              if (this._inspectedProperties[ak]) continue; // already exists
              if (declaredProperties[ak] && !this.canShowProperty(ak, declaredProperties[ak], inspectedObject)) continue;
              properties.add(ak, data[ak]);
            }
          }
        }
      }
      if (!this.showAllProperties) {
        ...
        while (addIt.next()) {
          if (all.has(addIt.key)) {
            ...
            let newVal = '';
            for (var is = 0; is < nodecount - 1; is++) newVal += '|';
...
Inspector.prototype.buildPropertyRow = function(propertyName, propertyValue) {
  ...
  for(let key1 in enTransCn) {
    if (propertyName === key1) {
        cnPropertyName = enTransCn[key1]
    }
  }
  td1.textContent = propertyName; //cnPropertyName;
  tr.appendChild(td1);
...
Inspector.prototype.updateAllHTML = function() {
  ...
  if (!data) {  // clear out all of the fields
    ...
    for (var sname in inspectedProps) {
      let input = inspectedProps[sname];
      var propertyValue = data[sname];
      if (input instanceof HTMLSelectElement) {
        let decProp = this.declaredProperties[sname];
        this.updateSelect(decProp, input, sname, propertyValue);
      } else if (input.type === "color") {
        input.value = this.convertToColor(propertyValue);
      } else if (input.type === "checkbox") {
        input.checked = !!propertyValue;
...
Inspector.prototype.updateSelect = function(decProp, select, propertyName, propertyValue) {
  ...
  //let changes = decProp.changes
  if (typeof choices === "function") choices = choices(this.inspectedObject, propertyName,propertyValue,select);
  // if (typeof changes === "function") changes = changes(propertyValue, this.inspectedObject,propertyName,select);
...
Inspector.prototype.updateAllProperties = function() {
  ...
  if (diagram.selection.count === 1 || !this.multipleSelection) { // single object update
    ...
    diagram.startTransaction('set all properties');
    for (var namess in inspectedProps) {
      let input = inspectedProps[namess];
      let value = input.value;
      var arr1 = value.split('|');
      var arr2 = [];
      if (this._multipleProperties[namess]) {
        // don't split if it is union and its checkbox type
        if (this.declaredProperties[namess] && this.declaredProperties[namess].type === 'checkbox' && this.showAllProperties) {
          arr2.push(this._multipleProperties[namess]);
        } else {
          arr2 = this._multipleProperties[namess].toString().split('|');
        }
      }
      var it = diagram.selection.iterator;
      var change = false;
      if (this.declaredProperties[namess] && this.declaredProperties[namess].type === 'checkbox') change = true; // always change checkbox
      if (arr1.length < arr2.length // i.e Alpha|Beta -> Alpha procs the change
        && (!this.declaredProperties[namess] // from and to links
          || !(this.declaredProperties[namess] // do not change color checkbox and choices due to them always having less
            && (this.declaredProperties[namess].type === 'color' || this.declaredProperties[namess].type === 'checkbox' || this.declaredProperties[namess].type === 'choices')))) {
        change = true;
      } else { // standard detection in change in properties
        for (var j = 0; j < arr1.length && j < arr2.length; j++) {
          if (!(arr1[j] === arr2[j])
            && !(this.declaredProperties[namess] && this.declaredProperties[namess].type === 'color' && arr1[j].toLowerCase() === arr2[j].toLowerCase())) {
            change = true;
          }
        }
      }
      if (change) { // only change properties it needs to change instead all of them
        for (var i = 0; i < diagram.selection.count; i++) {
          ...
          if (data) { // ignores the selected node if there is no data
            ...
            // don't update "readOnly" data properties
            let decProp = this.declaredProperties[namess];
            if (!this.canEditProperty(namess, decProp, it.value)) continue;
            ...
            // modify the data object in an undo-able fashion
            diagram.model.setDataProperty(data, namess, value);

            // notify any listener
            if (this.propertyModified !== null) this.propertyModified(namess, value, this);
          }
        }
      }
    }
