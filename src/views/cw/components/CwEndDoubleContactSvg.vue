  ...
  methods: {
    ...
    createSvgPath(createCount = 6, length = 30) {
      ...
      let outBox = `M 0 0 v ${length} h ${width - length
        ...
        }  h ${-length}  v ${-boxSize} h ${length-width} z  M 0 ${boxSize} h ${boxSize} l ${interval - boxSize
        } ${(length - boxSize * 2) / 2} l ${-(interval - boxSize)} ${(length - boxSize * 2) / 2
        } h ${-boxSize} l ${interval - boxSize} ${-(length - boxSize * 2) / 2}  L 0 ${boxSize} z  `;
      let innerBox = `M ${interval} ${boxSize} h ${width - (interval + boxSize)} v ${length - boxSize * 2
        } h  ${-(width - (interval + boxSize))} v ${
          -(length - boxSize * 2)
        } z`;
