    ...
    methods: {
      ...
      getAttributeCols() {
        return [
          ...
          { field: 'smooth', title: '平滑处理', queryType: 'switch', require: false, query: true, hide: false },
          { field: 'seriesLabel', title: '数据标签', queryType: 'switch', require: false, query: true, hide: false },
        ]
      },
      onClick(e){},
      comText() {
      ...
      setOptions(chart, result) { 
          ...
          series: [
            {
              data: result.data[0] ,
              type: 'line',
              smooth: this.component.attribute.smooth,
              label: {
                show: this.component.attribute.seriesLabel,  
                position: 'top'  
              }
            }
          ]
        ...
        if('name' in result && result.name.length > 0){
          for(let i=0; i<result.name.length; i++){
            option.series[i].name =  result.name[i]
          }
        }
      ...
      /*
        ------elementCode例子-------
        {
          chartOption(urlData){
            if(urlData){
              return {
                "category": Object.keys(urlData[0]),
                "data": [Object.values(urlData[0])],
              }
            }
              return {
                  "category": ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'],
