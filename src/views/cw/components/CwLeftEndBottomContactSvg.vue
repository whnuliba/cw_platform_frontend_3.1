<template>
  <div ref="myref" v-bind="$attrs">
    <svg :viewBox="svgViewBox" :width="component.styles.width" :height="component.styles.height"
      ...
      <text ref="textElement" x="30%" y="50%" text-anchor="middle" dy=".3em" font-weight="bold" font-size="8px"
...
<script>
export default {
  name: 'CwLeftEndBottomContactSvg',
  components: {},
  props: {
    componentModalObj: {
      type: Object,
  ...
  methods: {
        ...
        [
          h('path', {
            attrs: {
              ...
              d:"M0 0 L0 30 L50 30 L50 25 L80 25 L80 0z M80 5 L75 15 L80 25 L75 25 L70 15 L75 5z M5 5 L70 5 L70 25 L5 25z M11 5 L14 5 L14 9 L15 9 L15 21 L14 21 L14 25 L12 25 L12 21 L11 21 L11 9 L12 9 L12 5z M21 5 L24 5 L24 9 L25 9 L25 21 L24 21 L24 25 L22 25 L22 21 L21 21 L21 9 L22 9 L22 5z M31 5 L34 5 L34 9 L35 9 L35 21 L34 21 L34 25 L32 25 L32 21 L31 21 L31 9 L32 9 L32 5z M41 5 L44 5 L44 9 L45 9 L45 21 L44 21 L44 25 L42 25 L42 21 L41 21 L41 9 L42 9 L42 5z M51 5 L54 5 L54 9 L55 9 L55 21 L54 21 L54 25 L52 25 L52 21 L51 21 L51 9 L52 9 L52 5z M61 5 L64 5 L64 9 L65 9 L65 21 L64 21 L64 25 L62 25 L62 21 L61 21 L61 9 L62 9 L62 5z",
    ...
    createSvgPath(createCount = 6, length = 30) {
      ...
      let contact_w = 1;
      let contact_h = 4;
      //let outBox = `M 0 0  h ${width} v ${length-boxSize}  h ${-length}  v ${boxSize} h ${-(width-length)} z  M  ${width} ${boxSize} l ${-interval+boxSize} ${(length-boxSize*2)/2} l ${interval-boxSize} ${(length-boxSize*2)/2} h ${-boxSize} l  ${-interval+boxSize} ${-(length-boxSize*2)/2} l ${interval-boxSize} ${-(length-boxSize*2)/2} z`

      let outBox = `M 0 0 v ${length} h ${width - length
        } v ${-boxSize}  h ${length}  v ${-length + boxSize
        } h ${-width} z  M  ${width} ${boxSize} l ${-interval + boxSize} ${(length - boxSize * 2) / 2
        } l ${interval - boxSize} ${(length - boxSize * 2) / 2} h ${-boxSize} l  ${-interval + boxSize
        } ${-(length - boxSize * 2) / 2} l ${interval - boxSize} ${-(length - boxSize * 2) / 2
        } h ${boxSize} z`;
      let innerBox = `M ${boxSize} ${boxSize} h ${width - (interval + boxSize)} v ${length - boxSize * 2
        } h  ${-(width - (interval + boxSize))} v ${boxSize * 2 - length} z`;
      let direction1 = `m ${dirSiz} 0 h ${cellSize - contact_w
        } v ${contact_h} h ${contact_w} v ${length - (boxSize + contact_h) * 2
        } h ${-contact_w} v ${contact_h}  h ${-cellSize + contact_w * 2
        } v ${-contact_h} h ${-contact_w} v ${-(
          length -
          (boxSize + contact_h) * 2
        )} h ${contact_w} v ${-contact_h} h ${contact_w -cellSize} z`;
      //let direction = `m ${dirSiz+boxSize} 0 h ${boxSize} v ${length-boxSize*2} h ${-boxSize} z`
      let direction = `m ${dirSiz + cellSize} 0 h ${cellSize - contact_w
        } v ${contact_h} h ${contact_w} v ${length - (boxSize + contact_h) * 2
        } h ${-contact_w} v ${contact_h}  h ${-cellSize + contact_w * 2
        } v ${-contact_h} h ${-contact_w} v ${-(
          length -
