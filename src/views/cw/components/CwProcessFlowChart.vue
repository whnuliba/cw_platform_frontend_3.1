        ...
        <div style="display:flex;justify-content:center;margin-bottom:5px;">
            <div v-if='isApproval' style="display:flex;">
                <div class="approve-sel">
                    <el-select v-model="userId" :placeholder="commonI18n('common.todo.approver')">
                        <el-option
                ...
                <el-button id="approveBtn" class="el-btn" type="primary" @click="approveSubmit">{{commonI18n("common.btn.submitApproval")}}</el-button>
            </div>
            <el-button type="primary" class="el-btn" style="margin-left:0" @click="modifyChart" v-if="true">{{commonI18n("common.btn.update")}}</el-button>
            <!--  -->
        </div>
           ...
           <div class="palette-div" :style="{height: windowHeight}">
            ...
            <div>
                ...
                <el-collapse-item title="基础模版" name="1">
                    <template slot="title">
                        <i class="header-icon el-icon-s-unfold"></i> {{commonI18n('const.pageDesign.page.basicTemplate')}}
                    </template>
            ...
            <!-- 中间流程图部分 -->
            <div id="myDiagramDiv1" ref="myDiagramDiv1"></div>
            <!--  ref="myDiagramDiv1" -->

            <!-- 节点属性调整区域(右侧节点信息表) -->
            <div id="node-set" :style="{verticalAlign: 'top', width:'21%'}">
                                ...
                                <tr>
                                    <td  style="text-align: center;width: 100%">
                                        <div className="nodeInfo" style="text-align: center;width: 100%">
                                          {{ commonI18n('const.pageDesign.page.unSelectNodeStatus') }}
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
            ...
            >
                <component :dialogSubmit="dialogSubmit" :is="variable" :isFlowDts="false" ref="mychild"  
            ...
            <el-form :inline="true" label-position="right" label-width="100px" :model="flowRow">
                ...
                <el-form-item  :label="'电芯型号'">
                  ...
                  <el-select :disabled="true" v-model="flowRow.cellId" style="width:100%">
                    <el-option v-for="(item,i) in cellTypeOption" :key="'cell'+i" :value="item.id" :label="item.cellTypeName"></el-option>
                  </el-select>
                </el-form-item>

                <el-form-item :label="'使用状态'">
                  <el-select v-model="flowRow.useState" :disabled="true">
                    <el-option v-for="(item,a) in userStateOption" :key="'user'+a" :value="item.value" :label="item.label"></el-option>
                  </el-select>
                </el-form-item>

                <el-form-item :label="'流程类型'">
                  <el-select v-model="flowRow.flowType" :disabled="true">
                    <el-option v-for="(item,b) in flowTypeOption" :key="'flow'+b" :value="item.value" :label="item.label"></el-option>
                  </el-select>
                </el-form-item>
    ...
    export default {
        ...
        data() {
          return {
            ...
            chartData: {},
            isJumpStation: [
              { text: this.commonI18n('common.processFlowTmpList.jumpStation'), value: 1 },
              { text: this.commonI18n('common.processFlowTmpList.passStation'), value: 2 },
            ],
            paramsTempData: [
              "SelectModel",
              "OCV1Model",
            ...
            nodeData: "",
            // 节点信息模态变量
            customCulumns: [{ label: "字段1", key: "param1", type: "input" }],
            //修改保存界面属性
            modalTitle: "",
            allflowColumns: [
            ...
            paramsSetColumn: [
              // {label:'参数字段',key:'params', type: 'select',selType:'A',selVal:'setlabel',selLabel:'setlabel'},
              // {label:'流程名称',key:'pageName'},
              { label: "参数字段", key: "paramsType", type: "cascader", isMulti: true },
              { label: "参数样式", key: "paramsStyle" },
            ],
            parentLabel: {
              first: "",
              second: "",
        ...
        methods: {
              ...
              this.$store.dispatch("fmsCommon/actionFmsPost", _data).then((data) => {
                ...
                if (data.length > 0) {
                  ...
                  this.nodeCodeArr = data.map((item) => {
                    ...
                    // this.commonI18n(item.processName)
                  });
                }
              });
            },
          ...
          //  <!-- 流程图json保存,用于在脚本中触发的函数方法 -->
          chartJsonSave(node, isModifyBtn,url) {
            let json = {
              data: {
            ...
            const _data = { url: url, params: json };  // "/process-flow-tmp/addflow"
            this.$store.dispatch("fmsCommon/actionFmsPost", _data).then((data) => {
          ...
          initProcessAddTable() {
            const _data = {
              ...
              params: { data: this.rowInfo.id },    //this.processAddParam.flowId
            };
            this.$store.dispatch("fmsCommon/actionFmsPost", _data).then((data) => {
         ...
         approveSubmit() {
            // 审批确认按钮
            this.initProcessAddTable();
            let currentNodeStationInfo = JSON.parse(this.myDiagramEntity.model.toJson()).nodeDataArray;
            ...
            this.$confirm(
              ...
              .then(() => {
                this.approvalJson.data.bizId = this.rowInfo.id;
                this.approvalJson.data.currentState = this.rowInfo.status;
          ...
          chartInit() {
              ...
              if (node.data.templateCode && node.data.templateCode.value) {
                ...
                if (rowId) {
                  ...
                  _this.$nextTick(() => {
                    ...
                    _this.handlerEvent('params-temp', node, row, _this.$refs.mychild)
                  });
                } else {
                  _this.$message({
            ...
            function hideCX() {
              if (myDiagram1.currentTool instanceof go.ContextMenuTool) {
                myDiagram1.currentTool.doCancel();
              }
            }
            ...
            function showContextMenu(obj, diagram, tool) {
              ...
              _this.rightTop = mousePt.y + 38 + "px";
            ...
            // 初始化拖拽面板一般在左边
            // 基础模板
            let myPalette = goBase(go.Palette, this.$refs.myPaletteBase, {
              // 使用自定义淡入淡出而不是默认动画
              "animationManager.initialAnimationStyle": go.AnimationManager.None,
              InitialAnimationStarting: animateFadeDown, // 相反，使用此函数进行动画处理
              nodeTemplateMap: myDiagram1.nodeTemplateMap, // 共享 myDiagram1 使用的模板
              model: new go.GraphLinksModel({
                ...
                nodeDataArray: [
                  ...
                  //   width: 80,
                  //   height: 80,
                  //   text: `${this.commonI18n('common.processFlowTmpList.node')}1`,
                  // },
                  // {
                  //   key: 2,
                  //   fill: "#282c34",
                  ...
                  //   width: 80,
                  //   height: 40,
                  //   text: `${this.commonI18n('common.processFlowTmpList.node')}2`,
                  // },
                  // {
                  //   key: 3,
                  //   fill: "#282c34",
                  ...
                  //   width: 80,
                  //   height: 80,
                  //   text: `${this.commonI18n('common.processFlowTmpList.node')}3`,
                  // },
                  // {
                  //   key: 4,
                  //   fill: "#282c34",
                  ...
                  //   width: 80,
                  //   height: 80,
                  //   text: `${this.commonI18n('common.processFlowTmpList.node')}4`,
                  // },
                  // {
                  //   key: 5,
                  //   fill: "#282c34",
                  ...
                  //   width: 90,
                  //   height: 40,
                  //   text: `${this.commonI18n('common.processFlowTmpList.node')}5`,
                  // },
                  // {
                  //   key: 6,
                  //   fill: "#282c34",
                  ...
                  //   width: 60,
                  //   height: 60,
                  //   text: `${this.commonI18n('common.processFlowTmpList.node')}6`,
                  // },
                ],
              }),
            });
            ...
            new Inspector("myInspector", myDiagram1, {
              // 允许一次检查多个节点
              ...
              properties: {
                ...
                nodeCode: {
                  ...
                  choices: function (node, propName, propertyValue) {
                    ...
                    _this.codeGetTemplate(node, myDiagram1, propertyValue);
                    return _this.nodeCodeArr
                    // .filter((item) => item.text !== "test");
                  },
                },
                templateCode: {
                  show: Inspector.showIfNode,
                ...
                status: {
                  ...
                  choices: function (node, propName, propertyValue) {
                    ...
                    if (propertyValue.value === 1) {
                      ...
                      myDiagram1.model.setDataProperty(nodes, "fill", "#FFFF77");
                      myDiagram1.model.setDataProperty(nodes, "color", "#000");
                    } else {
                      // 过站变成黑色
                      let nodes = myDiagram1.model.findNodeDataForKey(node.data.key);
                      myDiagram1.model.setDataProperty(nodes, "fill", "#000");
                      myDiagram1.model.setDataProperty(nodes, "color", "#fff");
                    }
                    return [
                              { text: _this.commonI18n('common.processFlowTmpList.jumpStation'), value: 1 },
                              { text: _this.commonI18n('common.processFlowTmpList.passStation'), value: 2 },
                            ] 
                    //_this.isJumpStation;
                  },
          ...
          codeGetTemplate(node, myDiagram1, propertyValue) {
            ...
            myDiagram1.model.setDataProperty(nodes, "text",propertyValue.text ? `${propertyValue.text}\ [${propertyValue.value}]`: `${this.commonI18n('common.processFlowTmpList.node')}`);
            // myDiagram1.model.setDataProperty(nodes, "text",propertyValue.text ? `${this.commonI18n(propertyValue.text)}\ [${propertyValue.value}]`: `${this.commonI18n('common.processFlowTmpList.node')}`);
          },
          nodeClick(e, obj) {
                  ...
                  if (rowId && this.nodeData.nodeCode) {
                    this.$confirm(
                      ...
                      this.$t("common.btn.hint"),
                      {
                        ...
                        let json = { data: rowId };
                        this.handlerEvent('node-dele',json,this.myDiagramEntity,nodeData);
                          //   const _data = { url: "/process-flow-tmp-dts/del", params: json };
                          //   this.$store.dispatch("fmsCommon/actionFmsPost", _data).then((data) => {
                          //       this.myDiagramEntity.commandHandler.deleteSelection();
                          //       this.nextDelLinks = JSON.parse(
          ...
          // 流程图修改按钮
          modifyChart() {
            this.handlerEvent('flow-modify-select')
          ...
          // 自定义参数finish
          designFormOnFinish() {
            let paramsArr = this.modalTempParams.paramsType;
            this.customCulumns = paramsArr.map((item, i) => {
              if (item[3] && item[3].indexOf(",") !== -1) {
                let sel = item[3].split(",");
                return (item = {
                  label: item[0],
                  key: item[1],
                  type: item[2],
                ...
                return (item = { label: item[0], key: item[1], type: item[2] });
              }
            });
          },
          // 工艺流程图JSON
          ...
          moveOnFinish() {
            ...
            for (var nit = this.myDiagramEntity.nodes; nit.next();) {
              ...
              if (!nit.value.linksConnected.rb) {
                ...
                let linkArr = nit.value.linksConnected.rb.m;
                if (!nodes.nodeCode) {
                  this.$message({
                    type: "warning",
                    message: "请先选择工艺节点!!",
           ...
           // if (nodeData.nodeDataArray.length) {
              _this.handlerEvent('move-on-finish',newDataArr,nodeData)
            //   const dataParam = {
          ...
          load(graphJson) {
            ...
            //   };
              ...
              for (let i = 0; i < nodeArrt.nodeDataArray.length; i++) {
                ...
                // data.forEach(li => {
                //   console.log(li,item.nodeCode.value)
                //   if (li.processCode === item.nodeCode.value) {
                //     item.text = `${li.processName} [${li.processCode}]`;
                //   }
                // })
...
/deep/.inspector-cw td {
  float: left;
  width: 50%;
  margin-bottom: 5px;
  font-weight: bold;
}
/deep/.inspector-cw input {
  width: 95%;
  padding: 3px;
