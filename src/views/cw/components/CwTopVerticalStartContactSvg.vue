...
<script>
export default {
  name: 'CwTopVerticalStartContactSvg',
  components: {},
  props: {
    componentModalObj: {
      type: Object,
  ...
  methods: {
    ...
    createSvgPath(createCount = 6, length = 30) {
      ...
      let contact_w = 1;
      let contact_h = 4;
      //let outBox = `M ${boxSize} 0 v ${length} h ${-boxSize} v ${width-length} h ${length} v ${-width} z  M ${boxSize} ${width} l ${(length-boxSize*2)/2} ${-interval+boxSize} l ${(length-boxSize*2)/2} ${interval-boxSize} v  ${-boxSize} l ${-(length-boxSize*2)/2} ${-interval+boxSize} l ${-(length-boxSize*2)/2} ${interval-boxSize} zX`

      let outBox = `M ${boxSize} 0 h ${length - boxSize} v ${width} h ${-length} v ${-width + length
