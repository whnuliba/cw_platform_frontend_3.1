...
<template>
    ...
    <!--制作流程图-->
      ...
      <div>
        ...
        <el-button class="filter-item" ref="saveBtn" type="primary" @click="(v1,v2,v3,v4)=>{handlerEvent('save-click',v1,v2,v3,v4)}" icon="el-icon-circle-check" plain>
          {{ graphId.length ? $t("common.btn.edit") : $t("common.btn.add") }}
        </el-button>
        <el-button  class="filter-item" @click="reFresh()" icon="el-icon-refresh-left" plain>
          {{ $t("common.btn.refresh") }}
        </el-button>
         <!-- <el-button  class="filter-item" @click="preview()" icon="el-icon-refresh-left" plain>
          {{ '预览' }}
        </el-button> -->
        ...
        <div  style="position: relative;">
          ...
          <div style="width: 300px; float: left; height:618px ; display: flex; justify-content: space-between;overflow-y:auto;padding:0 10px;" id="nodePropForm">
            <el-tabs v-model="activeName" :stretch="true">
              <el-tab-pane label="属性" name="first">
                <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="80px" size="mini">
                  <div v-for="(item) in cols" :key="item.id" style="display:inline">
                    <el-form-item v-if="item.edit && item.editType==='input'" :label="commonI18n(item)" :prop="item.field"
                      ...
                      placeholder="" 
                      @focus="()=>{if(item.onClick) item.onClick(defaultForm.params) }"
                    ...
                    <el-form-item v-if="item.edit && item.editType==='colorPicker'" :label="commonI18n(item)" :prop="item.field">
                        <el-color-picker   v-model="defaultForm.params[item.field]" 
                          :disabled="item.disabled || editDisabled[item.field]"  @change="(v)=>{if(item.onChange) item.onChange(v,item,defaultForm.params)}"/>
                    </el-form-item>
                  ...
                  <div class="node-condition" v-if="wfConditionDisplay && defaultForm.params['nodeType']=='COND'" style="align-content:center;text-align:center;width:95%;padding-left:5px;">
                    ...
                    <el-row v-for="(item,b) in defaultForm.params.conditions" :key="'con' + b" :gutter="10">
                      <el-col :span="8"><div class="grid-content bg-purple">
                        <el-input 
                            size="mini"
                ...
                <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="80px" size="mini">
                  <div v-for="(item) in colsAttr" :key="item.id" style="display:inline">
                    <el-form-item v-if="item.edit && item.editType==='input'" :label="commonI18n(item)" :prop="item.field"
                                  :rules="[{ required: item.required, message: item.title+'不能为空', trigger: 'blur' }]">
                      <el-input 
                      v-model="defaultForm.params[item.field]" 
                    ...
                    <el-form-item v-if="item.edit && item.editType==='inputNumber'" :label="commonI18n(item)" :prop="item.field"
                                  :rules="[{ required: item.required, message: item.title+'不能为空', trigger: 'blur' }]">
                          <el-input-number v-model="defaultForm.params[item.field]" 
                            :disabled="item.disabled || editDisabled[item.field]" 
                            placeholder="" 
                            @focus="()=>{if(item.onClick) item.onClick(defaultForm.params) }"
                            @change="(v)=>{if(item.onChange) item.onChange(v,item,defaultForm.params)}"
                            style="width: 170px" :min="1" :max="100"
                          />
                    </el-form-item>
                    <el-form-item v-if="item.edit && item.editType==='date'" :label="commonI18n(item)" :prop="item.field">
                            <el-date-picker
  ...
  export default {
    ...
    data() {
      ...
      const _initdata = {
        ...
        initSelect: {
          ...
          fontConfig: [
            { label: "8px", value: "Bold 5px Lato, sans-serif" },
            { label: "10px", value: "Bold 10px Lato, sans-serif" },
            { label: "11px", value: "Bold 11px Lato, sans-serif" },
            { label: "12px", value: "Bold 12px Lato, sans-serif" },
            { label: "13px", value: "Bold 13px Lato, sans-serif" },
            { label: "14px", value: "Bold 14px Lato, sans-serif" },
            { label: "15px", value: "Bold 15px Lato, sans-serif" },
            { label: "16px", value: "Bold 16px Lato, sans-serif" },
            { label: "17px", value: "Bold 17px Lato, sans-serif" },
            { label: "18px", value: "Bold 18px Lato, sans-serif" },
            { label: "19px", value: "Bold 19px Lato, sans-serif" },
            { label: "20px", value: "Bold 20px Lato, sans-serif" },
            { label: "25px", value: "Bold 25px Lato, sans-serif" },
          ],
        },
        defaultForm: {
          row: {},
        ...
        cols: [
          ...
          { field: "approveRole", title: "组织角色", align: "center",width:200,editType:"select" ,edit:true,onChange:(val,item,row)=>{this.onSelectChang(val,item,row)}},
         ...
         // }},
          ...
          { field: "jobName", title: "岗位名称",hide:true, align: "center",width:200,editType:'input' ,edit:false},
        ...
        colsAttr: [
          { field: "textContent", title: "文本内容",hide:true, align: "center",width:200 ,editType:'input',edit:true,onChange:(val,item,row)=>{this.onNodeConfigChange(val,item,row)}},
          { field: "fontConfig", title: "字体大小",hide:true, align: "center",width:200 ,editType:'select',edit:true,onChange:(val,item,row)=>{this.onNodeConfigChange(val,item,row)}},
          { field: "nodeColor", title: "节点颜色",hide:true, align: "center",width:200 ,editType:'colorPicker',edit:true,onChange:(val,item,row)=>{this.onNodeConfigChange(val,item,row)}},
          { field: "strokeColor", title: "边框颜色",hide:true, align: "center",width:200 ,editType:'colorPicker',edit:true,onChange:(val,item,row)=>{this.onNodeConfigChange(val,item,row)}},
          { field: "strokeWidth", title: "边框宽度",hide:true, align: "center",width:200 ,editType:'inputNumber',edit:true,onChange:(val,item,row)=>{this.onNodeConfigChange(val,item,row)}},
          { field: "textColor", title: "文本颜色",hide:true, align: "center",width:200 ,editType:'colorPicker',edit:true,onChange:(val,item,row)=>{this.onNodeConfigChange(val,item,row)}},
        ],
        diagramEntity: {},
      };
    ...
    methods: {
      async initChartTb(url, params,url1,params1,isNeedRender) {
          let _data = { url: url, params: params }
          let response = await new Promise((resolve,rej) => this.$store.dispatch('fmsCommon/actionPost', _data)  // 用new Promise可以把请求的数据return出来
                        .then(data => {
                          resolve(data)
                          this.initChart(url1,params1,isNeedRender,data)
                        }).catch(err => {
                                console.error(err)
                            })
          )
      ...
      initChart(url1,params,isNeedRender,resData) {
        if (resData) {
          params.data.sqlWhere[0].value = resData[0].ID
        }
        
        let graph = { url:url1, params: params }
        this.$store.dispatch('fmsCommon/actionPost', graph)
                            .then(data1 => {
                              if (data1.length) {
                                this.initChartData = data1;
                                this.graphJson = data1[0];
                                this.graphId = data1[0].ID;
                                if (isNeedRender) {
                                    this.myDiagram.model = go.Model.fromJson(data1[0].GRAPH_SCRIPT);
                                  }
                              } 
                              else {
                                this.graphId = '';
      ...
      saveCreateNode(jsonUrl,params,url, params1,url1,params2,isNeedRender){
        //this.resetNodeNo()
        this.graphJson = this.myDiagram.model.toJson()
        this.$confirm(i18n.t("common.tip_info.common_info"), i18n.t("common.tip_info.tips"), {
                    confirmButtonText: i18n.t("common.btn.ok"),
                    cancelButtonText: i18n.t("common.btn.cancel"),
                    type: 'warning'
                }).then(() => {
                    const _data = { url: jsonUrl, params: params }
                    this.$store.dispatch('fmsCommon/actionPost', _data)
                        .then(data => {
                            ...
                            this.initChartTb(url,params1,url1,params2,isNeedRender)
                        }).catch(err => {
                            console.error(err)
                        })
                }).catch(() => {
      ...
      // 如有数据初始渲染结构图
      /**  async initChart(url, params,isNeedRender) {
          let _data = { url: url, params: params }
          let response = await new Promise((resolve,rej) =>this.$store.dispatch('fmsCommon/actionPost', _data)
                        .then(data => {
                          this.initChartData = data;
                          this.graphJson = data.data[0];
                          this.graphId = data.data[0].ID;
                          if (isNeedRender) {
                            this.myDiagram.model = go.Model.fromJson(data.data[0].GRAPH_SCRIPT)
                          }
                          resolve(data.data)
                        }).catch(err => {
                            console.error(err)
   ...
   init() {
        ...
        new go.Diagram("myDiagramDiv",  
          {
            ...
            initialScale: 0.9, //拖入画板后形成的图形初始化大小是原来的0.85倍
      ...
      myDiagram.addDiagramListener("ChangedSelection", (e) => {
          if(e.subject && e.subject.$e && e.subject.$e.value)  this.selectWfNode(e.subject.$e.value)
            //  let n = []
        ...
        new go.Palette("myPaletteDiv",  // must name or refer to the DIV HTML element
          {
            ...
            // { text: "开始", figure: "Ellipse", "size":"75 75", fill: "#00AD5F" },
              { text: "开始", figure: "RoundedRectangle",nodeType:'NORMAL', fill: "#075B46",start:'Y' },
              // { text: "Step" },
              // { text: "DB", figure: "Database", fill: "lightgray" },
              // { text: "条件", figure: "Diamond", fill: "lightskyblue",nodeType:'COND',start:'N' },
              { text: "常规", figure: "Ellipse", "size":"75 75", fill: "#0DC2C4" ,nodeType:'NORMAL' ,start:'N',},
              { text: "会签", figure: "Rectangle", "size":"75 75", fill: "#1CFC88" ,nodeType:'GROUP',start:'N'},
              { text: "动态", figure: "Ellipse", "size":"75 75", fill: "#03AB41" ,nodeType:'DYNAMIC',start:'N'},
              { text: "结束", figure: "RoundedRectangle", fill: "#CE0620",nodeType:'NORMAL' ,start:'N'},
   ...
   initDocumentModel(){
      ...
      onNodeConfigChange(v,item,row) {
        switch(item.field) {
          case 'nodeColor':
            this.myDiagram.model.setDataProperty(this.currSelectNode.data, 'fill', row.nodeColor)
            break;
          case 'textColor':
            this.myDiagram.model.setDataProperty(this.currSelectNode.data, 'txtColor', row.textColor)
            break;  
          case 'textContent':
            this.myDiagram.model.setDataProperty(this.currSelectNode.data, 'text', row.textContent)
            break;  
          case 'fontConfig':
            this.myDiagram.model.setDataProperty(this.currSelectNode.data, 'font', row.fontConfig)
            break;
          case 'strokeColor':
            this.myDiagram.model.setDataProperty(this.currSelectNode.data, 'shapeStrokeColor', row.strokeColor)
            break;
          case 'strokeWidth':
            this.myDiagram.model.setDataProperty(this.currSelectNode.data, 'shapeStrokeWidth', row.strokeWidth)
            break;
