<template>
    <div ref="myref" v-bind="$attrs" class="svg-container" >
      ...
      <!-- 使用 path 绘制正方形 -->
      <path fill-rule="nonzero" d="M 0 0 L 40 0 L 40 40 L 0 40 Z" fill="rgba(128, 128, 128, 0.5)" stroke="rgba(128, 128, 128, 1)" stroke-width="1" />
      <!-- 在正方形中间添加数字 -->
      <text x="50%" y="60%" text-anchor="middle" fill="#000" font-size="12">节点2</text>
    </svg>
    </div>
  </template>
  <script>
  export default {
    ...
    methods: {
      getStyleCols() {
        return [
          { field: 'width', title: '长度', queryType: 'input', defaultValue: '40px', unit: 'px' },
      ...
      getAttributeCols() {
        return [
          { field: 'text', title: '工艺节点', queryType: 'select', require: false, query: true, hide: false, defaultValue: '节点2' },
      ...
      createSvgPath() {
        let containerBox = "M 0 0 L 40 0 L 40 40 L 0 40 Z"
        return containerBox;
      },
      handleDragEnd() { }
    },
