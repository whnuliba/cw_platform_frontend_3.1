...
<script>
export default {
  name: 'CwTopVerticalEndTopContactSvg',
  components: {},
  props: {
    componentModalObj: {
      type: Object,
  ...
  methods: {
    ...
    createSvgPath(createCount = 6, length = 30) {
      ...
      let contact_w = 1;
      let contact_h = 4;
      // let outBox = `M 0 0  v ${width-length} h ${boxSize} v ${length} h ${length-boxSize}  v ${-width}  h ${-length} z M ${boxSize} ${width} l ${(length-boxSize*2)/2} ${-interval+boxSize} l ${(length-boxSize*2)/2} ${interval-boxSize} v  ${-boxSize} l ${-(length-boxSize*2)/2} ${-interval+boxSize} l ${-(length-boxSize*2)/2} ${interval-boxSize} zX `

      let outBox = `M 0 0  h ${length} v ${width} h ${-length + boxSize
        } v ${-length}  h ${-boxSize} v ${length - width} z M ${boxSize} ${width} l ${(length - boxSize * 2) / 2
