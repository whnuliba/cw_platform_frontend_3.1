      ...
      <div class="combox_title">
        <el-tabs v-model="activeName" :stretch="true">
          <el-tab-pane  name="first">
            ...
            <el-menu default-active="2" class="el-menu-vertical-demo" :style="{height: compLabelHeight}">
              <el-submenu  v-for="(comval, comi,it) in newCompData" :index="String(it)">
                <template slot="title">
                  <span>{{comi}}</span>
                </template>
...
import ShowSvg from "./components/showSvg/index.vue"
export default {
      name: 'CwLeftArea',
  ...
  watch: {
    isCanvaview(newVal, oldVal) {
      ...
      this.newCompData = this.isCanvaview ? this.canvascompData : this.compData; ;
    }
  },
      data() {
        ...
        //     return aa;
        //   }
        const componentsClassification = (bolean) => {
          let aa = [];
          for (let key in modules) {
            if (modules[key].methods.hasOwnProperty('comText') && (key !== 'CwDesignDialogsPreview' && key !== 'CwSubPanelPreview' && key !== 'CwWorkFlowPreview'
              && key !== 'CwIteratorView'
              && key !== 'CwSubPanelPreview'
              && key !== 'CwUCPanelPreview'
              && key !== 'CwTabPanelView'
              && key !== 'CwDialogPanelView'
              && key !== 'CwDrawerPanelView'
            )) {
              let obj = {
                title: modules[key].methods.comText(),
                compName: key,
                classification: modules[key].methods.ificationType()?.classification,
                isCanvasview: modules[key].methods.ificationType()?.isCanvasview ? modules[key].methods.ificationType()?.isCanvasview : false,
                modules: modules[key],
              }
              if (bolean == true) {
                if (modules[key].methods.ificationType()?.isCanvasview == true) {
                  aa.push(obj)
                }
              } else {
                aa.push(obj)
        ...
        const _initdata = {
          ...
          canvascompData:componentsClassification(true),
          compData:componentsClassification(false),
          newCompData:componentsClassification(false),
          activeName:'first',
          menuData:[],
          compLabelHeight: '',
        }
