<template>
  <div
    id="myScrollbarContentPanel"
    ref="myScrollbarContentPanel"
    @dragover="handleDragOver"
    @dragenter="handleDragEnter"
    @drop="handleDrop"
    class="my-scrollbar-content"
    :style="{ transform: 'scale(' + this.scaleValue + ')' }"
    @wheel="(e)=>{panelScroll(e)}"
  ...
  >
    ...
    <CwRuler
      v-if="!isNeedUseCanvasDraw"
      :h_width="panelStyle.width"
      :v_height="panelStyle.height"
    ...
    />
    <div
      class="myScrollbarContentPanel_main"
      :style="{position: 'absolute',width:panelStyle.width+'px',height:panelStyle.height+'px'}"
      tabindex="0"
      @keyup.stop="movePanel"
    >
      <!-- <CwCanvasDraw v-if="isNeedUseCanvasDraw"
        :width="panelStyle.width"
        :height="panelStyle.height" :panelStyle="panelStyle" 
      ...
      <!-- <CwRightMenu
        :class-index="rightclickInfo.menuId"
...
import CwRuler from "./CwRuler.vue";
import CwGrid from "./CwGrid.vue";
...
import CwCanvasDraw from "./CwCanvasDraw.vue";
...
export default {
  ...
  watch: {
    componentData: {
      handler: function (newVal, oldVal) {
        ...
        this.isNeedUseCanvasDraw && this.redrawCanvas(this.lineDatas);
      },
      deep: true,
      // immediate: true,
    },
    ...
    'panelStyle.drawClear': {
        handler: function (newVal, oldVal) {
          if (newVal) {
            this.lineDatas = [];
            // this.$store.commit('app/SET_LINES_DATA',this.lineDatas);
    ...
    //  是否使用画布Canvas
    'panelStyle.isUseCanvas': {
        handler: function (newVal, oldVal) {
          if (newVal) {
            this.isNeedUseCanvasDraw = true;
            this.redrawCanvas(this.lineDatas);
            this.$store.commit('app/IS_CANVAS_VIEW',true);
          } else {
            this.isNeedUseCanvasDraw = false;
            this.canDrawAndConnectLine = false;
            this.$store.commit('app/IS_CANVAS_VIEW',false);
          }
        },
        deep: true
    },
    //  清除画布所有内容
    'panelStyle.isConnectLine': {
        handler: function (newVal, oldVal) {
          if (newVal) {
            this.canDrawAndConnectLine = true;
          } else {
            this.canDrawAndConnectLine = false;
          }
        },
          ...
          handler: function (newVal, oldVal) {
              if (newVal) {
                  this.lineDatas.forEach((item,i) => {
                    if(newVal.fieldName === item.startPoint[0].fieldName || newVal.fieldName === item.endPoint[0].fieldName){
                      this.lineDatas.splice(i,1)
                    } 
    ...
    //   handler: function (newVal, oldVal) {
    //     if (newVal != oldVal) {
    //       let obj = this.componentData[oldVal];
    //       if(!obj)
    //         return
  ...
  data() {
    const _initdata = {
      ...
      nodePosition: {},
      lineDatas: [],
      canMouseMoveLineConnect: false,
      lastClickTime: 0,
      isNeedUseCanvasDraw: false,
      keyC: true,
      componentModalObj: {},
      allAttributeData: {},
      // itemStyle: {},
  ...
  beforeDestroy() {
    // document.getElementById("myScrollbarContentPanel").removeEventListener('keydown', this.movePanel)
  },
  created() {},
  mounted() {
    this.canvasLine = document.getElementById("paintingCanvas");
    this.ctx = this.canvasLine.getContext('2d');
    this.$bus.$on("selevtComponentData", (data) => {
      this.componentData = data;
    });
    this.$bus.$on("selevtLineData", (data) => {
      this.lineDatas = data;
      this.redrawCanvas(this.lineDatas);
    });
    ...
    this.$bus.$on("fieldName", (val, itemData) => {
      ...
      // if( Object.keys(this.allAttributeData).length === 0){
      //   console.log(itemData.id)
      //   this.allAttributeData[itemData.id] = {
      //     id:itemData.id,
      //     fieldName:val ? val : 'null',
      //   }
      // }else{
      //   for(let i in this.allAttributeData){
      //     let item = this.allAttributeData[i];
      //     if(itemData.id!= item.id){
      //       this.allAttributeData[itemData.id] = {
      //         id:item.id,
      //         fieldName:val ? val : 'null',
    ...
    this.$nextTick(() => {
      ...
      // 让 Canvas 获得焦点
      Canvas.focus();
      Canvas.addEventListener('keydown', this.myScrollbarContentPanelKeydown);
      ...
      this.initElement();
      // this.initRuler();
      this.drawRuler(ctx); // 初始化标尺
      this.drawGrid(ctx); // 初始化网格
      // 初始化网格
      // this.$bus.$on("allStyleData", (val) => {
      //   let obj = JSON.parse(JSON.stringify(val));
      //   for (let i in obj) {
      //     for (let t in obj[i]) {
      //       styleData[i.split("-")[0]].forEach((item) => {
      //         if (item.unit) {
      //           if (t == item.field) {
      //             obj[i][t] = obj[i][t] + item.unit;
      //           }
      //         }
      //       });
      //     }
      ...
      //   this.itemStyle = obj;
      // });
      // this.$bus.$on("moveData", (moveobj) => {
      //   this.componentData.forEach(item=>{
      //     if(item.id === moveobj.id){
      //       item.top = moveobj.top+ "px";
      //       item.left = moveobj.left+ "px";
      //     }
      //   })
      // });
      this.$bus.$on("attributeData", (attributeObj) => {
        this.componentData.forEach((item) => {
          if (item.id === attributeObj.compId.split("-")[1]) {
            ...
            if( this.isNeedUseCanvasDraw && modules[item.component].methods.changeWidthHeight) {
              let arr = []
  ...
  methods: {
    myScrollbarContentPanelKeydown(event) {
      ...
      let newLines = this.lineDatas.map((item, i) => {
          if (item.startPoint[0].fieldName === left.fieldName && item.startPoint[1].fieldName === top.fieldName) {
              switch (item.startPoint[0].direction) {  // 判断线开始节点的连接方向
                case 'right': 
                      moveL = {locating:left.locating + left.nodeWidth,fieldName:left.fieldName,direction:'right',lineAttr: {direction:'from',color:'lightGreen',width:2} },
                      moveT = {locating:top.locating+ top.nodeHeight/2,fieldName:top.fieldName,direction:'right'};
                      item.startPoint = [moveL, moveT]
                  break;
                  case 'left':
                      moveL = {locating:left.locating,fieldName:left.fieldName,direction:'left',lineAttr: {direction:'from',color:'lightGreen',width:2} },
                      moveT = {locating:top.locating+ top.nodeHeight/2,fieldName:top.fieldName,direction:'left'};
                      item.startPoint = [moveL, moveT] 
                  break;
                  case 'bottom':
                      moveL = {locating:left.locating + left.nodeWidth/2,fieldName:left.fieldName,direction:'bottom',lineAttr: {direction:'from',color:'lightGreen',width:2} },
                      moveT = {locating:top.locating+ top.nodeHeight,fieldName:top.fieldName,direction:'bottom'};
                      item.startPoint = [moveL, moveT] 
                  break;
                  case 'top':
                      moveL = {locating:left.locating + left.nodeWidth/2,fieldName:left.fieldName,direction:'top',lineAttr: {direction:'from',color:'lightGreen',width:2} },
              ...
              switch (item.endPoint[0].direction) { // 判断线结束节点的连接方向
                case 'right':
                      eMoveL = {locating:left.locating + left.nodeWidth,fieldName:left.fieldName,direction:'right',lineAttr: {direction:'to',color:'lightGreen',width:2} },
                      eMoveT = {locating:top.locating+ top.nodeHeight/2,fieldName:top.fieldName,direction:'right'};
                      item.endPoint = [eMoveL, eMoveT] 
                  break;
                  case 'left':
                      eMoveL = {locating:left.locating,fieldName:left.fieldName,direction:'left',lineAttr: {direction:'to',color:'lightGreen',width:2} },
                      eMoveT = {locating:top.locating+ top.nodeHeight/2,fieldName:top.fieldName,direction:'left'};
                      item.endPoint = [eMoveL, eMoveT] 
                  break;
                  case 'bottom':
                      eMoveL = {locating:left.locating + left.nodeWidth/2,fieldName:left.fieldName,direction:'bottom',lineAttr: {direction:'to',color:'lightGreen',width:2} },
                      eMoveT = {locating:top.locating+ top.nodeHeight,fieldName:top.fieldName,direction:'bottom'};
                      item.endPoint = [eMoveL, eMoveT] 
                  break;
                  case 'top':
                      eMoveL = {locating:left.locating + left.nodeWidth/2,fieldName:left.fieldName,direction:'top',lineAttr: {direction:'to',color:'lightGreen',width:2} },
      ...
      if (this.canMouseMoveLineConnect) {
        ...
        newVal.forEach((item,i) => {
            ...
            if (  ( pBottomL[1] - position.y <=parseInt(item.styles.height) && position.y - pTopL[1]<= parseInt(item.styles.height) ) &&
                ...
                let x1 = {locating: leftX, fieldName: item.fieldName, direction: "left",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[leftX, item.fieldName,"left"];
                ...
                let x1 = {locating: rightX, fieldName: item.fieldName, direction: "right",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[rightX,item.fieldName,"right"];
                ...
                let x1 = {locating: bottomX, fieldName: item.fieldName, direction: "bottom",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[bottomX,item.fieldName,"bottom"];
                ...
                let x1 = {locating: topX, fieldName: item.fieldName, direction: "top",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[bottomX,item.fieldName,"bottom"];
    ...
    mouseUpEvent(e) { 
      ...
      newVal.forEach((item,i) => {
          ...
          if ( ( pBottomL[1] - position.y <=parseInt(item.styles.height) && position.y - pTopL[1]<= parseInt(item.styles.height) ) &&
                        (pBottomL[0]  - position.x>=10 && pBottomL[0]  - position.x<= Math.sqrt(Math.pow(parseInt(item.styles.height)/2,2)+100)) &&
                      (pTopL[0]  - position.x>=10 && pTopL[0]  - position.x <= Math.sqrt(Math.pow(parseInt(item.styles.height)/2,2)+100)) ){
              this.endX = {locating: leftX, fieldName: item.fieldName, direction: "left",lineAttr:  {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[leftX, item.fieldName,"left"]; 
              ...
              if (this.startPosition0.fieldName !== this.endX.fieldName && this.startPosition0.direction === 'bottom') {  
                  ...
                  endPoint:[
                    {locating: topX, fieldName: item.fieldName, direction: "top",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}},
                  ...
                  this.lineDatas.push(
                    {
                      startPoint:[this.startPosition0,this.startPosition1], 
                      endPoint:[
                      {locating: bottomX, fieldName: item.fieldName, direction: "bottom",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}},
                      ...
                      data: {from: this.startPosition0.fieldName,to: this.endX.fieldName}
                        ...
                        (position.x - pBottomR[0] >=10 && position.x - pBottomR[0] <= Math.sqrt(Math.pow(parseInt(item.styles.height)/2,2)+100)) &&
                      (position.x - pTopR[0] >=10 && position.x - pTopR[0] <= Math.sqrt(Math.pow(parseInt(item.styles.height)/2,2)+100)) )){
              this.endX = {locating: rightX, fieldName: item.fieldName, direction: "right",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[rightX, item.fieldName,"right"]; 
                  ...
                  this.lineDatas.push(
                    {
                      startPoint:[this.startPosition0,this.startPosition1], 
                      endPoint:[
                        {locating: leftX, fieldName: item.fieldName, direction: "left",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}},
                  ...
                  this.lineDatas.push(
                    {
                      ...
                      data: {from: this.startPosition0.fieldName,to: this.endX.fieldName}
                        ...
                        (position.y - pBottomL[1] >=10 && position.y - pBottomL[1] <= Math.sqrt(Math.pow(parseInt(item.styles.width)/2,2)+100)) &&
                      (position.y - pBottomR[1] >=10 && position.y - pBottomR[1] <= Math.sqrt(Math.pow(parseInt(item.styles.width)/2,2)+100)) ){
              this.endX = {locating: bottomX, fieldName: item.fieldName, direction: "bottom",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[bottomX, item.fieldName,"bottom"]; 
                  ...
                  this.lineDatas.push(
                    {
                      startPoint:[this.startPosition0,this.startPosition1], 
                      endPoint:[
                      {locating: rightX, fieldName: item.fieldName, direction: "right",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}},
                      ...
                      data: {from: this.startPosition0.fieldName,to: this.endX.fieldName}
                        ...
                        (pTopL[1] - position.y>=10 && pTopL[1] - position.y <= Math.sqrt(Math.pow(parseInt(item.styles.width)/2,2)+100)) &&
                      (pTopR[1] - position.y >=10 && pTopR[1] - position.y <= Math.sqrt(Math.pow(parseInt(item.styles.width)/2,2)+100)) ){
              this.endX = {locating: topX, fieldName: item.fieldName, direction: "top",lineAttr: {direction:'to',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[bottomX, item.fieldName,"bottom"]; 
    ...
    drawArrow(x1,y1, x2, y2) {
      ...
      this.ctx.fillStyle = 'lightgreen'; // 设置填充颜色
      this.ctx.fill(); // 实心填充
    },
    // 清除画布
    // clearCanvas() {
  ...
  handleLineClick(e) {
      ...
      this.lineDatas.map((line, index) => {
        ...
        if (distance < 5) { // 5 是一个阈值，可以根据需要调整
          ...
          return line.startPoint[0].lineAttr.color = 'lightgreen';
        }
      });
    },
  getCanvasPosition(e) {
  ...
  getCursorStyle(type) {
    ...
    onCanvasClick(event) {
      ...
      this.selectElementByPosition(mouseX, mouseY, event);
      ...
      if (selItem) {
        ...
        this.$bus.$emit('attributeData', attributeObj)
        let obj = {
          compId: selItem.component + '-' + selItem.id,
          data: {
            ...selItem.styles,
    ...
    selectElementByPosition(x, y, event) {
      // 清除所有元素的选中状态
      if(!event.ctrlKey){
        this.componentData.forEach(item => {
      ...
      for (let i = this.componentData.length - 1; i >= 0; i--) {
        if (!this.componentData[i].onCanvas) {
          continue; // 如果组件不在画布上，跳过
        }
        ...
        if (
          ...
          selectedItem = item;
          this.componentData[i].isSelected = true; // 设置选中状态
          break;
        }
      }
    ...
    onCanvasMouseDown(event) {
      ...
      let selectedItem = this.onselectElementByPosition(mouseX, mouseY);
      let tselectedItems = _this.componentData.filter(item => item.isSelected === true);
      ...
      // 鼠标按下后节点附近出现连接线
      if (_this.componentData.length > 0 && _this.canDrawAndConnectLine) {
                        // 连接线单机事件
                        ...
                        let newVal = _this.componentData;
                        const position = _this.getCanvasPosition(event);
                        _this.ctx.strokeStyle = "lightgreen";
                        ...
                        newVal.forEach((item,i) => {
                            ...
                            (position.x - pTopR[0] >=10 && position.x - pTopR[0] <= Math.sqrt(Math.pow(parseInt(item.styles.height)/2,2)+100)) )
                            {
                                // 绘制起点
                                console.log("绘制右起点圆点",'===================================>')
                                _this.startPosition0 = {locating: rightX, fieldName: item.fieldName, direction: "right",lineAttr: {direction:'from',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`} } //[rightX, item.fieldName,'right']
                                ...
                                // console.log(_this.startPosition0.locating,_this.startPosition1.locating+(position.y - _this.startPosition1.locating),"折行坐标",
                                // _this.startPosition0.locating- 150,_this.startPosition1.locating+(position.y - _this.startPosition1.locating))
                            ...
                            (pTopL[0]  - position.x>=10 && pTopL[0]  - position.x <= Math.sqrt(Math.pow(parseInt(item.styles.height)/2,2)+100))
                                ...
                                // 绘制起点
                                console.log("绘制左起点圆点",'===================================>')
                                _this.startPosition0 = {locating: leftX, fieldName: item.fieldName, direction: "left",lineAttr:  {direction:'from',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[leftX, item.fieldName,"left"]
                            ...
                            (position.y - pBottomR[1] >=10 && position.y - pBottomR[1] <= Math.sqrt(Math.pow(parseInt(item.styles.width)/2,2)+100)) ) {
                                // 绘制起点
                                console.log("绘制底部起点圆点",'===================================>')
                                _this.startPosition0 = {locating: bottomX, fieldName: item.fieldName, direction: "bottom",lineAttr:  {direction:'from',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[bottomX, item.fieldName,"bottom"]
                            ...
                            (pTopR[1] - position.y >=10 && pTopR[1] - position.y <= Math.sqrt(Math.pow(parseInt(item.styles.width)/2,2)+100)) ) {
                                // 绘制起点
                                console.log("绘制顶部起点圆点",'===================================>')
                                _this.startPosition0 = {locating: topX, fieldName: item.fieldName, direction: "top",lineAttr:  {direction:'from',color:'lightgreen',width:2,shadowColor:`rgba(0,0,0,0.5)`}} //[bottomX, item.fieldName,"bottom"]
        ...
        function onMouseMoveDraw(event) {
          ...
          if (selItem.isSelected && !(selectedItems.length > 1)) {
            ...
            _this.componentData.forEach(other => {
          ...
          // _this.componentData.forEach((item) => {
    ...
    myScrollbarContentPanelClick(event){
      let isitemselect = this.componentData.some(item => item.isSelected === true)
      if(this.$refs.Shaperef && event.button === 0 && !event.altKey && !isitemselect) {
    ...
    startSelection(event) {
      ...
      if (event.button === 0 && event.altKey) { // 左键且未按 Alt 键
        ...
        const containerRect = this.$refs.myScrollbarContentPanel.getBoundingClientRect();
        this.selectionRect = {
        ...
        this.$refs.myScrollbarContentPanel.appendChild(overlay);
  ...
  endSelection() {
    ...
    if (this.overlay) {
      this.$refs.myScrollbarContentPanel.removeChild(this.overlay);
      this.overlay = null;
    }
  },
  checkSelectedElements() {
      if (!this.isNeedUseCanvasDraw) {
        this.selectedElements = [];
        const allSelectableElements = this.$refs.myScrollbarContentPanel.querySelectorAll('.shape'); // 确保你的组件有 .selectable 类
  ...
  isElementInRect(elementRect) {
    ...
    movePanel(e){
      ...
      if(e.ctrlKey && e.keyCode === 86) {
        ...
        if (!this.copyData.data) {
          ...
          this.componentData.push(component_data);
      }
    ...
    handleDrop(event) {
      ...
      if (componentData.action == "setLocation") {
        // this.$refs[componentData.compId][0].$el.style.left =
        //   event.offsetX - componentData.offsetX + "px";
        // this.$refs[componentData.compId][0].$el.style.top =
        //   event.offsetY - componentData.offsetY + "px";
      } else {
        let uuid = this.uuid();
        this.maxNumber = 0;
        let i =
          this.getCompMaxIndex(this.componentData, componentData.name) + 1;
        ...
        let component = {
          ...
          styles: {
            position: "relative",
            left: this.isNeedUseCanvasDraw ? localX + "px" :event.offsetX - componentData.offsetX + "px",
            top: this.isNeedUseCanvasDraw ? localY + "px" : event.offsetY - componentData.offsetY + "px",
          },
          componentText: "",
          handler: {},
          elementCode: "",
          ...
          isSelected:false,
          onCanvas: this.isNeedUseCanvasDraw ? true : false,
        };
        for (let key in modules) {
          if (
            modules[key].methods.hasOwnProperty("getStyleCols") &&
            modules[key].name == componentData.name
          ) {
          ...
          if (modules[key].methods.hasOwnProperty("getAttributeCols") && modules[key].name == componentData.name){
    ...
    drawRuler(ctx) {
      const lineColor = "#CBCCCC";
      const textColor = "#CBCCCC";
      const rulerStep = 10;
      const rulerMajorStep = 50;
      ctx.strokeStyle = lineColor;
      ctx.fillStyle = textColor;
      // 水平标尺（顶部）
      ctx.beginPath();
      for (let x = 0; x <= this.canvas.width; x += rulerStep) {
        ctx.moveTo(x, 0);
        if (x % rulerMajorStep === 0) {
          ctx.lineTo(x, 15);
          ctx.fillText(x, x, 15);
        } else {
          ctx.lineTo(x, 7);
      ...
      // 垂直标尺（左侧）
      ctx.save();
      ctx.translate(0, 0); // 移除原 translate(20,0)
      ctx.rotate(Math.PI / 2); // 旋转90度使垂直方向朝上
      ctx.beginPath();
      for (let y = 0; y <= this.canvas.height; y += rulerStep) {
        ctx.moveTo(y, 0); // 原点在顶部
        if (y % rulerMajorStep === 0) {
          ctx.lineTo(y, -15); // 延伸长度
          // 调整文本位置到右侧显示
          ctx.fillText(y, y + 3, -3); // x偏移+3，y偏移-3（旋转后的坐标系）
        } else {
          ctx.lineTo(y, -7);
        }
      }
    ...
    drawGrid(ctx) {
      ...
      ctx.lineWidth = 1 / this.scale; // 动态线宽适应缩放
      ctx.stroke();
    },
    ...
    initRuler() {
      ...
      ctx.clearRect(0,0,this.drawingWidth,this.drawingHeight)
      ctx.beginPath();
      for (let i = 10.5; i < this.panelStyle.width; i += 5) {
        ctx.moveTo(i, 0.5);
        if (i % 50 == 0.5) {
          {
            ctx.lineTo(i, 15.5);
      ...
      for (let i = 10.5; i < this.panelStyle.height; i += 5) {
    ...
    updateCanvasSize() {
      if (this.canvas) {
        const canvasWidth = this.panelStyle.width || 1920;
        const canvasHeight = this.panelStyle.height || 1080;
    ...
    // handleScroll(e) {
    //   const { ctrlKey, altKey, metaKey, deltaY } = e;
    //   if (ctrlKey || altKey || metaKey) {
    //     e.preventDefault();
    //     const canvasRect = this.$refs.myScrollbarContentPanel.getBoundingClientRect();
    //     const mouseX = e.clientX - canvasRect.left;
    //     const mouseY = e.clientY - canvasRect.top;
    //     let newScale = this.scaleValue - (deltaY > 0 ? this.contentStep : -this.contentStep);
    //     newScale = Math.max(this.contentMinScale, Math.min(this.contentMaxScale, newScale));
    //     const centerX = (mouseX - this.contentOffsetX) / this.scaleValue;
    //     const centerY = (mouseY - this.contentOffsetY) / this.scaleValue;
    //     this.scaleValue = newScale;
    //     this.contentOffsetX = mouseX - centerX * newScale;
    //     this.contentOffsetY = mouseY - centerY * newScale;
    //     this.$refs.myScrollbarContentPanel.style.transform = `translate(${this.contentOffsetX}px, ${this.contentOffsetY}px) scale(${newScale})`;
    //     this.initRuler();
    //   }
    // },
    ...
    openSvgMenu(event, selectedItem) {
      ...
      let selData = this.componentData.filter(item => item.isSelected === true && item.onCanvas === true).map(item=>item.id);
    ...
    //     styles: {
    //       ...this.copyData.data.styles,
    //       left: params.e.offsetX + "px",
    //       top: params.e.offsetY + "px",
    //     },
    //     handler: {},
    //     elementCode: "",
    //     tabPanels: [],
    //     fieldName: this.copyData.data.component,
    //     attribute: {
    //       fieldName: this.copyData.data.component,
    //     },
    //   };
    ...
    deleteData(compId) {
        for (var i = 0; i < this.componentData.length; i++) {
          if (
            this.componentData[i].component + "-" + this.componentData[i].id == compId
          ) {
      ...
      //   this.rmenuCopy({'compId':compId,'component':component})
      // if(ctrlKey && e.keyCode == 86)
      //   this.rmenuPaste()
    },
    savaQueueManager (){
      this.$emit("savaQueueManager",this.componentData)
    }
  },
};
