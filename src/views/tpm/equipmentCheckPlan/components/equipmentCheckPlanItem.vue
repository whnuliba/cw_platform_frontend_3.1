...
export default {
  ...
  data() {
    const _initdata = {
      ...
      calendarNo: '',
      url: {
        add: '/equipmentCheckPlanItem/add',
        del: '/equipmentCheckPlanItem/del',
        list: '/equipmentCheckPlanItem/guest/get-equipmentPlanItemByPartNo',
        updateContent: '/equipmentCheckPlanItem/updateCheckContent',
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        ...
        { field: 'paramName', title: '检查内容', align: 'left', query: false, queryType: 'input', set: true, require: true, columnType: 'textareainput',layout:"row",width: 240, },
        { field: 'checkWay', title: '检查方式', align: 'left', query: false, queryType: 'input', editReadonly: true, set: true, require: true, columnType: 'textareainput',layout:"row",width: 240,  },
        { field: 'checkRequire', title: '检查要求', align: 'left', query: false, queryType: 'input', set: true, require: true, columnType: 'textareainput',layout:"row",width: 240, },
        { field: 'valueDown', title: '下限值', align: 'left', query: false, queryType: 'input', set: true, require: true,width: 80, },
        { field: 'checkContent', title: '检查结果', align: 'left', query: false, queryType: 'input',columnType: 'input', layout: "row", set: true, require: true },
        ...
        { field: 'lastModifyTime', title: '结果录入时间', align: 'center', width: '145px', queryType: 'input', set: false, require: false },
      ]
    }
  ...
  methods: {
    ...
    saveContent(row) {
      this.$confirm(('确认保存'), i18n.t('common.tip_info.tips'), {
        ...
        const _data = {
          url: this.url.updateContent, params: { data: { id: row.id, content: row.checkContent } }
        }
        this.$store.dispatch('fmsCommon/actionTpmPost', _data)
          .then(data => {
            let _this = this
