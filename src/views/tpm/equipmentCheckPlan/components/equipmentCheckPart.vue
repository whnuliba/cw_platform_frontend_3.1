    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="equipmentMtPartRowClick">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="160">
        ...
        <template slot-scope="scope">
          ...
          <el-button type="warning" v-ele-show="{btnCode:'equipment_check_part_finish'}" v-if="scope.row.status != 106" size="small" @click="edit(scope.row)">
            {{ "完成" }}
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      equipmentMtId: '',
      url: {
        add: '/equipmentCheckPart/add',
        del: '/equipmentCheckPart/del',
        list: '/equipmentCheckPart/guest/get-equipmentCheckPartHed',
        finish:'/equipmentCheckPart/get-completeCheckPart'
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        ...
        { field: 'checkResult', title: '点检结论', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'checkState', title: '结论状态', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'isQualified', title: '是否合格', align: 'left', query: false, queryType: 'select', set: true, require: true },
        // { field: 'checkTime', title: '点检完成时间', align: 'left', query: false, queryType: 'datetime', set: true, require: true },
        { field: 'checkTime', title: '点检完成时间', align: 'center', width: '145px', queryType: 'input', set: false, require: false },
        { field: 'checkPerson', title: '点检人员', align: 'left', query: false, queryType: 'input', set: false, require: false },
