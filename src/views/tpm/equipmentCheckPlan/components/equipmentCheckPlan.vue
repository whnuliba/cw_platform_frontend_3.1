  ...
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="equipmentMtPlanRowClick" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="200">
        ...
        <template slot-scope="scope">
          <el-button type="warning" v-ele-show="{btnCode:'equipment_check_plan_finish'}" v-if="scope.row.status != 106&&scope.row.status != 103" size="small" @click="finish(scope.row)">
            {{ "完成" }}
          </el-button>
          &nbsp;
          <el-button type="warning" v-ele-show="{btnCode:'equipment_check_plan_close'}" v-if="scope.row.status != 106&&scope.row.status != 103" size="small" @click="close(scope.row)">
            {{ "关闭" }}
          </el-button>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      value1: new Date(),
      url: {
        add: '/equipmentCheckPlan/add',
        del: '/equipmentCheckPlan/del',
        list: '/equipmentCheckPlan/list',
        finish: '/equipmentCheckPlan/get-completeCheckPlan',
        close: '/equipmentCheckPlan/get-closedCheckPlan',
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        ...
        { field: 'planStartTime', title: '计划点检开始', align: 'center', width: '145px', queryType: 'datetime', set: false, require: false },
        { field: 'checkPerson', title: '点检人', align: 'center', width: '145px', queryType: 'input', set: false, require: false },
        { field: 'plnaEndTime', title: '计划点检结束', align: 'center', width: '145px', queryType: 'rangedatetime', query: true, set: false, require: false },
