    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column v-for="(item) in cols" v-if="!item.hide" v-show="!item.hide" :key="item.id" :label="commonI18n(item)" :prop="item.field" :width="item.width" :min-width="item.minWidth" :align="item.align" show-overflow-tooltip>
        <template slot-scope="{row}">
          <el-input v-if="item.layout==='row' && item.columnType==='input'"  v-model="row[item.field]" :placeholder="commonI18n(item)" style="width: 90%" class="filter-item">
          </el-input>
          <el-input resize="none" type="textarea" autosize v-else-if="item.layout==='row' && item.columnType==='textareainput'"  v-model="row[item.field]" :placeholder="commonI18n(item)" style="width: 95%" class="filter-item" disabled>
          </el-input>
          <span v-else>
      ...
      <el-table-column align="center" :label="$t('common.oper')" fixed="right" width="80">
        <template slot="header" slot-scope="scope">
          {{ $t('common.oper') }}
          <i style="margin-left: 10px;cursor:pointer;" class="el-icon-bank-card" @click="selectFields" />
        </template>
        <template slot-scope="scope">
          <el-button type="warning" v-ele-show="{btnCode:'update_content'}" size="small" @click="saveContent(scope.row)">
            {{ "保存" }}
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      equipmentMtId: '',
      url: {
        add: '/lubricationPlanDetail/add',
        del: '/lubricationPlanDetail/del',
        list: '/lubricationPlanDetail/getLubricationPlanDetailByHedId',
        updateContent: '/lubricationPlanDetail/add',
      },
      queryParams: {
        requestData: { }
      },
      initSelect: {
        ...
        isMust: [{ label: '否', value: 0 }, { label: '是', value: 1 }],
        status: []
      },
      defaultForm: {
        row: {},
      ...
      cols: [
        { field: 'paramNo', title: '参数编号', align: 'left', query: false, queryType: 'input',set: true, require: true },
        { field: 'lubricationContent', title: '润滑内容', align: 'left', query: false, queryType: 'input', columnType:'textareainput', set: true, require: true,layout:"row",width: 240, },
        { field: 'lubricationWay', title: '检查方式', align: 'left', query: false, queryType: 'input',  columnType:'textareainput', editReadonly: true, set: true, require: true,layout:"row",width: 240, },
        { field: 'lubricationRequire', title: '润滑要求', align: 'left', query: false, queryType: 'input',  columnType:'textareainput', set: true, require: true, layout:"row",width: 240, },
        { field: 'lubricationResult', title: '润滑结果', align: 'left', query: false, queryType: 'input', columnType:'input', layout:"row", set: true, require: true },
        { field: 'valueDown', title: '下限值', align: 'left', query: false, queryType: 'input', set: true, require: true,width: 80, },
        { field: 'valueUp', title: '上限值', align: 'left', query: false, queryType: 'input', set: true, require: true,width: 80, },
        // { field: 'status', title: '状态', align: 'left', width: '80px', queryType: 'select', set: false, require: false },
        // { field: 'isMust', title: '是否必检', align: 'left', query: false, queryType: 'select', set: true, require: true },
        // { field: 'createTime', title: '创建时间', align: 'center', width: '145px', queryType: 'datetime', set: false, require: false },
        { field: 'lastModifyUser', title: '检查人', align: 'center', width: '145px', queryType: 'input', set: false, require: false,width: 80, },
        { field: 'lastModifyTime', title: '完成时间', align: 'center', width: '145px', queryType: 'input', set: false, require: false },
  ...
  watch: {
    equipmentMtPartId: function(val) {
      this.equipmentMtId = val
      this.getList()
      this.selectOptions()
    },
    workShiftCaNo: function(val) {
      this.calendarNo = val
    }
  },
  created() {
  ...
  methods: {
    ...
    saveContent(row) {
      this.$confirm(('确认保存'), i18n.t('common.tip_info.tips'), {
        confirmButtonText: i18n.t('common.btn.ok'),
        cancelButtonText: i18n.t('common.btn.cancel'),
        type: 'warning'
      }).then(() => {
        const _data = {
          url: this.url.updateContent, params: { data: { id: row.id, lubricationResult: row.lubricationResult }} }
        this.$store.dispatch('fmsCommon/actionTpmPost', _data)
          .then(data => {
            let _this = this
            this.$message({
              type: 'success',
              message: 'saveContent success!'
            })
            // this.query()
            const _data1 = { url: this.url.list, params: { data: this.equipmentMtId }}
            this.$store.dispatch('fmsCommon/actionTpmPost', _data1)
              .then(data => {
                data.forEach(item=>{
                  if(item.id == row.id){
