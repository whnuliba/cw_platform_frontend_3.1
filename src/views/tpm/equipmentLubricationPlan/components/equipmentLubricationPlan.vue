<template>
  <div>
    <div class="filter-container">
      ...
      <!--<el-button v-ele-show="{btnCode:'equipment_check_plan_add'}" type="warning" class="filter-item" @click="add">
        {{ $t("common.btn.add") }}
      </el-button>-->
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="equipmentMtPlanRowClick" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="200">
        ...
        <template slot-scope="scope">
          <el-button type="warning" v-ele-show="{btnCode:'equipment_lub_plan_finish'}"  v-if="scope.row.status != 106&&scope.row.status != 103" size="small" @click="finish(scope.row)">
            {{ "完成" }}
          </el-button>
          &nbsp;
          <el-button type="warning" v-ele-show="{btnCode:'equipment_lub_plan_close'}" v-if="scope.row.status != 106&&scope.row.status != 103" size="small" @click="close(scope.row)">
            {{ "关闭" }}
          </el-button>
  ...
  export default {
    ...
    data() {
      const _initdata = {
        ...
        value1: new Date(),
        url: {
          add: '/equipmentLubricationPlan/add',
          del: '/equipmentLubricationPlan/del',
          list: '/equipmentLubricationPlan/list',
          finish: '/equipmentLubricationPlan/get-completeLubrivationPlan',
          close: '/equipmentLubricationPlan/get-closedLubrivationPlan',
        },
        queryParams: {
          requestData: {}
        },
        ...
        initSelect: {
          ...
          isMt: [{ label: '否', value: 0 }, { label: '是', value: 1 }],
          workShift: [{ label: '否', value: 0 }, { label: '是', value: 1 }],
          status: []
        },
        defaultForm: {
          row: {},
        ...
        cols: [
          { field: 'checkNo', title: '单号', align: 'left', query: true, queryType: 'input', set: false, require: true },
          { field: 'deviceNo', title: '设备编号', align: 'left', query: true, queryType: 'filter-select', editReadonly: true, set: true, require: true },
          { field: 'deviceName', title: '设备名称', align: 'left', query: false, queryType: 'input', editReadonly: false, set: false, require: false },
          { field: 'classCode', title: '设备类型', align: 'left', query: true, queryType: 'select', set: false, require: false },
          { field: 'planStartTime', title: '计划润滑开始', align: 'center', width: '145px', queryType: 'datetime', set: false, require: false },
          { field: 'checkPerson', title: '润滑人员', align: 'center', width: '145px', queryType: 'input', set: false, require: false },
          { field: 'plnaEndTime', title: '计划润滑结束', align: 'center', width: '145px', queryType: 'rangedatetime', query: true, set: false, require: false },
          //{ field: 'mtPerson', title: '点检人员', align: 'left', query: false, queryType: 'input', set: false, require: true },
          //{ field: 'workShift', title: '班次', align: 'left', query: false, queryType: 'input', set: false, require: true },
          { field: 'workShift', title: '是否发起维修', align: 'left', query: false, queryType: 'select', set: false, require: true },
          { field: 'status', title: '状态', align: 'left', width: '80px', query: true, queryType: 'select', set: true, require: false },
    ...
    methods: {
      ...
      equipmentMtPlanRowClick(row, column, event) {
        this.$emit('equipmentMtPlanRowClick', row)
      },
      ...
      getList() {
        ...
        if(this.queryParams.requestData.plnaEndTime){
          this.queryParams.requestData['startTime'] = this.queryParams.requestData.plnaEndTime[0]
          this.queryParams.requestData['endTime'] = this.queryParams.requestData.plnaEndTime[1]
          // delete this.queryParams.requestData.plnaEndTime
        }
        let param = JSON.parse(JSON.stringify(this.queryParams)) 
        delete param.requestData.plnaEndTime
      ...
      finish(row) {
        this.$confirm(('确认将次保养计划完成'), i18n.t('common.tip_info.tips'), {
          confirmButtonText: i18n.t('common.btn.ok'),
          cancelButtonText: i18n.t('common.btn.cancel'),
          type: 'warning'
        }).then(() => {
          const _data = { url: this.url.finish, params: { data: row.id } }
          this.$store.dispatch('fmsCommon/actionTpmPost', _data)
            .then(data => {
              this.$message({
                type: 'success',
          ...
          this.$message({
            type: 'info',
            message: 'cancel finish'
          })
        })
      },
      close(row) {
        this.$confirm(('确认将次保养计划关闭'), i18n.t('common.tip_info.tips'), {
          confirmButtonText: i18n.t('common.btn.ok'),
          cancelButtonText: i18n.t('common.btn.cancel'),
          type: 'warning'
        }).then(() => {
          const _data = { url: this.url.close, params: { data: row.id } }
          this.$store.dispatch('fmsCommon/actionTpmPost', _data)
            .then(data => {
              this.$message({
                type: 'success',
