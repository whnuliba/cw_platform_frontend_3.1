  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <el-button type="warning" v-ele-show="{btnCode:'equipmentAlarm_edit'}" v-if="scope.row.status != 106" size="small" @click="edit(scope.row)">
            {{ "完成" }}
          </el-button>
        </template>
      </el-table-column>
  ...
  export default {
    ...
    data() {
      const _initdata = {
        ...
        value1: new Date(),
        url: {
          add: '/equipmentAlarm/add',
          del: '/equipmentAlarm/del',
          list: '/equipmentAlarm/list',
          finish: '/equipmentAlarm/finsh-equipmentAlarm'
        },
        queryParams: {
          requestData: {}
        },
        ...
        cols: [
          ...
          { field: 'alarmNo', title: '报警编码', align: 'left', query: false, queryType: 'input', editReadonly: true, set: true, require: true },
          { field: 'alarmName', title: '报警名称', align: 'left', query: false, queryType: 'input', editReadonly: true, set: true, require: true },
          { field: 'disposalWay', title: '处置方式', align: 'left', query: false, queryType: 'select' , set: true, require: true },
    ...
    methods: {
      ...
      selectOptions() {
        ...
        this.$store.dispatch('fmsCommon/actionTpmPost', _data)
          ...
          _data = { url: '/device-constant/guest/alarm-disposal-way', params: {} }
        this.$store.dispatch('fmsCommon/actionTpmPost', _data)
          .then(data => {
            this.initSelect.disposalWay = data.map(c => {
              return {
                label: c.name,
                value: c.value
              }
