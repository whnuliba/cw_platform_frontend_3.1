<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="150">
        <template slot="header" slot-scope="scope">
          {{ $t('common.oper') }}
          <i style="margin-left: 10px;cursor:pointer;" class="el-icon-bank-card" @click="selectFields" />
        </template>
        <template slot-scope="scope">
          <el-button type="warning" v-ele-show="{btnCode:'equipment_part_applay_edit'}" v-if="scope.row.recipientState == 101" size="small" @click="edit(scope.row)">
            {{ "执行" }}
          </el-button>
          <el-button type="warning" v-ele-show="{btnCode:'equipment_part_applay_finish'}" v-if="scope.row.recipientState == 103" size="small" @click="finishRow(scope.row)">
            {{ "完成" }}
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog v-el-drag-dialog :title="$t('common.btn.edit')" :visible.sync="dialogFormVisibleFinish" width="700px">
      <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="120px">
        <div v-for="(item) in finishCols" :key="item.id" style="display:inline">
          <el-form-item v-if="item.set && item.queryType==='input'"
                        :label="commonI18n(item)"
                        :prop="item.field"
                        label-width="120px"
  ...
  export default {
    ...
    data() {
      const _initdata = {
        ...
        value1: new Date(),
        url: {
          add: '/equipmentPartApplay/add',
          del: '/equipmentPartApplay/del',
          list: '/equipmentPartApplay/list',
          excute:'/equipmentPartApplay/excute-equipmentPartApplay',
          finish: '/equipmentPartApplay/finish-equipmentPartApplay'
        },
        queryParams: {
          requestData: {}   
        },
        ...
        cols: [
          ...
          { field: 'partNo', title: '部件编码', align: 'left', query: true, queryType: 'input', editReadonly: true,set: true, require: false },
          ...
          { field: 'recipientQty', title: '领用数量', align: 'left', query: false, queryType: 'input', set: true, require: true },
          { field: 'partSn', title: '备件条码', align: 'left', query: false, queryType: 'input', set: false, require: true },
          { field: 'recipientState', title: '领用状态', align: 'left', query: false, queryType: 'select', set: false, require: false },
          { field: 'completeTime', title: '完成时间', align: 'left', query: false, queryType: 'datetime', set: false, require: true },
          { field: 'person', title: '领取人', align: 'left', query: false, queryType: 'input', set: false, require: false },
        ...
        finishCols: [
          ...
          { field: 'materialNo', title: '备件料号', align: 'left', query: true, queryType: 'input', editReadonly: true,set: true, require: true },
          { field: 'materialName', title: '备件描述', align: 'left', query: true, queryType: 'input', editReadonly: true,set: true, require: true },
          { field: 'recipientQty', title: '领用数量', align: 'left', query: false, queryType: 'input', editReadonly: true, set: true, require: true },
          { field: 'partSn', title: '备件条码', align: 'left', query: false, queryType: 'input', set: true, require: true },
          { field: 'recipientState', title: '领用状态', align: 'left', query: false, queryType: 'select', set: false, require: false },
      ...
      Object.assign(_initdata.defaultForm.params, this.initData(_initdata.finishCols))
      return _initdata
    },
    created() {
    ...
    methods: {
      ...
      destory(hide = false) {
        ...
        this.dialogFormVisibleFinish = hide
      ...
      finishRow(row) {
        this.finish = true
        this.dialogFormVisibleFinish = true
        this.allowDisable = true
          ...
          if (valid) {
            this.$store.dispatch('fmsCommon/actionTpmPost', _data)
              .then(data => {
                ...
                this.dialogFormVisibleFinish = false
                this.query()
              }).catch(err => {
                console.error(err)
              }).finally(() => {
      ...
      selectOptions() {
        ...
        let _data = { url: '/device-constant/guest/recipient-states', params: {} }
        this.$store.dispatch('fmsCommon/actionTpmPost', _data)
          .then(data => {
            this.initSelect.recipientState = data.map(c => {
              return {
                label: c.name,
                value: c.value
              }
