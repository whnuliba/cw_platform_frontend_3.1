  ...
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'equipment_part_barcode_edit'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'equipment_part_barcode_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-el-drag-dialog :title="$t('common.btn.edit')" :visible.sync="dialogFormVisible" width="700px">
      <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="120px">
        <div v-for="(item) in cols1" :key="item.id" style="display:inline">
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      equipmentMtId: '',
      url: {
        add: '/equipmentPartBarcode/add',
        del: '/equipmentPartBarcode/del',
        edit: '/equipmentPartBarcode/edit',
        list: '/equipmentPartBarcode/guest/get-equipmentPartInventoryByPartNo'
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        { field: 'partNo', title: '部件编码', align: 'left', query: false, queryType: 'input', editReadonly: true, set: false, require: true },
        { field: 'barcode', title: '条码', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'inTime', title: '入库时间', align: 'left', query: false, queryType: 'datetime', set: true, require: true },
        { field: 'invState', title: '库存状态', align: 'left', query: false, queryType: 'select', set: true, require: true },
        { field: 'useQty', title: '已使用', align: 'left', query: false, queryType: 'input', set: false, require: true },
        { field: 'maxQty', title: '当前库存', align: 'left', query: false, editReadonly: false, queryType: 'input', set: true, require: true },
        { field: 'unit', title: '单位', align: 'left', query: false, queryType: 'input', set: true, require: true },
      ...
      cols1: [
        { field: 'partNo', title: '部件编码', align: 'left', query: false, queryType: 'input', editReadonly: true, set: false, require: true },
        { field: 'barcode', title: '条码', align: 'left', query: false, queryType: 'input',editReadonly: true, set: true, require: true },
        { field: 'inTime', title: '入库时间', align: 'left', query: false, queryType: 'datetime', set: true, require: true },
        { field: 'invState', title: '库存状态', align: 'left', query: false, queryType: 'select', set: true, require: true },
        { field: 'useQty', title: '已使用', align: 'left', query: false, queryType: 'input', set: false, require: true },
        { field: 'maxQty', title: '当前库存', align: 'left', editReadonly: false, query: false, queryType: 'input', set: true, require: true },
        { field: 'unit', title: '单位', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'status', title: '状态', align: 'left', width: '80px', queryType: 'select', set: false, require: false },
        { field: 'createTime', title: '创建时间', align: 'center', width: '145px', queryType: 'datetime', set: false, require: false }
      ]
  ...
  methods: {
    ...
    submit(formName) {
      ...
      this.defaultForm.params.partNo = this.equipmentMtId
    ...
    selectOptions() {
      const _data = { url: '/device-constant/guest/partNo-storage-states', params: {} }
      this.$store.dispatch('fmsCommon/actionTpmPost', _data)
        .then(data => {
          this.initSelect.invState = data.map(c => {
            return {
              label: c.name,
