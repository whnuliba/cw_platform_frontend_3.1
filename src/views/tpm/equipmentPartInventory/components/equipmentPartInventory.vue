<template>
  <div>
    <div class="filter-container">
      ...
      <el-button v-ele-show="{btnCode:'equipment_part_inventory_add'}" type="warning" class="filter-item" @click="add">
        {{ $t("common.btn.add") }}
      </el-button>
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="equipmentMtRowClick" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'equipment_part_inventory_edit'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'equipment_part_inventory_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="120px">
        <div v-for="(item) in cols" :key="item.id" style="display:inline">
          ...
          <el-form-item v-if="item.set && item.queryType==='select'"
            ...
            <el-select v-model="defaultForm.params[item.field]" placeholder="..." @change="(val)=>changeSelect(val,item)" style="width:180px;">
              <el-option v-for="item in initSelect[item.field]"
                         :key="item.value"
                         :label="item.label"
                         :value="item.value" />
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      value1: new Date(),
      url: {
        add: '/equipmentPartInventory/guest/get-createPartInventory',
        del: '/equipmentPartInventory/del',
        list: '/equipmentPartInventory/items',
        edit: '/equipmentPartInventory/edit',
      },
      queryParams: {
        requestData: { }
      },
      ...
      initSelect: {
        ...
        status: [],
        isWearingPart: [{ label: '否', value: 0 }, { label: '是', value: 1 }],
      },
      defaultForm: {
        row: {},
        params: {
      ...
      cols: [
        { field: 'deviceNo', title: '设备分类', minWidth: 150, align: "left" ,hide:false,query: true, queryType: 'select', set: true, width:150 },
        { field: 'partNo', title: '备件料号', align: 'left', query: false,hide:false, queryType: 'input', editReadonly: true, set: true, require: true, width:150 },
        { field: 'partName', title: '备件描述', align: 'left', query: false, queryType: 'input',editReadonly: true, set: true, require: true , width:300 },
        // { field: 'inventory', title: '当前库存', align: 'left', query: false, queryType: 'input', set: false, require: true },
        { field: 'securityInventory', title: '安全库存', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'fWareCode', title: '一级库位编码', align: 'left', query: false,hide:false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'fWareName', title: '一级库位名称', align: 'left', query: false,hide:false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'sWareCode', title: '二级库位编码', align: 'left', query: false,hide:false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'sWareName', title: '二级库位名称', align: 'left', query: false,hide:false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'orgCode', title: '组织编码', align: 'left', query: false,hide:false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'orgName', title: '所属组织名称', align: 'left', query: false,hide:false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'lotAttr1', title: '批属性1', align: 'left', query: false,hide:false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'lotAttr2', title: '批属性2', align: 'left', query: false,hide:false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'lotAttr3', title: '批属性3', align: 'left', query: false,hide:false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'isWearingPart', title: '是否易损件', align: 'left', query: false, queryType: 'select', set: false, require: true },
  ...
  methods: {
    ...
    save(formName) {
      ...
      const _data = { url: url, params: { data: this.defaultForm.params }}
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$store.dispatch('fmsCommon/actionTpmPost', _data)
            .then(data => {
