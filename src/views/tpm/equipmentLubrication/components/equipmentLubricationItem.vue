  ...
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'equipment_mt_item_edit'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'equipment_mt_item_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      equipmentMtIdItem: '',
      url: {
        add: '/equipmentLubricationItem/add',
        del: '/equipmentLubricationItem/del',
        list: '/equipmentLubricationItem/get-LubricationItem'
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        ...
        { field: 'paramName', title: '参数名称', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'lubricationContent', title: '润滑内容', align: 'left', query: false, queryType: 'input', set: true, require: true, columnType: 'textareainput',layout:"row",width: 240, },
        { field: 'lubricationWay', title: '润滑方式', align: 'left', query: false, queryType: 'input', editReadonly: true, set: true, require: true, columnType: 'textareainput',layout:"row",width: 240,  },
        { field: 'lubricationRequire', title: '检查要求', align: 'left', query: false, queryType: 'input', set: true, require: true,columnType: 'textareainput',layout:"row",width: 240, },
        ...
        { field: 'isMust', title: '是否必检', align: 'left', query: false, queryType: 'select', set: true, require: true,width: 80, },
        { field: 'createUser', title: '项目创建人', align: 'center', width: 80, queryType: 'datetime', set: false, require: false },
  ...
  watch: {
    equipmentMtId: function(val) {
      console.log(val)
      this.equipmentMtIdItem = val
      this.getList()
      this.selectOptions()
    }
  },
  ...
  methods: {
    ...
    getList() {
      ...
      const _data = { url: this.url.list, params: { data: this.equipmentMtIdItem }}
    ...
    destory(hide = false) {
      ...
      this.defaultForm.params.isMust = 1
    },
    add() {
    ...
    submit(formName) {
      Object.assign(this.defaultForm.params, { hedId: this.equipmentMtIdItem })
