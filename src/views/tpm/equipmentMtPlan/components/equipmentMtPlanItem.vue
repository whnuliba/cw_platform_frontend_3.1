    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column v-for="(item) in cols" v-if="!item.hide" v-show="!item.hide" :key="item.id" :label="commonI18n(item)" :prop="item.field" :width="item.width" :min-width="item.minWidth" :align="item.align" show-overflow-tooltip>
        <template slot-scope="{row}">
          <el-input v-if="item.layout==='row' && item.queryType==='input'"  v-model="row[item.field]" :placeholder="commonI18n(item)" style="width: 90%" class="filter-item">
          </el-input>
          <el-input resize="none" type="textarea" autosize v-else-if="item.layout==='row' && item.queryType==='textareainput'"  v-model="row[item.field]" :placeholder="commonI18n(item)" style="width: 95%" class="filter-item" disabled>
          </el-input>
          <span v-else>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      equipmentMtId: '',
      url: {
        add: '/equipmentMtPlanItem/add',
        del: '/equipmentMtPlanItem/del',
        list: '/equipmentMtPlanItem/guest/get-equipmentMtPlanItem',
        updateContent: '/equipmentMtPlanItem/updateContent',
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        ...
        { field: 'paramName', title: '检查内容', align: 'left', query: false, queryType: 'textareainput', set: true, require: true,layout:"row", width: 240, },
        { field: 'mtWay', title: '检查方式', align: 'left', query: false, queryType: 'textareainput', editReadonly: true, set: true, require: true,layout:"row", width: 240, },
        { field: 'mtRequire', title: '检查要求', align: 'left', query: false, queryType: 'textareainput', set: true, require: true, layout:"row", width: 240, },
        { field: 'mtContent', title: '检查结果', align: 'left', query: false, queryType: 'input', layout:"row", set: true, require: true },
  ...
  methods: {
    ...
    saveContent(row) {
      this.$confirm(('确认保存'), i18n.t('common.tip_info.tips'), {
        ...
        const _data = {
          url: this.url.updateContent, params: { data: { id: row.id, content: row.mtContent }} }
        this.$store.dispatch('fmsCommon/actionTpmPost', _data)
          .then(data => {
            let _this = this
            this.$message({
