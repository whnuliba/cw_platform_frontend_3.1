  ...
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="equipmentMtPlanRowClick" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="200">
        ...
        <template slot-scope="scope">
          <el-button type="warning" v-ele-show="{btnCode:'equipment_mt_plan_finish'}" size="small" v-if="scope.row.status != 106 && scope.row.status != 103" @click="finish(scope.row)">
            {{ "完成" }}
          </el-button>
          &nbsp;
          <el-button type="warning" v-ele-show="{btnCode:'equipment_mt_plan_close'}" v-if="scope.row.status != 106 && scope.row.status != 103" size="small" @click="close(scope.row)">
            {{ "关闭" }}
          </el-button>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      value1: new Date(),
      url: {
        add: '/equipmentMtPlan/add',
        del: '/equipmentMtPlan/del',
        list: '/equipmentMtPlan/list',
        finish: '/equipmentMtPlan/finish-EquipmentMtPlanStatus',
        close: '/equipmentMtPlan/close-EquipmentMtPlanStatus',
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        ...
        { field: 'planStartTime', title: '计划保养开始', align: 'center', width: '145px', queryType: 'datetime', set: false, require: false },
        { field: 'mtPerson', title: '保养执行人', align: 'center', width: '145px', queryType: 'input', set: false, require: false },
        { field: 'planEndTime', title: '计划保养结束', align: 'center', width: '145px', queryType: 'rangedatetime', query: true, set: false, require: false },
        ...
        { field: 'isMt', title: '是否发起维修', align: 'left',width: '60px', query: false, queryType: 'select', set: false, require: true },
  ...
  methods: {
    ...
    getList() {
      ...
      if(this.queryParams.requestData.planEndTime){
          this.queryParams.requestData['startTime'] = this.queryParams.requestData.planEndTime[0]
          this.queryParams.requestData['endTime'] = this.queryParams.requestData.planEndTime[1]
          // delete this.queryParams.requestData.planEndTime
      }
      let param = JSON.parse(JSON.stringify(this.queryParams)) 
      delete param.requestData.planEndTime
