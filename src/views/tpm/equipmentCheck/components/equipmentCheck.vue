<template>
  <div>
    <div class="filter-container">
      ...
      <el-button v-ele-show="{btnCode:'equipment_check_add'}" type="warning" class="filter-item" @click="add">
        {{ $t("common.btn.add") }}
      </el-button>
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="equipmentMtRowClick" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'equipment_check_edit'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'equipment_check_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      value1: new Date(),
      url: {
        add: '/equipmentCheck/add',
        del: '/equipmentCheck/del',
        list: '/equipmentCheck/list'
      },
      queryParams: {
        requestData: { }
      },
