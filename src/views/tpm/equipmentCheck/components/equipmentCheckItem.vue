  ...
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'equipment_check_item_edit'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'equipment_check_item_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      equipmentCheckId1: '',
      url: {
        add: '/equipmentCheckItem/add',
        del: '/equipmentCheckItem/del',
        list: '/equipmentCheckItem/guest/get-equipmentCheckItem'
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        ...
        { field: 'checkContent', title: '检查内容', align: 'left', query: false, queryType: 'input', set: true, require: true, columnType: 'textareainput',layout:"row",width: 240, },
        ...
        { field: 'createUser', title: '创建人', align: 'center', width: '145px', queryType: 'datetime', set: false, require: false,width: 80, },
  ...
  watch: {
    equipmentCheckId: function(val) {
      this.equipmentCheckId1 = val
      this.getList()
      this.selectOptions()
    }
  },
  ...
  methods: {
    ...
    getList() {
      ...
      const _data = { url: this.url.list, params: { data: this.equipmentCheckId1 }}
    ...
    submit(formName) {
      Object.assign(this.defaultForm.params, { hedId: this.equipmentCheckId1 })
