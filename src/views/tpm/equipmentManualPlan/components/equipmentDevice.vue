<template>
  <div>
    <div class="filter-container">
      <div>
        ...
        <el-select v-if="item.query && item.queryType==='select'" v-model="queryParams.requestData[item.field]" :placeholder="commonI18n(item)" style="width: 170px" class="filter-item">
          <el-option v-for="item in initSelect[item.field]"
                     :key="item.value"
                     :label="commonI18n(item)"
                     :value="item.value" />
        </el-select>
        <el-select  v-if="item.query && item.queryType==='filter-select'" :filter-method="(val) => {return dataSelectFilter(val, item)}" style="width: 170px" :filterable="true" v-model="queryParams.requestData[item.field]" :disabled="allowDisable && item.editReadonly" :placeholder="commonI18n(item)"  class="filter-item">
      ...
      <el-button type="primary" class="filter-item" @click="getRadioVal">
        {{ $t("common.btn.query") }}
      </el-button>
      ...
      <el-radio-group v-model="radioVal" @input="getRadioVal">
        <el-radio label="保养">保养</el-radio>
        <el-radio label="点检">点检</el-radio>
        <el-radio label="润滑">润滑</el-radio>
      </el-radio-group>
  ...
  <el-button v-ele-show="{btnCode:'equipment_mt_add'}" type="warning" class="filter-item" @click="add">
    ...
    <pagination small v-show="total>0" :total="total" :page.sync="listQuery.current" :limit.sync="listQuery.pageSize" @pagination="getRadioVal" />
  ...
  export default {
    ...
    data() {
      const _initdata = {
        ...
        url: {
          add: '/equipmentMt/add',
          del: '/equipmentMt/del',
          list: '/equipmentMt/guest/get-equipmentDeviceNo',
          lubList: '/equipmentLubrication/guest/get-equipmentLubricationGroupDeviceNo',
          checkList:'/equipmentCheck/guest/get-equipmentCheckGroupDeviceNo',
        },
        queryParams: {
          requestData: {}
        },
    ...
    methods: {
      ...
      getListByUrl(url) {
        this.listLoading = true
        Object.assign(this.queryParams, this.listQuery)
        const _data = { url: url, params: this.queryParams }
        this.$store.dispatch('fmsCommon/actionTpmPost', _data)
          .then(data => {
            this.list = data.data
            this.total = data.total
      ...
      getRadioVal(val) {
        if (this.radioVal == "保养") {
          this.getListByUrl(this.url.list)
        } else if (this.radioVal == "点检") {
          this.getListByUrl(this.url.checkList)
        } else if (this.radioVal == "润滑") {
          this.getListByUrl(this.url.lubList)
        }

        let source = this.radioVal
        this.$emit('getRadioVal',source)
