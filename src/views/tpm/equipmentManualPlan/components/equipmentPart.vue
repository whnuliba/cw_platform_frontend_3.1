<template>
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails" @selection-change="handleSelectionChange">
    ...
    <el-button type="warning" v-ele-show="{btnCode:'equipment_manual_plan_create'}" class="filter-item" @click="createPlan">
     生成计划
    </el-button>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      url: {
        ...
        mtlist: '/equipmentMt/guest/get-equipmentByDeviceNo',
        checkList: '/equipmentCheck/guest/get-equipmentCheck',
        lubList:'/equipmentLubrication/guest/get-equipmentLubricationByDeviceNo',
        mtCreatePlan: '/equipmentMtPlan/manual-createEquipmentMtPlan',
        lubCreatePlan: '/equipmentLubricationPlan/guest/get-createLubricationPlan',
        checkCreatePlan:'/equipmentCheckPlan/guest/get-AutoCheckPlanCreate'
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        //{ field: 'deviceNo', title: '设备编号', align: 'left', query: false, queryType: 'input', editReadonly: true, set: true, require: true },
        ...
        { field: 'partName', title: '部件名称', align: 'left', query: false, queryType: 'input', set: false, require: true },
        { field: 'cycle', title: '周期', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'cycleUnit', title: '周期单位', align: 'left', query: false, queryType: 'select', set: true, require: true },
        { field: 'mtNo', title: '单号', align: 'left', query: false, queryType: 'input', set: false, require: true },
  ...
  watch: {
    ...
    equipmentSource: function (val) {
      this.equipmentSource = val
    }
  },
  created() {
  ...
  methods: {
    ...
    getList() {
      ...
      let url
      if (this.equipmentSource == "保养") {
        url = this.url.mtlist
      } else if (this.equipmentSource == "点检") {
        url = this.url.checkList
      } else if (this.equipmentSource == "润滑") {
        url = this.url.lubList
      }

      const _data = { url: url, params: { data: this.equipmentMtId }}
    ...
    createPlan() {
      ...
      let url
      if (this.equipmentSource == "保养") {
        url = this.url.mtCreatePlan
      } else if (this.equipmentSource == "点检") {
        url = this.url.checkCreatePlan
      } else if (this.equipmentSource == "润滑") {
        url = this.url.lubCreatePlan
      }


      this.$confirm(('确认手动生成计划'), i18n.t('common.tip_info.tips'), {
        ...
        const _data = { url: url, params: { data: this.multipleSelection } }
        this.$store.dispatch('fmsCommon/actionTpmPost', _data)
          .then(data => {
            this.$message({
              type: 'success',
