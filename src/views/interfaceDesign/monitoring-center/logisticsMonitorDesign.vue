...
<template>
  <el-row>
    <div :style="{ display: 'flex' }">
      ...
      <el-col :span="20">
        <div class="design-and-inspect" ref="monitorSpan">
          ...
          <div class="handle-resize" ref="dragText">
            <!-- :style="{ textAlign: 'center', color: 'red', cursor: 'move' }" -->
          </div>
        </div>
        <div class="resize-div" ref="resize-div" :style="upArea">
...
export default {
  ...
  data() {
    return {
      designArea: { height: "600px", display: "flex" },
      upArea: {},
  ...
  mounted() {
    this.resizeDrag(this.$refs.dragText);
    this.init();
  ...
  computed: {},
  methods: {
    resizeDrag(resizeRef) {
      // 鼠标拖动事件
      resizeRef.onmousedown = (e) => {
        document.onmousemove = (ev) => {
          this.designArea.height = `${ev.clientY}px`;
          this.upArea.height = `${1050 - ev.clientY}px`;
        };
        // 鼠标松开事件
    ...
    init() {
      ...
      myDiagram.grid = Make(
        ...
        Make(go.Shape, "BarH", { fill: "#00a1ff2b", strokeWidth: 0.5 }), //#d0d9dd
        Make(go.Shape, "BarV", { fill: "#00a1ff2b", strokeWidth: 0.5 }),
        Make(go.Shape, "LineH", { stroke: "#ccc", strokeWidth: 0.5 }),
        Make(go.Shape, "LineV", { stroke: "#ccc", strokeWidth: 0.5 })
      );
      ...
      function showSmallPorts(node, show) {
        node.ports.each(function (port) {
          if (port.portId !== "" && node.data.group && node.data.eqType === "报警设备") {
        ...
        Make(
          ...
          Make(
            go.Shape,
            {
              ...
              fill: "rgba(128, 128, 128, 0.7)", // 每个节点的背景颜色
              stroke: "gray",
              strokeWidth: 0.1,
      ...
      myDiagram.groupTemplate = Make(
        ...
        Make(
          ...
          new go.Binding("stroke", "st"),
          new go.Binding("fill", "groupShapeFill")
        ),
        Make(
          go.Panel,
          "Spot",
      ...
      myDiagram.linkTemplate = Make(
        ...
        Make(go.Shape, { isPanelMain: true, stroke: "black", strokeWidth: 7 }),
        Make(go.Shape, { isPanelMain: true, stroke: "gray", strokeWidth: 5 }),
        Make(go.Shape, {
          isPanelMain: true,
          stroke: "white",
          strokeWidth: 3,
        ...
        Make(go.Shape, { toArrow: "Triangle", scale: 1.3, fill: "gray", stroke: null })
      );
      ...
      new Inspector("myInspector", myDiagram, {
        ...
        properties: {
          ...
          textColor: {
            show: (Inspector.showIfNode = function (t) {
              return t.data.eqType && t.data.eqType !== "库位"
          ...
          textAngle: {
            ...
            choices: function (node, propName) {
              ...
              return [
                ...
                360,
                0,
                // new go.Spot(0.5, 1.5, 0, 0),
                // new go.Spot(0.5, 1.5, 0, 0),
                // new go.Spot(1.5, 0.5, 0, 0),
                // new go.Spot(-0.5, 0.5, 0, 0),
              ];
            },
          },
          resizable: {
...
.inspector-logis input:disabled {
  background-color: #bdbdbd;
  color: #1b1b1b;
}

.inspector-logis select {
  background-color: #fff;
}
</style>
