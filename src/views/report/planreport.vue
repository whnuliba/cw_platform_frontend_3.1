...
import { GetStorageDetailList, GetStorageReportYear } from "@/api/storage";
import { GetPlanReport } from "@/api/plan";
...
export default {
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        PLAN_LIST_QUANTITY: undefined,
        PLAN_LIST_FINISH_QUANTITY: undefined,
        GOODS_PROPERTY1: undefined,
      },
      data: [],
      dttime: [],
      listLambdaQuery: [],
  ...
  methods: {
    ...
    getSysTable() {
      this.$set(this.tablename, "tableCode", "REPORT_RECODE");
        ...
        if (this.value2 != "" && this.value2 != undefined && this.value2 != null) {
          if (this.value3 != "" && this.value3 != undefined && this.value3 != null) {
            if (this.value4 != "" && this.value4 != undefined && this.value4 != null) {
              ...
              lambdaSearch.LAMBDA_KEY = "CREATE_TIME";
              lambdaSearch.LAMBDA_VALUE = undefined;
              lambdaSearch.LAMBDA_START =
                this.dateTimeArray[0] + "-" + "01 " + "00:00:00";
            ...
            let reportlist = [];
            let j = 0;
            GetPlanReport(query).then((response) => {
              for (let i = 0; i < 12; i++) {
                if (response.data[i].REPORTLIST.length != 0) {
                  response.data[i].REPORTLIST.forEach((item, index, array) => {
                    reportlist[j] = item;
                    j++;
                  });
                } else {
                  reportlist[j] = {
                    MONTH_TIME: i + 1 + "月",
                    GOODS_PROPERTY1: 0,
                    PLAN_LIST_FINISH_QUANTITY: 0,
                    PLAN_LIST_QUANTITY: 0,
                  };
                  j++;
                }
              }
              for (let i in reportlist) {
                this.$set(reportlist[i], "currentIndex", parseInt(i) + 1);
              }
              this.tableIndex = Math.ceil(
                reportlist.length / this.reportTableOptions.REPORT_DIRECTION
              );
              if (reportlist.length == this.reportTableOptions.REPORT_DIRECTION) {
                this.tableIndex += 1;
              }
              let findex = parseInt(this.reportTableOptions.REPORT_DIRECTION);
              let dindex = 0;
              let lindex = 0;
              let arraylist = [];
              reportlist.forEach((item, index, array) => {
                if (lindex == 0 && index == findex - 1) {
                  findex += findex - 1;
                  dindex = 0;
                  this.reportlistdata[lindex + 1] = arraylist;
