...
export default {
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        FLAG: undefined,
        TRAY_CODE: undefined,
        BATTERY_NO: undefined,
        BATTERY_CODE: undefined,
        STATUS: undefined,
  ...
  methods: {
    ...
    getSysTable() {
      this.$set(this.tablename, "tableCode", "REPORT_MONTH_DETAIL");
    ...
    async GetDetailPage() {
      ...
      await GetStorageDetailList(this.listLambdaQuery).then((response) => {
        ...
        //加载数据
        //this.storagedetaillistData = storagedetaillist;
        ...
        storagedetaillist.forEach((item, index, array) => {
          if (lindex == 0 && index == findex - 1) {
            findex += findex - 1;
            dindex = 0;
            this.storagedetaillistData[lindex + 1] = arraylist;
            arraylist = undefined;
            arraylist = new Array();
            lindex++;
          }
    ...
    handleFilter() {
      ...
      if (this.value2 != "" && this.value2 != undefined && this.value2 != null) {
        if (this.value3 != "" && this.value3 != undefined && this.value3 != null) {
          ...
          this.listLoading = true;
          this.storagedetaillistData = new Array();
          let query = {
            year: this.value2.getFullYear(),
          };
          GetStorageReportYear(query).then((response) => {
            if (this.direction == "landscape") {
              ...
              this.tableIndex = 1
              this.storagedetaillistData[1] = response.data;
            }
          });
          setTimeout(() => {
            this.listLoading = false;
    ...
    // printTable() {
    //     this.list = this.storagedetaillistData
    //     let j = 1
    //     for (let v of this.list) {

    //       this.data.push({
    //         field1: j,
    //         field2: v.PLAN_LIST_ID,
    //         field3: v.TRAY_CODE,
    //         field4: v.BATTERY_NO,
    //         field5: v.BATTERY_CODE,
    //         field6: v.STATUS,
    //         field7: v.FLAG,
    //         field8: v.MEMO,
    //         field9: v.PROCESS_TYPE,
    //         field10: v.CREATE_TIME,
    //       });
    //       j++
    //     }
    //     printJS({
    ...
    //       headerStyle: "text-align:center;", //color:#f00;width:100%;border:1px #000 solid;
    //       style:
    //        "table tr td,th {text-align:center; border-collapse: collapse;padding: 15px;border:1px #000 solid; }", // 表格样式
    //     });
    //   }
  },
};
