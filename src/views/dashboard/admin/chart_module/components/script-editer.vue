<template>
  <div class="app-container">
          <div class="in-coder-panel" >
            <codemirror ref="mycodemirror" v-model="code" :options="options" ></codemirror>
...
export default {
 ...
 props: {
    ...
    mounted () {
      // this.editor = this.$refs.mycodemirror.codemirror;
      // 设置codemirror高度为600
      // this.editor.setSize('auto', 800);
      // 初始化
      //this._initialize()
      this.initRact()
    ...
    initScript(val){
      if(val){
        ...
        //   .then(data => {
            ...
            // })
            // let inrnum = (str.join(',\n').indexOf(',',str.join(',\n').indexOf('width')))+1
            // let inputstr = str.join(',\n').slice(0, inrnum) + '//(0-24)' + str.join(',\n').slice(inrnum)
            // console.log(params.trim());
            
            this.code = params.replace(/(^\s*)|(\s*$)/g, "");
