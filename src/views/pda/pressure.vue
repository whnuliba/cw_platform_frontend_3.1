    ...
    <el-tabs>
      ...
      <el-tab-pane label="完成生产">
          <el-form ref="form1" :model="end" label-width="35%">
            <el-form-item label="设备编码:">
              <el-input v-model="end.deviceNo" placeholder="设备编码" style="width: 160px"   class="filter-item"></el-input>
            </el-form-item>
            <el-form-item>
...
export default {
 ...
 directives: { waves, elDragDialog },
  ...
  methods: {
    ...
    endsubmit(){
      const _data = {url:'/pressure/pdaCompleteProduct',params:this.end}
    ...
    submit(){
      const _data = {url:'/pressure/startProduct',params:this.barcode}
      this.$store.dispatch('fmsCommon/actionProductPost',_data)
        .then(data => {
          this.$message({
            type: 'success',
    ...
    submintsubsection(){
      this.$confirm("确认分卷吗?", "提示", {
        ...
        type: "warning",
         ...
         const _data = {url:'/pressure/subsection',params:{data:this.subsection}}
        this.$store.dispatch('fmsCommon/actionProductPost',_data)
          .then(data => {
            this.$message({
              type: 'success',
