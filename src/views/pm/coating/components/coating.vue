<template>
  <div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="345">
    ...
    <el-dialog v-el-drag-dialog title="编辑" :visible.sync="dialogFormVisible" width="700px" top="1%">
      <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="140px">
    ...
    <el-dialog v-el-drag-dialog title="打标" :visible.sync="dialogMarkingVisible" width="500px">
        <div>
          <el-button type="primary" size="small"  class="filter-item" @click="print" v-print="printObj">{{ '打印' }}</el-button>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      url: {
        ...
        del: '/coating/del',
        list: '/coating/list',
        submit: '/coating/completeProduct',
        printBarCode: '/coating/printBarCode',
      },
      queryParams: {
        requestData: { }
      },
      initSelect: {
        ...
        transitionStatus: [{ label: '待碾压', value: 0 }, { label: '已下转', value: 1 }, { label: '静置超时', value: 2 }],
      },
      defaultForm: {
        row: {},
        params: {
      ...
      cols: [
        ...
        { field: 'coatNo', title: '涂布工单号', width:'150', align: 'center', query: false, queryType: 'input', set: false, require: false },
        // { field: 'polarity', title: '极性', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'homoId', title: '匀浆ID', align: 'left', hide: true, query: false, queryType: 'input', set: true, require: false },
      ],
      materialcols:[
  ...
  methods: {
    ...
    viewmaterial(row){
      ...
      const _data = { url: '/materialFeedingItemTb/selectBySrcId', params: {data:row.id} }
    ...
    marking1(row){
      let printdata = ''
      const _data1 = {url:'/page-conf/guest/biz-type-code',params:{data:{bizCode:'print_coat',bizType:'打印条码'}}}
      ...
      .then(data => {
       ...
       const _data = { url: this.url.printBarCode, params: {id:row.id}}
      this.$store.dispatch('fmsCommon/actionProductPost', _data)
        .then(data => {
