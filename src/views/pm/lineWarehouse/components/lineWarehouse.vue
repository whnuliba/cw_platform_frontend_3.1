  ...
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="rowClick" @row-dblclick="rowdoubleClick">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'line_warehouse_add'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'line_warehouse_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    const _initdata = {
      table_mark: 'pp_warehouse_list',
      dialogFormVisible: false,
      allowDisable: false,
      dialogSelectFieldVisible: false,
      value: new Date(),
      url: {
        add: '/lineWarehouse/add',
        del: '/lineWarehouse/del',
        list: '/lineWarehouse/list'
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        { field: 'wno', title: '编码', align: 'left', query: true, queryType: 'input', set: true, require: false },
        { field: 'wloc', title: '位置信息', align: 'left', query: true, queryType: 'input', set: true, require: false },
        { field: 'wtype', title: '仓库类型', align: 'left', query: true, queryType: 'select', set: true, require: false },
