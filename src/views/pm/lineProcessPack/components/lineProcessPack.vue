...
<!--      <el-button type="warning"   class="filter-item" @click="add" v-ele-show="{btnCode:'line_warehouse_add'}">
        {{ $t("common.btn.add") }}
      </el-button>-->
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="rowClick" @row-dblclick="rowdoubleClick" :height='225'>
      ...
      <el-table-column align="center" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          ...
          <span class="el-dropdown-link">
            ...
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item v-if="scope.row.actState == 1" command="del">删除</el-dropdown-item>
              <el-dropdown-item v-if="scope.row.actState == 2" command="placefile">设备归档</el-dropdown-item>
              <el-dropdown-item v-if="scope.row.actState == 2" command="placeonfile">领料删除</el-dropdown-item>
        ...
        <!-- <template slot-scope="scope">
          <el-button v-if="scope.row.actState == 1"   type="primary"  size="small"  class="filter-item" @click="del(scope.$index,scope.row)">{{ '删除' }}</el-button>
          <el-button v-if="scope.row.actState == 2"   type="primary"  size="small"  class="filter-item" @click="placefile(scope.$index,scope.row)">{{ '归档' }}</el-button>
          <el-button v-if="scope.row.actState == 2"   type="primary"  size="small"  class="filter-item" @click="placeonfile(scope.row)">{{ '领料归档' }}</el-button>
        </template> -->
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    const _initdata = {
      table_mark: 'pp_lineProcessPack_list',
      dialogFormVisible: false,
      allowDisable: false,
      dialogSelectFieldVisible: false,
      value: new Date(),
      url: {
        add: '/lineProcessPack/add',
        del: '/lineProcessPack/del',
        list: '/lineProcessPack/list',
        barcodeComplete: '/lineProcessPack/barcodeComplete',
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        { field: 'packNo', title: '领料单', align: 'left', query: true, queryType: 'input', set: true, require: false },
        ...
        { field: 'deviceNo', title: '目标机台', align: 'left', query: true, queryType: 'input', set: true, require: false },
        // { field: 'processCode', title: '工序', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'actState', title: '激活状态', align: 'left', query: true, queryType: 'select', set: true, require: false },
        // { field: 'priority', title: '优先级', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'materialNo', title: '物料编码', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'materialName', title: '物料名称', align: 'left', query: false, queryType: 'input', set: true, require: false }
      ]
    }
  ...
  methods: {
    ...
    placeonfile(row) {
      this.$confirm('确定要归档吗?', '提示', {
        ...
        const _data = { url: '/lineProcessPack/barcodeCompleteByPackNo', params: {data:{packNo:row.packNo,}} }
    ...
    selectOptions() {
      // 初始化线体信息
      // 初始化工序信息
      const _data = { url: '/lineProcessPack/LineProcessPackStates', params: '' }
      this.$store.dispatch('fmsCommon/actionProductPost', _data)
        .then(data => {
    ...
    del(row) {
      this.$confirm('确定要删除吗?', '提示', {
        ...
        const _data = { url: '/lineProcessPack/rollbackPackInventoryPackNo', params: { data: {packNo:row.packNo} }}
        this.$store.dispatch('fmsCommon/actionProductPost', _data)
          .then(data => {
    ...
    placefile(row) {
      this.$confirm('确定要归档吗?', '提示', {
        ...
        const _data = { url: this.url.barcodeComplete, params: { data: {id:row.id,deviceNo:row.deviceNo} }}
        this.$store.dispatch('fmsCommon/actionProductPost', _data)
          .then(data => {
            // this.list.splice(index, 1)
            this.$message({
