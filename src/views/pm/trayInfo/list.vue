<template>
    <div class="app-container">
      ...
      <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="rowClick" :height='550'>
        ...
        <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="160">
          ...
          <template slot-scope="scope">
            <el-button  type="primary"  size="small"  class="filter-item" @click="edit(scope.row)">{{ '修改' }}</el-button>
  ...
  export default {
    ...
    data() {
      const _initdata = {
        ...
        url: {
          ...
          del: '/trayInfo/del',
          list: '/trayInfo/list',
          submit: '/trayInfo/completeProduct',
        },
        queryParams: {
          requestData: { }
        },
        ...
        cols: [
          { field: 'trayNo', title: '托盘编码', align: 'left', query: true, queryType: 'input', set: true, require: true },
          { field: 'trayType', title: '托盘类型', align: 'left', query: true, queryType: 'select', set: true, require: false },
          {field: 'useState', title: '使用状态', align: 'center',query:true,queryType:'select',set:true,filter:true,require:true },
          {field: 'cellNumber', title: '单元格数', align: 'center',query:false,queryType:'input',set:true,filter:true,require:false },
          { field: 'length', title: '长', minWidth: 100, align: "center" ,queryType:'input',set:true,require:false },
          { field: 'width', title: '宽', minWidth: 100, align: "center",queryType:'input',set:true,require:false  },
    ...
    methods: {
      ...
      selectOptions() {
        let _data = {url:'/trayInfo/trayType',params:{}}
       this.$store.dispatch('fmsCommon/actionProductPost',_data)
      .then(data => {
          console.log(data);
