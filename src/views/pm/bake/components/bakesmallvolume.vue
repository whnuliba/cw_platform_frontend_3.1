<template>
  <div>
    <!-- <el-button   style="margin-bottom:10px;"  type="primary"  size="small"  class="filter-item" @click="addTableData">{{ '新增' }}</el-button> -->
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column v-for="(item) in cols" v-if="!item.hide" v-show="!item.hide" :key="item.id" :label="commonI18n(item)" :prop="item.field" :width="item.width" :min-width="item.minWidth" :align="item.align">
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      cols: [
        { field: 'orderNo', title: '工单', align: 'left', query: true, queryType: 'input', set: false, require: false },
        ...
        { field: 'upNums', title: '前段裁片数', align: 'left', query: false, queryType: 'input', set: true, require: false },
  ...
  methods: {
    saveRowData(scope){
      ...
      params.upNums = params.upNums ? Number(params.upNums) : null
      params.downNums = params.downNums ? Number(params.downNums) : null
      params['cuttingId'] = this.queryParams.requestData.cuttingId
      if(scope.row.id){
    ...
    addTableData(){
      if(this.queryParams.requestData.cuttingId){
    ...
    reloadData(val) {
      // this.queryParams.requestData.cuttingId = val
      this.queryParams['cuttingId'] = val
      this.getList()
      this.selectOptions()
    },
