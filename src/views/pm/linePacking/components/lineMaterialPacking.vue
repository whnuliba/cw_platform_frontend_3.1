  ...
  <div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="rowClick" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'lineMaterial_add1'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'lineMaterial_del1'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    const _initdata = {
      table_mark: 'pm_lineMaterialPacking_list',
      dialogFormVisible: false,
      allowDisable: false,
      dialogSelectFieldVisible: false,
      wid: '',
      url: {
        add: '/lineMaterialPacking/add',
        del: '/lineMaterialPacking/del',
        list: '/lineMaterialPacking/list'
      },
      queryParams: {
        requestData: {
        }
      ...
      cols: [
        ...
        { field: 'isPack', title: '是否最小包装', align: 'left', query: false, queryType: 'select', set: true, require: false },
        { field: 'packId', title: '领料单ID', hide: true, align: 'left', query: false, queryType: 'input', set: false, require: false }
      ]
    }
