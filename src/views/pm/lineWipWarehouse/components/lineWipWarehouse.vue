<template>
  <div>
    <div class="filter-container">
      ...
      <el-button type="warning"   class="filter-item" @click="add" v-ele-show="{btnCode:'lineWipWarehouse_add'}">
        {{ $t("common.btn.add") }}
      </el-button>
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="rowClick" @row-dblclick="rowdoubleClick">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'lineWipWarehouse_add'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'lineWipWarehouse_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  props: {
    // 外部传入的内容，用于实现双向绑定
    // value: String,
    lineWipWarehouseId: String
  },
  data() {
    const _initdata = {
      table_mark: 'pm_lineWipWarehouse_list',
      dialogFormVisible: false,
      allowDisable: false,
      dialogSelectFieldVisible: false,
      value: new Date(),
      url: {
        add: '/lineWipWarehouse/add',
        del: '/lineWipWarehouse/del',
        list: '/lineWipWarehouse/list'
      },
      queryParams: {
        requestData: { }
      },
      initSelect: {
        trayType: [],
        wtype: [{ label: '物料', value: '001' }, { label: '在制品', value: '002' }]
      },
      defaultForm: {
        row: {},
        params: {
      ...
      cols: [
        { field: 'wno', title: '仓库编码', align: 'left', editReadonly: true, query: true, queryType: 'input', set: true, require: false },
        { field: 'wloc', title: '位置信息', align: 'left', query: true, queryType: 'input', set: true, require: false },
