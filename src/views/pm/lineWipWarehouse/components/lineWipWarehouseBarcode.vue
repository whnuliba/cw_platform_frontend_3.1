...
export default {
  ...
  props: {
    // 外部传入的内容，用于实现双向绑定
    // value: String,
    lineWipWarehouseBarcodeId: String
  },
  data() {
    const _initdata = {
      table_mark: 'pm_lineWipWarehouseBarcode_list',
      dialogFormVisible: false,
      allowDisable: false,
      dialogSelectFieldVisible: false,
      sn: '',
      url: {
        add: '/lineWipWarehouseBarcode/add',
        del: '/lineWipWarehouseBarcode/del',
        list: '/lineWipWarehouseBarcode/list'
      },
      queryParams: {
        requestData: { }
      },
      initSelect: {
        trayType: [],
        useState: [{ label: '未领用', value: 0 }, { label: '已领用', value: 1 }],
      },
      defaultForm: {
        row: {},
        params: {
      ...
      cols: [
        { field: 'materialNo', title: '物料编号', align: 'left', query: false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'barcode', title: '物料条码', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'inTime', title: '入库时间', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'useState', title: '领用状态', align: 'left', query: false, queryType: 'select', set: true, require: false },
        { field: 'outTime', title: '出库时间', align: 'left', query: false, queryType: 'input', set: true, require: false },
        ...
        { field: 'processCode', title: '入库工序', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'wid', title: 'wid', hide: true, align: 'left', query: false, queryType: 'input', set: true, require: false }
      ]
    }
  ...
  watch: {
    lineWipWarehouseBarcodeId: function(val) {
      this.queryParams.requestData.wid = val
      this.getList()
      this.selectOptions()
    }
  },
