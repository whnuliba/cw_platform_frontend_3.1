...
export default {
  name: 'LineMaterialBarcodePacking',
  ...
  props: {
    // 外部传入的内容，用于实现双向绑定
    // value: String,
    lineMaterialBarcodePackingId: String,
    activeName1:String,
  },
  data() {
    const _initdata = {
      table_mark: 'pm_windingSvolume_list',
      dialogFormVisible: false,
      allowDisable: false,
      dialogSelectFieldVisible: false,
      sn: '',
      url: {
        add: '/windingSvolume/add',
        del: '/windingSvolume/del',
        list: '/windingSvolume/list'
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        { field: 'barCode', title: '小卷条码', align: 'left', query: true, queryType: 'input', set: false, require: false, width:120 },
        { field: 'orderNo', title: '小卷工单', align: 'left', query: true, queryType: 'input', set: false, require: false, width:120 },
        ...
        { field: 'startTime', title: '上料时间', width:'150', align: 'center', query: false, queryType: 'input', set: false, require: false },
        { field: 'endTime', title: '完成时间', width:'150', align: 'center', query: false, queryType: 'input', set: false, require: false },
        { field: 'productStates', title: '生产状态', align: 'left', query: false, queryType: 'input', set: false, require: false },
      ]
    }
  ...
  watch: {
    lineMaterialBarcodePackingId: function(val) {
      this.queryParams.requestData.srcId = val
      this.getList()
      this.selectOptions()
    },
    activeName1(val){
  ...
  methods: {
    ...
    selectOptions() {
      ...
      this.$store.dispatch('fmsCommon/actionProductPost',_data)
      .then(data => {
        this.initSelect['productStates'] = data.map(c=>{
               return {
                 label:c.name,
                 value:c.value
               }
