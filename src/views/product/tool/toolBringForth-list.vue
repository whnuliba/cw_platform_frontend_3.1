  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'ToolBringForth_add'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'ToolBringForth_del1'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  props: {
    // 外部传入的内容，用于实现双向绑定
    value: String,
    toolBringForthId: String
  },
  data() {
    const _initdata = {
      table_mark: 'pp_tbf_list',
      dialogFormVisible: false,
      allowDisable: false,
      dialogSelectFieldVisible: false,
      url: {
        add: '/productToolBringForth/add',
        del: '/productToolBringForth/del',
        list: '/productToolBringForth/list'
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        { field: 'toolNo', title: '工装编码', align: 'left', query: true, queryType: 'input', set: true, require: true },
        { field: 'toolName', title: '工装名称', align: 'left', query: true, queryType: 'input', set: true, require: true },
