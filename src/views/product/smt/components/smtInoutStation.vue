<template>
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="inoutStationRowClick" @row-dblclick="showDetails">
...
<!--        <template slot-scope="scope">
          <i class="el-icon-s-operation"  title="查看配料信息"  @click="inoutStationClick(scope.index,scope.row)"></i>
        </template>-->
      </el-table-column>
    </el-table>
...
export default {
  ...
  props: {
    // 外部传入的内容，用于实现双向绑定
    value: String,
    inoutStationId: String
  },
  data() {
    const _initdata = {
      table_mark: 'pp_inoutStation_lst',
      dialogFormVisible: false,
      allowDisable: false,
      dialogSelectFieldVisible: false,
      sn: '',
      url: {
        add: '/productSmtInoutStation/add',
        del: '/productSmtInoutStation/del',
        list: '/productSmtInoutStation/guest/selectBySn'
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        { field: 'sn', title: '条码', align: 'left', width: '122px', query: false, queryType: 'input', editReadonly: true, set: true, require: false },
        { field: 'inStationTime', title: '扫码入站', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'processNo', title: '工序', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'stationNo', title: '工位', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'person', title: '人员', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'deviceNo', title: '设备编码', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'outStationTime', title: '出站时间', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'snState', title: '条码状态', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'bcrNo', title: '条码枪编码', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'ioFlag', title: '出入标识', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'buckleMatState', title: '扣料状态', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'ipqcResult', title: 'IPQC结果', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'ipqcDesc', title: 'IPQC描述', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'ipqcStandard', title: 'IPQC标准', align: 'left', query: false, queryType: 'input', set: true, require: false },
  ...
  methods: {
    inoutStationRowClick(row, column, event) {
      this.$emit('inoutStationRowClick', row)
    },
    inoutStationClick(index, row) {
      this.$emit('inoutStationClick', row)
    },
    ...
    getList() {
      ...
      const _data = { url: this.url.list, params: { data: this.sn }}
