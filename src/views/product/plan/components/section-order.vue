<template>
  <div >
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="selectionOrderRowClick"  >
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="120">
        <template slot-scope="scope">     
          <i class="el-icon-paperclip" title="发起物料配送申请" v-if="scope.row.isPrepMat==100"  @click="pullMaterial(scope.row)"></i>
          &nbsp;
          <i class="el-icon-delete"  v-if="scope.row.status==106"  @click="del(scope.$index,scope.row)"></i>
            &nbsp;
          <i class="el-icon-s-operation"  title="查看工位备料信息"  @click="stationClickMat(scope.$index,scope.row)"></i>
              &nbsp;
          <i class="el-icon-s-operation" v-if="scope.row.status==107" title="完工检查及申请"  @click="orderComplete(scope.$index,scope.row)"></i>
                &nbsp;
          <i class="el-icon-s-operation" v-if="scope.row.status!=107 && scope.row.status!=106" title="发起退料申请"  @click="returnMaterial(scope.$index,scope.row)"></i>
        </template>
      </el-table-column>
    </el-table>
 ...
 directives: { waves, elDragDialog },
  ...
  data() {
    const  _initdata ={
      ...
      url:{
          add:"/section-order/add",
          del:"/section-order/del",
          list:"/section-order/guest/items"
      },
      defaultForm: {
        row:{},
        params:{
      ...
      cols: [       
            { field: 'orderNo', title: '内部单号',en_title:"PN",  align: "center",width:120,query:true,queryType:'input',editReadonly:false,set:false,require:true }, 
            ...
            { field: 'qty', title: '工单数量', align: "left",query:false,queryType:'input',set:true,require:true   },
            { field: 'scNo', title: '产线编码', align: "left",query:false,queryType:'input',set:true,require:true   },
            { field: 'scName', title: '产线名称', align: "left",query:false,queryType:'input',set:true,require:true   },
            { field: 'wsNo', title: '工厂编码', align: "left",query:false,queryType:'input',set:true,require:true   },
            { field: 'isHasMat', title: '物料状态', align: "left",query:false,queryType:'select',set:true,require:true   },
            { field: 'isPrepMat', title: '备料状态', align: "left",query:false,queryType:'select',set:true,require:true   },
            { field: 'startTime', title: '开始时间', align: "left",query:false,queryType:'input',set:true,require:true   },
            { field: 'endTime', title: '结束时间', align: "left",query:false,queryType:'input',set:true,require:true   },
            { field: 'status', title: '状态', align: "left",query:false,queryType:'select',set:true,require:true   },
  ...
  methods: {
       selectionOrderRowClick(row, column, event){
             this.$emit('changeSelectionOrder',  row)
       },
        reloadData(val){
            this.extOrderNo = val
            this.getList()
        },
        stationClickMat(index,row){
              this.$emit('changeSelectionOrderMat',  row)
        },
        orderComplete(index,row){
          this.$confirm("你确定要完成当前工单吗，若物料为扣减完成结单也会失败的", i18n.t("common.tip_info.tips"), {
          confirmButtonText: i18n.t("common.btn.ok"),
          cancelButtonText: i18n.t("common.btn.cancel"),
          type: 'warning'
        }).then(() => {        
                const _data = {url:'/section-order/order-complete',params:{data:row.id}}
          ...
          returnMaterial(index,row){
          this.$confirm("你确定要发起退料申请吗，只有非运行的工单才能发起退料申请", i18n.t("common.tip_info.tips"), {
          confirmButtonText: i18n.t("common.btn.ok"),
          cancelButtonText: i18n.t("common.btn.cancel"),
          type: 'warning'
        }).then(() => {        
                const _data = {url:'/section-order/return-material',params:{data:row.id}}
                this.$store.dispatch('fmsCommon/actionMdmPost',_data)
                .then(data => {
                      this.$message({
                      type: 'success',
        ...
        pullMaterial(row){
          this.$confirm("确定要发起物料配送申请吗？", i18n.t("common.tip_info.tips"), {
          confirmButtonText: i18n.t("common.btn.ok"),
          cancelButtonText: i18n.t("common.btn.cancel"),
          type: 'warning'
        }).then(() => {        
                const _data = {url:'/section-order/pull-material',params:{data:row.orderNo}}
                this.$store.dispatch('fmsCommon/actionMdmPost',_data)
                .then(data => {
                      this.$message({
                      type: 'success',
    ...
    getList() {
      ...
      const _data = {url:this.url.list,params:{data:this.extOrderNo}}
      this.$store.dispatch('fmsCommon/actionMdmPost',_data)
      .then(data => {
          this.list = data
  ...
  query(){
   ...
   selectOptions(){
       //初始化线体信息
     let _data = {url:'/constant/work-mat-states',params:{}}
       this.$store.dispatch('fmsCommon/actionMdmPost',_data)
      .then(data => {
           this.initSelect.isPrepMat = data.map(c=>{
             return {
               label:c.name,
               value:c.value
             }
         ...
         this.initSelect.isHasMat = data.map(c=>{
             return {
               label:c.name,
               value:c.value
             }
