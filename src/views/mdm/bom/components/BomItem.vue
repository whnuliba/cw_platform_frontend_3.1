<template>
  ...
  <div class="filter-container">
     ...
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="bomItemRowClick"  @row-dblclick="showDetails" :height='290'>
 ...
 directives: { waves, elDragDialog },
  ...
  data() {
    const  _initdata ={
      ...
      cols: [
                ...
                { field: 'bomNo', title: 'BOM编码',hide:true, align: "left",query:false,queryType:'input',set:true,require:true   },
                { field: 'bomName', title: 'BOM名称',hide:true, align: "left",query:false,queryType:'input',set:true,require:true   },
                { field: 'version', title: 'BOM版本',hide:true, align: "left",query:false,queryType:'input',set:true,require:true   },
                
                { field: 'orderNo', title: '工单编码',query:true,align: "left",queryType:'input',set:false,require:true  },
                { field: 'bomNo', title: 'BOM编码',query:true,align: "left",queryType:'input',set:false,require:true  },
                
                { field: 'ppn', title: '母件编码',hide:false,en_title:"PN",  align: "left",query:true,queryType:'input',editReadonly:true,set:true,require:true },               
                { field: 'ppnName', title: '母件名称',hide:false, align: "left",query:true,queryType:'input',set:true,require:true   },

                
                { field: 'pgn', title: '母件图号', align: "left",query:false,queryType:'input',set:true,require:true,hide:true,   },
                { field: 'rowNo', title: '行号',query:true,align: "left",queryType:'input',set:false,require:true  },
                { field: 'pn', title: '子件编码',en_title:"PN",  align: "left",query:true,queryType:'input',editReadonly:true,set:true,require:true },               
                { field: 'pnName', title: '子件名称', align: "left",query:true,queryType:'input',set:true,require:true   },
                { field: 'gn', title: '子件图号', align: "left",query:false,queryType:'input',set:true,require:true,hide:true,   },
                { field: 'qty', title: '数量',hide:false, align: "left",query:false,queryType:'input',set:true,require:true   },               
                { field: 'uom', title: '规格',hide:false, align: "left",query:false,queryType:'input',set:false,require:true   },
                { field: 'unit', title: '单位',hide:false, align: 'center' ,query:false,queryType:'input' },
                { field: 'status', title: '状态', align: "left",query:true,queryType:'select',set:true,require:true   },
                { field: 'createTime', title: '创建时间', align: "left",query:true,queryType:'date',set:true,require:true,width:150,},
  ...
  methods: {
    ...
    getList() {
      ...
      //  判断条件
      //  if(this.isNull(this.queryParams.requestData.ppn))
      //  {
      //   this.$message({
      //      type: 'warning',
      //     message: '请输入BOM信息(母件编码)不能同时为空'
      //   })
      //   return 
      //  }
      ...
      .then(data => {
          ...
          if(this.list.length>0){
            let pp = this.list[0]
            //  this.$set(this.queryParams.requestData,'ppn',pp.ppn)
            //  this.$set(this.queryParams.requestData,'ppnName',pp.ppnName)
            //  this.$set(this.queryParams.requestData,'pgn',pp.pgn)
            //  this.$set(this.queryParams.requestData,'bomNo',pp.bomNo)
            //  this.$set(this.queryParams.requestData,'bomName',pp.bomName)
             _id = pp.id
          }
          this.bomItemRowClick({id:_id},undefined,undefined)
       }).catch(err => {
          console.error(err)
       })
