  ...
  export default {
    ...
    data() {
      const _initdata = {
        ...
        cols: [
          { field: 'stationNo', title: '工位', minWidth: 100, align: "left",queryType: 'cascader', set: true, query: true, },
          { field: 'processNo', title: '工序', minWidth: 100, align: "left", queryType: 'selectforce', set: true, query: true,},
          { field: 'status', title: '工位库类型', minWidth: 100, align: "center", queryType: 'select', set: true, query: true,},
        ]
      }
    ...
    methods: {
      handleChange(row){
        ...
        this.defaultForm.params.stationId = this.$refs['xzCascader'][0].getCheckedNodes()[0].data.id
      },
      getstocktypeData(){
      ...
      forceUpdate(v){
        this.defaultForm.params['processId'] = v.processId
      },
      ...
      formatterr(row, column, cellValue, index){
        ...
        }else if(column.property === 'whId' || column.property === 'stockType' || column.property === 'status'){
      ...
      getList() {
        ...
        const _data = { url: '/station-process/guest/query', params: this.queryParams }
        this.$store.dispatch('fmsCommon/actionMdmPost', _data)
          .then(data => {
            this.list = data.data
      ...
      submit(formName) {
        const _data = {
          url: '/station-process/add', params: {
      ...
      save(formName) {
        ...
        let _data = { url: '/station-process/add', params: { data: this.defaultForm.params } }
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$store.dispatch('fmsCommon/actionMdmPost', _data)
              .then(data => {
      ...
      del(index, row) {
        this.$confirm(i18n.t("common.tip_info.delete_info"), i18n.t("common.tip_info.tips"), {
          ...
          const _data = { url: '/station-process/del', params: { data: row.id } }
          this.$store.dispatch('fmsCommon/actionMdmPost', _data)
            .then(data => {
