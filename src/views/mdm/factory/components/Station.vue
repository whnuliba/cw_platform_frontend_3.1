<template>
  <div>
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="stationRowClick" @row-dblclick="showDetails" >
 ...
 directives: { waves, elDragDialog },
  ...
  data() {
    const  _initdata ={
      ...
      cols: [
                { field: 'stationNo', title: '工位编码',en_title:"TrayNo", editReadonly:true, align: "left",query:true,queryType:'input',set:true,require:true },               
                { field: 'stationName', title: '工位名称',align: "left",query:true,queryType:'input',set:true,require:true   },
                { field: 'stationDsc', title: '工位描述', align: "left",queryType:'input',set:true,require:true  },
  ...
  methods: {
    stationRowClick(row, column, event){
           this.$emit('changeStationclick',  row)
        },
    sectionInfo(val){
    ...
    getList() {
      ...
      const _data = {url:'/factory/guest/get-station',params:{data:this.sectionId}}
  ...
  submit(formName){
       Object.assign(this.defaultForm.params,{sectionId:this.sectionId})
       const _data = {url:'/station/add',params:{
  ...
  query(){
   ...
   save(formName){
      ...
      this.defaultForm.params.id=''
       Object.assign(this.defaultForm.params,{sectionId:this.sectionId})
      let _data = {url:'/station/add',params:{data:this.defaultForm.params}}
         this.$refs[formName].validate((valid) => {
          if (valid) {
              this.$store.dispatch('fmsCommon/actionMdmPost',_data)
              .then(data => {
   ...
   del(index,row){
      this.$confirm(i18n.t("common.tip_info.delete_info"), i18n.t("common.tip_info.tips"), {
          ...
          type: 'warning'
                ...
                const _data = {url:'/station/del',params:{data:row.id}}
                this.$store.dispatch('fmsCommon/actionMdmPost',_data)
                .then(data => {
