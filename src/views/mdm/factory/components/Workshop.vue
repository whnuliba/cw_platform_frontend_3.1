<template>
  <div>
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="workshopRowClick"  @row-dblclick="showDetails" >
 ...
 directives: { waves, elDragDialog },
  ...
  data() {
    const  _initdata ={
      ...
      cols: [
                { field: 'wsNo', title: '车间编码',en_title:"TrayNo", editReadonly:true, align: "left",query:true,queryType:'input',set:true,require:true },               
                { field: 'wsName', title: '车间名称',align: "left",query:true,queryType:'input',set:true,require:true   },
                { field: 'wsDsc', title: '车间描述', align: "left",queryType:'input',set:true,require:true  },
  ...
  methods: {
        workshopRowClick(row, column, event){
           this.$emit('changeWorkshopSection',  row.id)
        },
         initData(arr){
       let args = {}
          arr.map(c=>{
    ...
    getList() {
      ...
      const _data = {url:'/factory/guest/get-workshop',params:{data:this.factoryId}}
  ...
  submit(formName){
       Object.assign(this.defaultForm.params,{factoryId:this.factoryId})
       const _data = {url:'/workshop/add',params:{
  ...
  query(){
   ...
   save(formName){
      ...
      this.defaultForm.params.id=''
       Object.assign(this.defaultForm.params,{factoryId:this.factoryId})
      let _data = {url:'/workshop/add',params:{data:this.defaultForm.params}}
         this.$refs[formName].validate((valid) => {
          if (valid) {
              this.$store.dispatch('fmsCommon/actionMdmPost',_data)
              .then(data => {
   ...
   del(index,row){
      this.$confirm(i18n.t("common.tip_info.delete_info"), i18n.t("common.tip_info.tips"), {
          ...
          type: 'warning'
                ...
                const _data = {url:'/workshop/del',params:{data:row.id}}
                this.$store.dispatch('fmsCommon/actionMdmPost',_data)
                .then(data => {
