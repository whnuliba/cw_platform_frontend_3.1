<template>
  ...
  <div class="filter-container">
          ...
          <el-button type="info"     class="filter-item" @click="reset" >
           ...
           <el-button type="warning"   class="filter-item" @click="add"  v-ele-show="{btnCode:'add_pn_info'}">
               {{ $t("common.btn.add") }}
           </el-button>
     </div>
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="showDetails" >
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i class="el-icon-edit"  @click="edit(scope.row)"  v-ele-show="{btnCode:'add_pn_info'}"></i>
          &nbsp;
          <i class="el-icon-delete"  @click="del(scope.$index,scope.row)"  v-ele-show="{btnCode:'del_pn_info'}"></i>
        </template>
      </el-table-column>
    </el-table>
  ...
  data() {
    const  _initdata ={
      ...
      initSelect:{
         ...
         isKey:[{label:'关键件',value:100},{label:'非关键件',value:200}],
           pnAbc:[{label:'A类',value:"A"},{label:'B类',value:"B"},{label:'C类',value:"C"}],
         pnType:[{label:'成品',value:"H001"},{label:'半成品',value:"H002"}
                  ,{label:'加工件',value:"H003"}
                  ,{label:'外协件',value:"H004"}
                  ,{label:'采购件',value:"H005"}]
      }, 
      defaultForm: {
        row:{},
        params:{
      ...
      cols: [
                { field: 'pn', title: '产品编码',en_title:"PN",  align: "left",query:true,queryType:'input',editReadonly:true,set:true,require:true },               
                { field: 'pnName', title: '路径编码', align: "left",query:true,queryType:'input',set:true,require:true   },
                { field: 'pnAbc', title: 'ABC分类', align: "left",query:true,queryType:'select',set:true,require:true   },
                { field: 'pnDsc', title: '产品描述', align: "left",query:true,queryType:'input',set:true,require:true   },               
                { field: 'status', title: '状态', align: "left",query:true,queryType:'input',set:false,require:true   },
                { field: 'createTime', title: '创建日期', align: 'center' ,query:false,queryType:'date' },
                { field: 'gn', title: '图号', align: "left",queryType:'input',set:true,require:true  },
                { field: 'pnSpec', title: '规格', align: "left",queryType:'input',set:true,require:true  },
                { field: 'pnType', title: '产品类别', align: "left",queryType:'select',set:true,require:true  }, 
                { field: 'isKey', title: '关键件', align: "left",queryType:'select',set:true,require:true  },        
