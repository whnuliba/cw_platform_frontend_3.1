<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-ele-show="{btnCode:'mould_tool_query'}" type="primary" class="filter-item" @click="query">
        {{ $t("common.btn.query") }}
      </el-button>
      <el-button v-ele-show="{btnCode:'mould_tool_query'}" type="info"     class="filter-item" @click="reset" >
        {{ $t("common.btn.reset") }}
      </el-button>
      <el-button v-ele-show="{btnCode:'mould_tool_add'}" type="warning" class="filter-item" @click="add">
        {{ $t("common.btn.add") }}
      </el-button>
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'mould_tool_edit'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'mould_tool_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    const _initdata = {
      table_mark: 'mdm_tm_lst',
      dialogFormVisible: false,
      allowDisable: false,
      dialogSelectFieldVisible: false,
      url: {
        add: '/equipmentToolModel/add',
        del: '/equipmentToolModel/del',
        list: '/equipmentToolModel/list'
      },
      queryParams: {
        requestData: { }
      },
      initSelect: {
        ...
        modelType: [{ label: '模具', value: 'M003' }, { label: '工装', value: 'M006' }]
      },
      defaultForm: {
        row: {},
        params: {
      ...
      cols: [
        { field: 'modelNo', title: '工装/模具编号', align: 'left', editReadonly: true, width: '105px', query: true, queryType: 'input', set: true, require: true },
        { field: 'modelName', title: '工装/模具名称', align: 'left', width: '105px', query: true, queryType: 'input', set: true, require: true },
