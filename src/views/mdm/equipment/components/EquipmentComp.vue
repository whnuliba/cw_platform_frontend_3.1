<template>
  <div>
    <div class="filter-container">
      ...
      <!--<el-button v-ele-show="{btnCode:'ep_comp_query'}" type="primary" class="filter-item" @click="query">
        {{ $t("common.btn.query") }}
      </el-button>
      <el-button v-ele-show="{btnCode:'ep_comp_add'}" type="warning" class="filter-item" @click="add">
        {{ $t("common.btn.add") }}
      </el-button>-->
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'ep_comp_edit'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'ep_comp_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      deviceId: '',
      url: {
        add: '/equipmentComponent/add',
        del: '/equipmentComponent/del',
        list: '/equipmentComponent/guest/get-equipmentComponent'
      },
      queryParams: {
        requestData: { }
      },
      ...
      cols: [
        { field: 'compNo', title: '部件编号', align: 'left', editReadonly: true, query: false, queryType: 'input', set: true, require: true },
        { field: 'compName', title: '部件名称', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'compDsc', title: '部件描述', align: 'left', width: '600px', query: false, queryType: 'input', set: true, require: false },
        ...
        /* { field: 'deviceId', title: '设备ID', align: 'left', queryType: 'input', set: false, require: false } */
      ]
    }
  ...
  methods: {
    ...
    getList() {
      ...
      //Object.assign(this.queryParams, { requestData: { deviceId: this.deviceId }})
      //const _data = { url: this.url.list, params: this.queryParams }
      const _data = { url: this.url.list, params: { data: this.deviceId }}
    ...
    submit(formName) {
      Object.assign(this.defaultForm.params, { deviceId: this.deviceId })
