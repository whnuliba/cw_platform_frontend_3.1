<template>
  <div>
    <div class="filter-container">
      ...
      <el-button v-ele-show="{btnCode:'ep_main_query'}" type="primary" class="filter-item" @click="query">
        {{ $t("common.btn.query") }}
      </el-button>
      <el-button v-ele-show="{btnCode:'ep_main_query'}" type="info"     class="filter-item" @click="reset" >
        {{ $t("common.btn.reset") }}
      </el-button>
      <el-button v-ele-show="{btnCode:'ep_main_add'}" type="warning" class="filter-item" @click="add">
        {{ $t("common.btn.add") }}
      </el-button>
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-click="equipmentInfoRowClick" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'ep_main_edit'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'ep_main_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
 ...
 directives: { waves, elDragDialog },
  ...
  data() {
    const _initdata = {
      table_mark: 'mdm_epm_lst',
      dialogFormVisible: false,
            allowDisable: false,
      dialogSelectFieldVisible: false,
      url: {
          add: '/equipmentMain/add',
          del: '/equipmentMain/del',
          list: '/equipmentMain/list'
      },
      queryParams: {
        requestData: { }
        },
      initSelect: {
        deviceType:[],
         ...
         abc: [{ label: 'A类', value: 'A' }, { label: 'B类', value: 'B' }, { label: 'C类', value: 'C' }]
      },
      defaultForm: {
        row: {},
        params: {
      ...
      cols: [
                { field: 'deviceNo', title: '设备编号', en_title: 'DeviceNo', align: 'left', editReadonly: true, query: true, queryType: 'input', set: true, require: true },
                { field: 'assetNo', title: '资产编号', align: 'left', query: true, queryType: 'input', set: true, require: true },
                { field: 'deviceName', title: '设备名称', align: 'left', width: '150px', query: true, queryType: 'input', set: true, require: true },
                { field: 'deviceModel', title: '设备型号', align: 'left', query: false, queryType: 'input', set: true, require: false },
                { field: 'deviceType', title: '设备分类', align: 'left', query: true, queryType: 'select', set: true, require: true },
                { field: 'deviceAttr', title: '设备属性', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'abc', title: 'ABC分类', align: 'left', width: '80px', query: true, queryType: 'select', set: true, require: false },
                ...
                { field: 'devicePower', title: '设备功率', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'startDate', title: '启用日期', align: 'center', width: '100px', queryType: 'date', set: true, require: false },
                { field: 'sectionNo', title: '使用工段编号', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'sectionName', title: '使用工段', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'personNo', title: '责任人编号', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'personName', title: '责任人', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'archiveNo', title: '档案编号', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'manuDate', title: '制造日期', align: 'center', width: '100px', queryType: 'date', set: true, require: false },
                { field: 'outFactoryNo', title: '出厂编号', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'vendorNo', title: '生产商编号', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'vendorName', title: '生产商名称', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'vendorPhone', title: '生产商电话', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'vendorFax', title: '生产商传真', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'vendorAddr', title: '生产商地址', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'maintNo', title: '维修单位编号', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'maintName', title: '维修单位名称', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'maintPhone', title: '维修单位电话', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'maintFax', title: '维修单位传真', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'maintAddr', title: '维修单位地址', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'maintPerson', title: '维修联系人', width: '100', align: 'left', queryType: 'input', set: true, require: false },
                { field: 'maintPersonPhone', title: '维修人电话', width: '100', align: 'left', queryType: 'input', set: true, require: false }
    ]
    }
  ...
  methods: {
      equipmentInfoRowClick(row, column, event) {
        this.$emit('equipmentInfoRowClick', row)
      },
