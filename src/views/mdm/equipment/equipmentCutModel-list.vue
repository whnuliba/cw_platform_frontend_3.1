<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button v-ele-show="{btnCode:'cut_mould_query'}" type="primary" class="filter-item" @click="query">
        {{ $t("common.btn.query") }}
      </el-button>
      <el-button v-ele-show="{btnCode:'cut_mould_query'}" type="info"     class="filter-item" @click="reset" >
        {{ $t("common.btn.reset") }}
      </el-button>
      <el-button v-ele-show="{btnCode:'cut_mould_add'}" type="warning" class="filter-item" @click="add">
        {{ $t("common.btn.add") }}
      </el-button>
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'cut_mould_edit'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'cut_mould_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="120px">
        <div v-for="(item) in cols" :key="item.id" style="display:inline">
          <el-form-item
            ...
            :label="commonI18n(item)"
            :prop="item.field" label-width="120px"
          ...
          <el-form-item
            ...
            :label="commonI18n(item)" label-width="120px"
...
export default {
  ...
  data() {
    const _initdata = {
      table_mark: 'mdm_cm_lst',
      dialogFormVisible: false,
      allowDisable: false,
      dialogSelectFieldVisible: false,
      url: {
        add: '/equipmentCutModel/add',
        del: '/equipmentCutModel/del',
        list: '/equipmentCutModel/list'
      },
      queryParams: {
        requestData: { }
      },
      initSelect: {
        ...
        modelType: [{ label: '冲模', value: 'M001' }, { label: '刀具', value: 'M002' }]
      },
      defaultForm: {
        row: {},
        params: {
      ...
      cols: [
        { field: 'modelNo', title: '冲模/刀具编号', align: 'left', editReadonly: true, width: '105px', query: true, queryType: 'input', set: true, require: true },
        { field: 'modelName', title: '冲模/刀具名称', align: 'left', width: '105px', query: true, queryType: 'input', set: true, require: true },
        { field: 'pn', title: '物料编号', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'pnName', title: '物料名称', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'gn', title: '图号', align: 'left', query: false, queryType: 'input', set: true, require: false },
        { field: 'modelType', title: '类型', align: 'left', width: '80px', query: true, queryType: 'select', set: true, require: false },
        ...
        { field: 'inTime', title: '入库时间', align: 'center', width: '145px', queryType: 'datetime', set: true, require: false },
        { field: 'inOrderNo', title: '入库单号', align: 'center', queryType: 'input', set: true, require: false },
        { field: 'validationDate', title: '验证时间', align: 'center', width: '145px', queryType: 'datetime', set: true, require: false },
        { field: 'validationOrderNo', title: '验证单号', align: 'left', queryType: 'input', set: true, require: false },
        { field: 'periodDate', title: '有效期', align: 'left', queryType: 'input', set: true, require: false },
        { field: 'qty', title: '数量', align: 'left', queryType: 'input', set: true, require: false },
        { field: 'modelSpec', title: '规格型号', align: 'left', queryType: 'input', set: true, require: false },
        { field: 'useNumber', title: '使用次数', align: 'left', queryType: 'input', set: true, require: false },
        { field: 'modelLife', title: '寿命', align: 'left', queryType: 'input', set: true, require: false }
      ]
    }
