  ...
  <div>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
        ...
        <template slot-scope="scope">
          <i v-ele-show="{btnCode:'work_shift_edit'}" class="el-icon-edit" @click="edit(scope.row)" />
          &nbsp;
          <i v-ele-show="{btnCode:'work_shift_del'}" class="el-icon-delete" @click="del(scope.$index,scope.row)" />
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    const _initdata = {
      ...
      url: {
        add: '/workShift/add',
        del: '/workShift/del',
        list: '/workShift/guest/selectByCalendarId'
      },
      queryParams: {
        requestData: { }
      },
      initSelect: {
        ...
        isPfx: [{ label: '否', value: 0 }, { label: '是', value: 1 }]
      },
      defaultForm: {
        row: {},
        params: {
      ...
      cols: [
        { field: 'shiftNo', title: '班次编号', align: 'left', query: false, queryType: 'input', editReadonly: true, set: true, require: true },
        { field: 'shiftName', title: '班次名称', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'calendarStartTime', title: '开始偏移量', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'calendarEndTime', title: '长度', align: 'left', query: false, queryType: 'input', set: true, require: true },
        { field: 'isPfx', title: '是否跨天', align: 'left', query: false, queryType: 'select', set: true, require: true },
  ...
  methods: {
    ...
    getList() {
      ...
      const _data = { url: this.url.list, params: { data: this.calendarId }}
    ...
    submit(formName) {
      Object.assign(this.defaultForm.params, { calendarId: this.calendarId, calendarNo: this.calendarNo })
