<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column min-width="10px" label="序号" align="center" fixed>
      ...
      <el-table-column align="center" :label="$t('common.btn.operate')" fixed="right" width="250">
        <template slot-scope="scope">
          <el-button
            type="primary"
            size="small"
 ...
 directives: { waves, elDragDialog },
  ...
  data() {
    const  _initdata ={
      ...
      cols: [
        { field: 'trayNo', title: '托盘号', align: 'center', width: 125,query: true, queryType: 'input' },
        { field: 'createUser', title: '操作人', align: 'center', width: 150 },
        { field: 'createTime', title: '创建时间', align: 'center', width: 150 },
        { field: 'content', title: '解盘信息', align: 'left', query: true, queryType: 'input' },
  ...
  methods: {
    detail(row){
      this.$refs.ContentShow.detail(row.contentOld);
    },
         initData(arr){
       let args = {}
          arr.map(c=>{
    ...
    getList() {
      ...
      const _data = { url:'/split-plt-apply/list',params:this.queryParams}
      this.$store.dispatch('fmsCommon/actionFmsPost',_data)
      .then(data => {
        let returnData = data.data.map(item => {
          item.contentOld = item.content;
          item.content = (item.content == null || item.content.length <= 200) ? item.content : item.content.substring(0,200)
          return item;
        });
