<template>
  <div class="my-app-container" style="margin-top:10px;margin-left:10px;">
           <div class="select-header" style="margin-bottom:10px" >
           <el-select class="code-mode-select" v-model="options.mode"
                    @change="changeMode">
...
// 内部真实的内容
  ...
  watch:{
   code(val){
        let mode = 1
        if("javascript"===this.options.mode)
          mode = 0
       this.$emit('changeScript',{data:val,mode:mode})
    },
    scriptsEditer:function(val){
        const _this = this
      const _data = {url:'/esb/guest/query-script',params:{data:val}}
