<template>
    <div class="app-container">
      ...
      <el-dialog :title="$t('common.btn.edit')" v-el-drag-dialog :visible.sync="dialogFormVisible" width="700px" @close="destory(false)">
  ...
  <script>
    ...
    export default {
      ...
      toggleSelection(rows) {
        ...
        setScript(data){
        this.scriptOps.scriptValue=data.data
        this.scriptOps.mode=data.mode
    },
    myscript(row){
    ...
    saveScript(){
      ...
      const _this = this
       ...
       if(!isYaml){
         ...
         let mode = 1
        if("javascript"===_this.$refs.scriptsEditer.options.mode)
          mode = 0
      const _data = {url:'/esb/save-script',params:{data:{scriptText: _this.scriptOps.scriptValue,taskId: _this.scriptOps.scriptId,status:mode}}}
      this.$store.dispatch('fmsCommon/actionTaskPost',_data)
      .then(data => {
          this.$message({
                    type: 'success',
        ...
        destory(hide = false) {
          ...
          if(this.$refs['defaultForm.params']){
            this.$refs['defaultForm.params'].clearValidate()
          }
          this.defaultForm.params.scheduleType='ESB'
        },
        start(){
