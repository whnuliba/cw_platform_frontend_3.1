<template>
    <div class="app-container">
      ...
      <el-table  v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" :cell-style="cellStyle" @row-click="bomItemRowClick"  @row-dblclick="showqrcode">
        ...
        <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="350">
          <template slot-scope="scope">
            ...
            <el-button size="mini" type="primary" :disabled="scope.row.status==2" @click="del(scope.$index,scope.row)" >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-dialog v-el-drag-dialog :title="$t('common.btn.edit')" :visible.sync="dialogFormVisible" width="800px">
        <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="120px">
          <div v-for="(item) in cols" :key="item.id" style="display:inline">
            ...
            >
              ...
              <el-input v-model="defaultForm.params[item.field]" :disabled="allowDisable && item.editReadonly" :style="{...item.style}" />
            </el-form-item>

         
            <el-form-item
              v-if="item.set && item.queryType==='text-userProp'"
              :label="commonI18n(item)"
              :prop="item.field"
              label-width="150px"
          ...
          :rules="[{ required: item.required, message: item.title+'不能为空', trigger: 'change' }]">
            <el-select :filter-method="(val) => {return dataSelectFilter(val, item)}" :filterable="true" @change="(val)=>changeSelect(val,item)" v-model="defaultForm.params[item.field]" :disabled="allowDisable && item.editReadonly" placeholder="请选择" :style="{...item.style}">
            ...
            >
              ...
              <el-input
                ...
                :disabled="allowDisable && item.editReadonly"  :style="{...item.style}"
              >
              </el-input>
            ...
            >
              <el-select v-model="defaultForm.params[item.field]" placeholder="..." :style="{...item.style}" @change="$forceUpdate()">
                <el-option
                  v-for="item in initSelect[item.field]"
                  :key="item.value"
                  :label="item.label"
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button v-if="!rowData.status || rowData.status!=2" type="primary" @click="save('defaultForm.params')">  {{ $t("common.btn.save") }}</el-button>
          <el-button v-if="!rowData.status || rowData.status!=2" type="primary" @click="submit('defaultForm.params')">  {{ $t("common.btn.submit") }}</el-button>
        </div>
      </el-dialog>
    ...
    export default {
      ...
      data() {
        const _initdata = {
          ...
          cols: [
            ...
            // { field: 'qcSName', title: '标准单', align: 'left',hide:true, query: false, queryType: 'input', set: true, require: true, width:140, editReadonly:true },
     ...
     convertUserData(item){
        ...
        this.qcType = apArr[apArr.length-1]
        let bizCode =  cpName = apArr[apArr.length-2];
        let bizType = apArr[apArr.length-3];
        ...
        //    }
         ...
         // const _params =this.$route.params
          this.queryParams.requestData.qcType = this.qcType
      ...
      edit(row) {
        ...
        this.rowData = row
         ...
         this.defaultForm.params['qcType']=this.qcType
      },
      submit(formName) {
        this.$confirm('你确定要创建异常审批单吗？，创建后不可删除！', i18n.t('common.tip_info.tips'), {
              confirmButtonText: i18n.t('common.btn.ok'),
              cancelButtonText: i18n.t('common.btn.cancel'),
              type: 'warning'
            }).then(() => {
              this.defaultForm.row = {}
              this.defaultForm.params.status=2 //已经发起审批
              const _data = { url: this.url.submit, params: { data: {qcType:this.qcType,...this.defaultForm.params} }}
      ...
      query() {
       ...
       save(formName) {
          this.defaultForm.row = {}
          this.defaultForm.params.status=1 //未发起审批
          const _data = { url: this.url.add, params: { data: {qcType:this.qcType,...this.defaultForm.params} }}
             this.$refs[formName].validate((valid) => {
              if (valid) {
                  this.$store.dispatch('fmsCommon/actionProductPost', _data)
                  .then(data => {
