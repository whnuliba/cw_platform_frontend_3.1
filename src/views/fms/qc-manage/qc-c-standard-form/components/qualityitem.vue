  ...
  <div class="qc-standard">
    <div class="filter-container">
      <div v-for="(item,index) in cols" :key="index" style="display:inline">
        ...
        <el-input v-if="item.query && item.queryType==='orderNoselect'" v-model="queryParams.requestData[item.field]" :placeholder="item.title" style="width: 120px" class="filter-item">
        </el-input>
    ...
    <el-table :header-cell-class-name="headerCell" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"
      ...
      <el-table-column align="left" :label="$t('common.btn.operate')" width="200" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button type="primary" size="small" icon="el-icon-edit" @click="edit(scope.row)" v-if="scope.row.status != 2">
            {{ $t("common.btn.edit") }}
          </el-button>
           &nbsp;
          <!-- <el-button v-if="scope.row.useState !== 1&&scope.row.status != 2" type="warning"  size="small" icon="el-icon-edit" @click="del(scope.$index,scope.row)">
            {{ $t("common.btn.delete") }}
          </el-button> -->
      ...
      <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="80px">
        <div v-for="(item) in form_cols" :key="item.id" style="display:inline">
          <el-form-item v-if="item.set && item.queryType==='input'" :label="commonI18n(item)" :prop="item.field"
            ...
            <el-input v-model="defaultForm.params[item.field]" :disabled="item.editReadonly" placeholder="" style="width:180px;" />
          </el-form-item>
          <el-form-item  v-if="item.set && item.queryType==='textarea'" :label="commonI18n(item)" :prop="item.field"
                        :rules="[{ required: item.required, message: item.title+'不能为空', trigger: 'blur' }]">
            <el-input type="textarea" v-model="defaultForm.params[item.field]" :disabled="allowDisable && item.editReadonly" placeholder="" style="width:180px;" />
          </el-form-item>
          <!-- <el-form-item v-if="item.set && item.queryType==='input'" :label="commonI18n(item)" :prop="item.field"
                        :rules="[{ required: item.required, message: item.title+'不能为空', trigger: 'blur' }]">
          ...
          :rules="[{ required: item.required, message: item.title+'不能为空', trigger: 'change' }]">
            <el-select :filterable="true" v-model="defaultForm.params[item.field]" :disabled="allowDisable && item.editReadonly" @change="changeSele" placeholder="请选择" style="width:180px;">
              <el-option v-for="it in initSelect[item.field]"
                         :key="it[item.field]"
                         :label="it[item.label]+'('+it[item.combVal]+')'"
                         :value="it[item.field]+'|'+it[item.combVal]+'|'+it[item.commId]">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="item.set && item.queryType==='orderNoselect'" :label="item.title"
          :rules="[{ required: item.required, message: item.title+'不能为空', trigger: 'change' }]">
            <el-select :filter-method="filterMethod" :filterable="true" @change="orderNoselectchange" v-model="defaultForm.params[item.field]" :disabled="allowDisable && item.editReadonly" placeholder="请选择" style="width:180px;">
              <el-option v-for="it in initSelect[item.field]"
                         :key="it.value"
                         :label="it.label"
                         :value="it.value">
          ...
          <el-form-item v-if="item.set && item.queryType==='changeSelect'" :label="item.title"
          :rules="[{ required: item.required, message: item.title+'不能为空', trigger: 'change' }]">
            <el-select :filter-method="(val) => {return dataSelectFilter(val, item)}" :filterable="true" @change="changeSelect(item.field)" v-model="defaultForm.params[item.field]" :disabled="allowDisable && item.editReadonly" placeholder="请选择" style="width:180px;">
              <el-option v-for="it in initSelect[item.field]"
                         :key="it.value"
                         :label="it.label"
                         :value="it.value">
          ...
          :rules="[{ required: item.required, message: item.title+'不能为空', trigger: 'change' }]">
            <el-select v-model="defaultForm.params[item.field]" :disabled="allowDisable && item.editReadonly" @change="changeSele" placeholder="请选择" style="width:180px;">
              <el-option v-for="it in initSelect[item.field]"
                         :key="it[item.field]"
                         :label="it[item.field]"
                         :value="it[item.field]">
  ...
  export default {
    ...
    data() {
      const _initdata = {
        ...
        cols: [
          { field: 'checkNo', title: '检验单', minWidth: 150, align: "left" ,query: false, queryType: 'input', set: true,query: true,  },
          { field: 'orderNo', title: '工单', minWidth: 150, align: "left" , queryType: 'input', set: true,query: true  },
          { field: 'pn', title: '产品型号', minWidth: 150, align: "left" , query: true, queryType: 'input', set: true,  },
          { field: 'lotNo', title: '批次', minWidth: 150, align: "left" , queryType: 'input', set: true,query: true  },
          { field: 'planNo', title: '排产号', minWidth: 150, align: "left" ,query: true, queryType: 'input', set: true,  },
          { field: 'processNo', title: '工序', minWidth: 150, align: "left" , queryType: 'select', set: false,query: true,  },
          { field: 'deviceNo', title: '设备编码', minWidth: 150, align: "left" ,query: false, queryType: 'input', set: true,query: true,  },
          { field: 'sn', title: '编码', minWidth: 150, align: "left" , queryType: 'input', set: true,  },
          { field: 'checkConclusion', title: '检验结论', minWidth: 150, align: "left" ,query: true, queryType: 'input', set: true,  },
          { field: 'cause', title: '原因描述', minWidth: 150, align: "left" , queryType: 'input', set: true,query: true,  },          
          ...
          { field: 'udf2', title: '完成人', minWidth: 150, align: 'left', query: false,  set: true  },
          { field: 'createTime', title: '创建时间', minWidth: 150, align: 'left', query: false,  set: true  },
          { field: 'udf1', title: '创建人', minWidth: 150, align: 'left', query: false,  set: true  },
          { field: 'createUser', title: '创建人工号', minWidth: 150, align: 'left', query: false,  set: true  },
          { field: 'standardType', title: '标准类型', minWidth: 150, align: 'left', query: false,  set: true  },
        ],
        form_cols: [
          { field: 'orderNo', title: '工单', minWidth: 150, align: "left" , queryType: 'orderNoselect', set: true,query: true,required:true  },
          { field: 'pn', title: '产品型号', minWidth: 150, align: "left" , query: true, queryType: 'input', set: true,editReadonly:true,required:true  },
          { field: 'qcSCode', combVal:'version',commId:'id', title: '标准单', label:'qcSName', minWidth: 150, align: "left" ,query: true, queryType: 'select', set: true,required:true  },
          // { field: 'standardType', title: '标准类型', minWidth: 150, align: 'left', query: false, queryType: 'input', set: true  },
          { field: 'deviceNo', title: '干系机台', minWidth: 150, align: "left" ,query: false, queryType: 'changeSelect', set: true,  },
          ...
          { field: 'planNo', title: '排产号', minWidth: 150, align: "left" ,query: false, queryType: 'input', set: true,editReadonly:true,required:true },
          { field: 'lotNo', title: '批次', minWidth: 150, align: "left" , queryType: 'input', set: true,query: true  },
          { field: 'sn', title: '编码', minWidth: 150, align: "left" , queryType: 'changeSelect', set: true,  },
          { field: 'cause', title: '原因描述', minWidth: 150, align: "left" , queryType: 'textarea', set: true,  },
        ],
        copyToProdId:'',
        extraBtn: [
    ...
    methods: {
      ...
      orderNoselectchange(val){
        // console.log(val);
        // console.log(this.initSelect['orderNo']);

        this.orderNoData.forEach(item=>{
          if(item.orderNo == val){
            // console.log(val);
            this.defaultForm.params.pn = item.pn2
            this.defaultForm.params.planNo = item.system
            this.defaultForm.params.orderNo = item.orderNo
            this.defaultForm.params.lotNo = item.lotNo
          }
        })
      ...
      filterMethod(val){
        // console.log(val);
        if(val){
          const _data2 = { url: '/pm-order/select-order',params: {data: val}}
            this.$store.dispatch('fmsCommon/actionProductPost', _data2)
            .then(data => {
              this.orderNoData = data
              this.initSelect['orderNo'] = data.map(c=>{
                return {
                  label:c.orderNo+'|'+c.extOrderNo+'|'+c.pn2+'|'+c.system,
                  value:c.orderNo
                }
              })
              }).catch(err => {
      ...
      dataSelectFilter(val,item){
         ...
         let _commId = this.defaultForm.params.qcSCode.split('|')[2]
         if(val && field==='sn' && this.defaultForm.params.orderNo && _commId){
          const _data2 = { url: '/qc-check-form/query-sn',params: {data: {sn:val,commId:_commId,orderNo:this.defaultForm.params.orderNo}}}
            this.$store.dispatch('fmsCommon/actionProductPost', _data2)
            .then(data => {
             // this.orderNoData = data
              this.initSelect['sn'] = data.map(c=>{
                return {
                  label:c.snName+'|'+c.sn,
                  value:c.sn
                }
      ...
      getList() {
        ...
        const _data = { url: '/check-form/items', params: this.queryParams }
        this.$store.dispatch('fmsCommon/actionProductPost', _data)
          .then(data => {
            this.list = data.data
      ...
      selectOptions() {
        // 产品标准
          ...
          const _params = this.routeObj
          this.queryParams.requestData.qcType = _params.qcType
          const com_data = { url: '/qc-standard/comm-standard', params: {data:_params.qcType} }
          this.$store.dispatch('fmsCommon/actionProductPost', com_data)
            .then(data => {
              this.initSelect.qcSCode = data.filter(item=>{
                  return item.useState == 1
                })
                this.initSelect.version = data.filter(item=>{
                  return item.useState == 1
                })
