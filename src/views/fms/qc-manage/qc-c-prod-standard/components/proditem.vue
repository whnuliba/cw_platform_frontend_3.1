            ...
            <!-- v-ele-show="{btnCode:'DEL_TIMER_TASK'}" -->
                <el-dropdown @command="(e)=>controlBtn(scope,e)">
                      ...
                      <el-dropdown-menu  slot="dropdown">
                        ...
                        display: scope.row.status === 1 && dr.key ==='standardCompelete'?
                        'none':(scope.row.status === 2 && dr.key ==='standardClose'?
                        'none':(scope.row.status === 1 && dr.key ==='dele'? 'none' :'block')), }"
                        />
                      </el-dropdown-menu>
                </el-dropdown>
    ...
    export default {
      ...
      data() {
        const _initdata = {
          ...
          initSelect: {
            status: [{ label: '请选择', value: '' },{ label: '启用', value: 1 }, { label: '禁用', value: 2 }, { label: '暂存', value: 3 }],
          ...
          cols: [
            ...
            { field: 'system', title: '体系', minWidth: 150, align: 'left', query: false, queryType: 'input', set: true  },
            ...
            { field: 'processNo', title: '工序编码', minWidth: 150, align: "left" , queryType: 'input', require:false, set: true,editReadonly:true  },
            ...
            { field: 'status', title: '状态', width: 100, align: 'left', query: true, queryType: 'select', set: true  },
            { field: 'useState', title: '通用状态', width: 100, align: 'left', query: false, queryType: 'select', set: true  },
      ...
      methods: {
        ...
        selectOptions() {
          // 质量类型数据
            ...
            this.$store.dispatch('fmsCommon/actionProductPost', _data)
                  .then(data => {
                      this.initSelect['qcType'] = data.map(c=>{
                        return {
                          label:c.name,
                          value:c.value
                        }
        ...
        testOnFinish(formData) {
          let params = {id:this.copyToProdId,commId:this.copyToProdRow.commId,...formData}

          const dataCopy = { url: '/qc-prod-standard/copyProdItem', params: {data: params}}
             this.$store.dispatch('fmsCommon/actionProductPost', dataCopy)
                  .then(data => {
                      this.$message({
                        type: 'success',
