            ...
            <!-- v-ele-show="{btnCode:'DEL_TIMER_TASK'}" -->
                <el-dropdown @command="(e)=>controlBtn(scope,e)">
                      ...
                      <el-dropdown-menu  slot="dropdown">
                        ...
                        display:scope.row.useState === '启用' && dr.key ==='standardCompelete'? 
                        'none':(scope.row.useState === '禁用' && dr.key ==='standardClose'?
                        'none':(scope.row.useState === '启用' && dr.key ==='dele'? 'none' :'block')) }"
                        />
                      </el-dropdown-menu>
                </el-dropdown>
          </template>
    ...
    export default {
      ...
      data() {
        const _initdata = {
          ...
          cols: [
            ...
            { field: 'qcType', title: '质量类型', minWidth: 150, align: 'left', queryType: 'select', require:true, set: true   },
            ...
            { field: 'useState', title: '使用状态', edit: 'text', align: 'left', width: 200, queryType: 'switch', require:false, set: true,fixed: 'right' },
          ],
          copyToProdId:'',
          dropBtn: [
      ...
      methods: {       
        ...
        getList() {
          ...
          const _data = { url: '/qc-standard/guest/query', params: this.queryParams }
          this.$store.dispatch('fmsCommon/actionProductPost', _data)
            .then(data => {
