 ...
 <template>
  <div class="app-container">
        <div class="filter-container">
          ...
          <el-button type="primary"     class="filter-item"  @click="query()" v-ele-show="{btnCode:'SYS_PATAMETER_DTS_LIST'}" >
             {{ $t("common.btn.query") }}
          </el-button>
          <el-button type="info"     class="filter-item" @click="reset" >
             {{ $t("common.btn.reset") }}
          </el-button>
           <el-button type="warning"   class="filter-item" @click="add"  v-ele-show="{btnCode:'SYS_PARAMETER_DTS_ADD'}" >
               {{ $t("common.btn.add") }}
           </el-button>
        </div>
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="showDetails" >
      ...
      <el-table-column align="center" label="操作" width="200px">
        <template slot-scope="scope">
          <el-button type="primary" size="small"  icon="el-icon-edit" @click="edit(scope.row)" v-ele-show="{btnCode:'SYS_PARAMETER_DTS_ADD'}">
             {{ $t("common.btn.edit") }}
          </el-button>
           &nbsp;
           <el-button type="warning"  size="small" icon="el-icon-edit" @click="del(scope.$index,scope.row)" v-ele-show="{btnCode:'SYS_PARAMETER_DTS_DEL'}">
              {{ $t("common.btn.delete") }}
          </el-button>
        </template>
      </el-table-column>
  ...
  <!-- 页码设置 -->
    ...
    <el-dialog title="编辑" v-el-drag-dialog :visible.sync="dialogFormVisible" width="700px" :append-to-body="true" :modal-append-to-body="false">
      <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="80px">
        <div v-for="(item) in cols" :key="item.id" style="display:inline">
          ...
          <el-form-item  v-if="item.set && item.queryType==='textArea'" :label="item.title" :prop="item.field"
            :rules="[{ required: true, message: item.title+'不能为空', trigger: 'blur' }]"
          >
            <el-input
 ...
 directives: { waves, elDragDialog },
  ...
  data() {
    const  _initdata ={
      ...
      cols: [
                { field: 'paramKey', title: '参数编码', minWidth: 100, align: "center",query:true,queryType:'input',set:true  },
                { field: 'paramValue', title: '参数值', minWidth: 100, query:true, align: "center",queryType:'input',set:true },
                { field: 'createTime', title: '创建日期', align: 'center' ,query:false,queryType:'date' },               
                { field: 'createUser', title: '创建人', align: 'center' },
                { field: 'paramDsc', title: '描述', minWidth: 100, align: "center",queryType:'textArea',set:true },
                { field: 'status', title: '使用状态', align: 'center',query:true,queryType:'select',set:true,filter:true},
        ]
    }
  ...
  methods: {
   ...
   //初始化表格信息
    getList() {
      ...
      this.queryParams.requestData.paramId = this.rowId;
      Object.assign(this.queryParams,this.listQuery)
      const _data = {url:'/sys-p/list',params:this.queryParams}
  ...
  submit(formName){
       const _data = {url:'/sys-p/add',params:{
          data:{
  ...
  query(){
   ...
   save(formName){
      ...
      this.defaultForm.params.paramId = this.rowId
      let _data = {url:'/sys-p/add',params:{data:this.defaultForm.params}}
         this.$refs[formName].validate((valid) => {
          if (valid) {
              this.$store.dispatch('fmsCommon/actionAuthPost',_data)
              .then(data => {
   ...
   del(index,row){
      this.$confirm(i18n.t("common.tip_info.delete_info"), i18n.t("common.tip_info.tips"), {
          ...
          type: 'warning'
                ...
                const _data = {url:'/sys-p/del',params:{data:row.id}}
                this.$store.dispatch('fmsCommon/actionAuthPost',_data)
                .then(data => {
