...
<template>
  <div class="app-container">
        <div class="filter-container">
          ...
          <el-button type="info"  v-ele-show="{btnCode:'EDIT_STATE_CODE'}"    class="filter-item" @click="reset" >
             {{ $t("common.btn.reset") }}
          </el-button>
     ...
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="showDetails" >
      ...
      <el-table-column align="left" label="操作" width="200">
        <template slot-scope="scope">
          <el-button type="primary" size="small" v-ele-show="{btnCode:'EDIT_STATE_CODE'}"  v-if="scope.row.status!=1"  icon="el-icon-edit" @click="edit(scope.row)">
             {{ $t("common.btn.edit") }}
          </el-button>
           &nbsp;
           <el-button type="warning" v-ele-show="{btnCode:'DEL_STATE_CODE'}"  v-if="scope.row.status!=1"   size="small" icon="el-icon-edit" @click="del(scope.$index,scope.row)">
              {{ $t("common.btn.delete") }}
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    ...
    const _initdata = {
      ...
      initSelect:{
          status:[{label:'用户',value:0},{label:'系统编码',value:1}]
      },
      defaultForm: {
        row:{},
        params:{
      ...
      cols: [
            { field: 'stateValue', title: '状态编码', minWidth: 100, align: "center",query:true,queryType:'input',set:true},
            { field: 'stateName', title: '状态描述', align: 'center' ,query:true,queryType:'input',set:true},
            { field: 'status', title: '参数类别', align: 'center'},
  ...
  methods: {
    ...
    getList() {
      ...
      const _data = {url:'/cellStateCodeController/list',params:this.queryParams}   // /state-code/list
  ...
  submit(formName){
       const _data = {url:'/state-code/add',params:{
  ...
  query(){
   ...
   save(formName){
      ...
      let _data = {url:'/state-code/add',params:{data:this.defaultForm.params}}
         this.$refs[formName].validate((valid) => {
          if (valid) {
              this.$store.dispatch('fmsCommon/actionFmsPost',_data)
              .then(data => {
   ...
   del(index,row){
      this.$confirm(i18n.t("common.tip_info.delete_info"), i18n.t("common.tip_info.tips"), {
          ...
          type: 'warning'
                ...
                const _data = {url:'/state-code/del',params:{data:row.id}}
                this.$store.dispatch('fmsCommon/actionFmsPost',_data)
                .then(data => {
