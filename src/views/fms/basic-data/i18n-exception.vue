       ...
       <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="showDetails" >
        ...
        <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="150">
          ...
          <template slot-scope="scope">
            ...
            &nbsp;
            <i class="el-icon-edit" title="刷新到缓存"  @click="refresh(scope.row)"  ></i>
            &nbsp;
            <i class="el-icon-edit" title="刷新到缓存"  @click="refreshAll(scope.row)"  ></i>
          </template>
        </el-table-column>
      </el-table>
   ...
   directives: { waves, elDragDialog },
    ...
    data() {
      const  _initdata ={
        ...
        cols: [
                  { field: 'exceptionCode', title: '编码', align: "left",query:true,queryType:'input',set:false,require:true   },
                  { field: 'zh', title: '中文',en_title:"PN",  align: "left",query:true,queryType:'input',set:true,require:true },               
                  { field: 'en', title: '英文', align: "left",query:true,queryType:'input',set:true,require:false   },
                  { field: 'vn', title: '越文', align: "left",query:true,queryType:'input',set:true,require:false   },
                  { field: 'ger', title: '德文', align: "left",query:true,queryType:'input',set:true,require:false   },               
    ...
    refresh(row){
        const _data = {url:"/i18n/guest/refresh",params:{
            data:row.exceptionCode
         }}
         this.$store.dispatch('fmsCommon/actionAuthPost',_data)
                .then(data => {
                this.$message({
    ...
    refreshAll(row){
        const _data = {url:"/i18n/guest/refreshAll",params:{
            data:row.exceptionCode
         }}
         this.$store.dispatch('fmsCommon/actionAuthPost',_data)
                .then(data => {
