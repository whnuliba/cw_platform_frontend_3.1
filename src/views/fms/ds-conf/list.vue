 ...
 directives: { waves, elDragDialog},
  ...
  data() {
    const  _initdata ={
      ...
      cols: [
                { field: 'dsCode', title: '数据源编码', align: 'center',queryType:'input',set:true,require:true},
                { field: 'dsName', title: '数据源名称', align: 'center',queryType:'input',set:true,require:true },
                { field: 'desc', title: '业务描述', align: 'center',queryType:'input',set:true,require:true },
  ...
  methods: {
    ...
    getList() {
      ...
      const _data = {url:'/dsconf/list',params:this.queryParams}
    ...
    saveScript(){
      ...
      const _data = {url:'/dsconf/save-script',params:{data:{configure: _this.scriptOps.scriptValue,id: _this.scriptOps.scriptId}}}
      this.$store.dispatch('fmsCommon/actionTaskPost',_data)
      .then(data => {
          this.$message({
                    type: 'success',
  ...
  submit(formName){
       const _data = {url:'/dsconf/add',params:{
  ...
  query(){
   ...
   save(formName){
      ...
      let _data = {url:'/dsconf/add',params:{data:this.defaultForm.params}}
         this.$refs[formName].validate((valid) => {
          if (valid) {
              this.$store.dispatch('fmsCommon/actionTaskPost',_data)
              .then(data => {
   ...
   del(index,row){
      this.$confirm(i18n.t("common.tip_info.delete_info"), i18n.t("common.tip_info.tips"), {
          ...
          type: 'warning'
                ...
                const _data = {url:'/dsconf/del',params:{data:row.id}}
                this.$store.dispatch('fmsCommon/actionTaskPost',_data)
                .then(data => {
