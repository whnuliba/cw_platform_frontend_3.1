<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button type="warning" class="filter-item" @click="add" v-ele-show="{btnCode:'ADD_DEVICE'}">
        {{ $t("common.btn.add") }}
      </el-button>
    </div>
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @row-dblclick="showDetails">
      ...
      <el-table-column align="left" label="操作" width="200">
        <template slot-scope="scope">
          ...
          &nbsp;
          <el-button type="warning" v-ele-show="{btnCode:'DELETE_DEVICE'}" size="small" icon="el-icon-edit" @click="del(scope.$index,scope.row)">
            {{ $t("common.btn.delete") }}
          </el-button>
        </template>
      </el-table-column>
      ...
      <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="80px">
        <div v-for="(item) in cols" :key="item.id" style="display:inline">
          <el-form-item v-if="item.set && item.queryType==='input'" :label="item.title" :prop="item.field"
            ...
            <el-input v-model="defaultForm.params[item.field]" suffix-icon="xxxx"/>
          </el-form-item>
  ...
  export default {
    ...
    data() {
      const _initdata = {
        ...
        cols: [
          { field: 'deviceNo', title: '设备编码', minWidth: 100, align: "center", query: true, queryType: 'input', set: true },
          { field: 'deviceName', title: '设备名称', minWidth: 100, align: "center",queryType: 'input', set: true },
          { field: 'IP', title: 'IP', minWidth: 100, align: "center", queryType: 'input', set: true },
          { field: 'port', title: 'port', minWidth: 100, align: "center", queryType: 'input', set: true },
          { field: 'protocolDesc', title: '通讯明细', align: 'center' , queryType: 'input', set: true},
          { field: 'protocol', title: '通讯协议', minWidth: 100, align: "center", queryType: 'select', set: true },
          { field: 'connectionStr', title: '连接串', align: 'center' },
          { field: 'useState', title: '使用状态', align: 'center', queryType: 'select', set: true, filter: true },
    ...
    methods: {
      ...
      getList() {
        ...
        const _data = { url: '/deviceInfo/list', params: this.queryParams }   ///deviceinfo/getdeviceinfo
      ...
      save(formName) {
        ...
        let _data = { url: '/deviceinfo/add', params: { data: this.defaultForm.params } }
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$store.dispatch('fmsCommon/actionFmsPost', _data)
              .then(data => {
      ...
      del(index, row) {
        this.$confirm(i18n.t("common.tip_info.delete_info"), i18n.t("common.tip_info.tips"), {
          ...
          const _data = { url: '/deviceinfo/del', params: { data: row.id } }
          this.$store.dispatch('fmsCommon/actionFmsPost', _data)
            .then(data => {
