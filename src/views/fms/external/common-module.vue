  ...
  <div class="app-container">
        ...
        <div class="filter-container">
            <div   v-for="(item,index1) in properties" :key="index1" style="display:inline">
                  <el-input  v-if="item.query && item.queryType==='input'" v-model="attr[item.field]" :placeholder="commonI18n(item)" :style="'width:' +(item.width||180)+'px;min-width:'+item.minWidth+'px;'" class="filter-item">
                     <!-- {{queryParams.requestData[item.field]=''}} -->
               </el-input>
                  <el-select   v-if="item.query && item.queryType==='select'" v-model="attr[item.field]" :placeholder="commonI18n(item)" :style="'width:' +(item.width||180)+'px;min-width:'+item.minWidth+'px;'" class="filter-item">
                    <el-option
                ...
                <el-date-picker v-if="item.query && item.queryType==='datetime'" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"  v-model="attr[item.field]"  type="datetime"   :placeholder="commonI18n(item)"  style="width: 150" class="filter-item"> </el-date-picker>
                <el-date-picker v-if="item.query && item.queryType==='date'" format="yyyy-MM-dd HH:mm:ss" value-format="yyyy-MM-dd HH:mm:ss"  v-model="attr[item.field]"  type="date"   :placeholder="commonI18n(item)"  style="width: 150" class="filter-item"> </el-date-picker>
                 <!-- :picker-options="pickerOptions" -->
                 <el-date-picker v-if="item.query && item.queryType==='month'" format="yyyy-MM" value-format="yyyy-MM"  v-model="attr[item.field]"  type="month"   :placeholder="commonI18n(item)"  style="width: 150" class="filter-item"> </el-date-picker>
                 <el-date-picker v-if="item.query && item.queryType==='commonDate'" :format="item.format" :value-format="item.format"  v-model="attr[item.field]"  :type="item.dateType"   :placeholder="commonI18n(item)"  style="width: 150" class="filter-item"> </el-date-picker>
          ...
          <div   v-for="(item,index) in cols" :key="index" style="display:inline">
                  <el-input  v-if="item.query && item.queryType==='input'" v-model="queryParams.requestData[item.field]" :placeholder="commonI18n(item)" :style="'width:' +(item.width||180)+'px;min-width:'+item.minWidth+'px;'" class="filter-item">
                     <!-- {{queryParams.requestData[item.field]=''}} -->
               </el-input>
                  <el-select   v-if="item.query && item.queryType==='select'" v-model="queryParams.requestData[item.field]" :placeholder="commonI18n(item)" :style="'width:' +(item.width||180)+'px;min-width:'+item.minWidth+'px;'" class="filter-item">
                    <el-option
                    v-for="item in initSelect[item.field]"
                    :key="item.value"
                    :label="commonI18n(item)"
     ...
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="showDetails" >
      ...
      <el-table-column min-width="50px"  :label="$t('common.no')" align="center" fixed>
      ...
      <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="80px">
        <div v-for="(item) in cols" :key="item.id" style="display:inline">
          <el-form-item  v-if="item.set && item.queryType==='input'" :label="item.title" :prop="item.field"
            :rules="[{ required: item.require, message: item.title+'can not be empty', trigger: 'blur' }]"
          >
           ...
           <el-form-item  v-if="item.set && item.queryType==='select'" :label="item.title"
            :rules="[{ required: item.require, message: item.title+'can not be empty', trigger: 'change' }]"
           >
              <el-select v-model="defaultForm.params[item.field]" @change="$forceUpdate()" :placeholder="$t('common.options')">
                <el-option
                  v-for="item in initSelect[item.field]"
                  :key="item.value"
                  :label="item.label"
    ...
    <!--处理导出字段选择的穿梭-->
     ...
     <el-dialog :title="$t('common.exportOptions')" v-el-drag-dialog :visible.sync="dialogEcportVisible" width="700px" >
      <el-form ref="exportColumn" :inline="true" :model="exportColumn" label-width="80px">

          <template>
                 <el-transfer v-model="exportColumn.fields"
                   :titles="[$t('common.source'), $t('common.target')]"
                  :data="exportColumn.colMap"></el-transfer>
          </template>
      </el-form>
      <div>
         <span>
          {{$t('common.explain') +':'+ $t('common.maxExport')+'['}}
         </span>
...
export default {
  ...
  methods: {
      ...
      if(!typeCode){
             this.$message({
                    type: 'info',
                    message: '初始化常量没有指定编码|The initialization constant does not specify the encoding'
          });
      }
     ...
     // const _params =this.$route.params
       ...
       const fullpath =this.$route.path
        ...
        if (!apArr.length || apArr.length<2) 
        {
            this.$message({
            type:"error",
            message: '路由错误|The routing error'
            })
            return
        }
        ...
        if(!bizCode)
        {
            this.$message({
            type:"error",
            message: '路由编码不存在|The routing code does not exist'
            })
            return 
        }
       ...
       let _params = {bizType:bizType,bizCode:bizCode}
            ...
            if(data || data.length>0){
                const scrpt = data[0]
                 ...
                 this.actionReport(null)
                      ...
                      this.$message({
                    type: 'info',
                    message: '没有找到业务类型对应的报表，请联系管理员检查参数配置|The report corresponding to the business type was not found. Please contact the administrator to check the parameter configuration'
                  });
                  return
            }
        })
    ...
    actionReport(id,load=false){
      ...
      selectFields(){
          ...
          this.currScripts.cols.forEach(item=>{
             if(item.export){
             this.exportColumn.origin[item.field] = that.commonI18n(item)
             this.exportColumn.colMap.push({
                  key: item.field,
                  label: that.commonI18n(item),
                  disabled: false
              })
            }
          })
      ...
      exportCSV(){
          const {where,sqlArgs,parameters,whereis} = this.setSqlWhere()
          const fields = {}
          // this.currScripts.cols.forEach(item=>{
          //    if(item.export){
          ...
          if(this.exportColumn.fields.length==0)
             {
                    this.$message({
                    type: 'info',
                    message: that.$t('common.notExportItem')//'没有需要导出的字段'
                  });
                  return
             }
          ...
          let args = {
           ...
           whereis:whereis,
           collectionName:this.currScripts.collectionName||'',
           orderBy:this.currScripts.orderBy||"",
           asc:this.currScripts.asc||true
        }
        if(this.properties){
       ...
       if(sqlArgs.length>0){
        ...
        if(this.currScripts.beforeAction && typeof this.currScripts.beforeAction ==="function"){
          let err = this.currScripts.beforeAction.call(this,args,this.queryParams)
          if(err && !err.state){
            this.$message({
                    type: 'error',
                    message: err.msg
      ...
      setSqlWhere(){
        const where = this.querySql.where=[]
          ...
          this.currScripts.cols.forEach(item => {
              if(item.query|| item.where){
                  ...
                  if(typeof this.queryParams.requestData[item.field]===undefined ||this.queryParams.requestData[item.field] =='' || this.queryParams.requestData[item.field]==null)
                   ...
                   if(cond=='like')
                     {
                         whereis.push({field:item.field,cond:cond,value:this.queryParams.requestData[item.field],logic:logic,dataType:item.queryType||null})
                      ...
                      where.push(item.field+" like /"+this.queryParams.requestData[item.field]+"$/ ")
                       ...
                       if(this.paramType==1){
                        ...
                        if(this.queryParams.requestData[item.field]){
                         ...
                         whereis.push({field:item.field,cond:">=",value:this.queryParams.requestData[item.field][0],logic:logic,dataType:item.queryType||null})
                         where.push(item.field+ "< ?")
                         sqlArgs.push(this.queryParams.requestData[item.field][1])
                         whereis.push({field:item.field,cond:"<",value:this.queryParams.requestData[item.field][1],logic:logic,dataType:item.queryType||null})
         ...
         return {where:where,sqlArgs:sqlArgs,parameters:parameters,whereis:whereis}
      },
      requestData(){
        //计算条件
        const {where,sqlArgs,parameters,whereis} = this.setSqlWhere()
