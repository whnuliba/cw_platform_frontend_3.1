<template>
  <div class="app-container">
        <div class="filter-container">
          ...
          <el-button type="primary"   v-ele-show="{btnCode:'fms_task_external_query'}"   class="filter-item"  @click="query()" >
             {{ $t("common.btn.query") }}
          </el-button>
          <el-button type="info"     class="filter-item" @click="reset" >
             {{ $t("common.btn.reset") }}
          </el-button>
           <el-button type="warning"   class="filter-item" @click="add" v-ele-show="{btnCode:'fms_task_external_add'}">
               {{ $t("common.btn.add") }}
           </el-button>
        </div>
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="showDetails" >
      ...
      <el-table-column align="left" label="操作" fixed="right" width="280">
        <template slot-scope="scope">
          <el-button type="primary" size="small" v-ele-show="{btnCode:'fms_task_external_add'}"   icon="el-icon-edit" @click="edit(scope.row)">
             {{ $t("common.btn.edit") }}
          </el-button>
           &nbsp;
           <el-button type="warning"  v-ele-show="{btnCode:'fms_task_external_delete'}"   size="small" icon="el-icon-edit" @click="del(scope.$index,scope.row)">
              {{ $t("common.btn.delete") }}
          </el-button>
 ...
 directives: { waves, elDragDialog},
  ...
  data() {
    const  _initdata ={
      ...
      initSelect:{
         dsType:[{label:'新建连接',value:1},{label:'连接池',value:2},{label:'INFLUXDB_V1',value:3},{label:'INFLUXDB_V2',value:4},{label:'MONGO',value:5}],
         trayType:[],
      ...
      cols: [
             { field: 'dsType', title: '源类型', minWidth: 100, align: "center",queryType:'select',set:true,require:true },
                { field: 'dsUrl', title: '地址', minWidth: 100, align: "center",queryType:'input',set:true,require:true   },
                { field: 'dsDriver', title: '驱动', minWidth: 100, align: "center",queryType:'input',set:true,require:true  },
                { field: 'dsUser', title: '用户',hide:true, minWidth: 100, align: "center" ,queryType:'input',set:true,require:true },
                { field: 'dsPwd', title: '密码',hide:true,password:true, minWidth: 100, align: "center",queryType:'input',set:true,require:true  },
                { field: 'bizCode', title: '业务编码', align: 'center',queryType:'input',set:true,require:true},
                { field: 'bizName', title: '业务名称', align: 'center',queryType:'input',set:true,require:true },
                { field: 'bizType', title: '业务类别', align: 'center',queryType:'input' ,query:true,set:true,require:true},
                { field: 'bizTypeName', title: '类别名称', align: 'center',queryType:'input' ,query:true,set:true,require:true },
                { field: 'bizDsc', title: '业务描述', align: 'center',queryType:'input',set:true,require:true },
  ...
  methods: {
    ...
    getList() {
      ...
      const _data = {url:'/external/list',params:this.queryParams}
  ...
  submit(formName){
       const _data = {url:'/external/add',params:{
  ...
  query(){
   ...
   save(formName){
      ...
      let _data = {url:'/external/add',params:{data:this.defaultForm.params}}
         this.$refs[formName].validate((valid) => {
          if (valid) {
              this.$store.dispatch('fmsCommon/actionTaskPost',_data)
              .then(data => {
   ...
   del(index,row){
      this.$confirm(i18n.t("common.tip_info.delete_info"), i18n.t("common.tip_info.tips"), {
          ...
          type: 'warning'
                ...
                const _data = {url:'/external/del',params:{data:row.id}}
                this.$store.dispatch('fmsCommon/actionTaskPost',_data)
                .then(data => {
