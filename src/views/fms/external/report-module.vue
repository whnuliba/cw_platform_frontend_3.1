<template>
  <div class="app-container">
        <div class="filter-container" style="padding:0;padding-top:10px;background:#D8DCE5" >
            <el-select v-model="biz.bizType" placeholder="报表类别" class="filter-item" @change="actionType">
                <el-option
                v-for="item in biz.reportTypes"
                :key="item.value"
                :label="commonI18n(item)"
                :value="item.value">
               </el-option>
            </el-select>
              <el-select v-model="biz.bizId" placeholder="选择报表" class="filter-item" @change="actionReport">
                <el-option
                v-for="item in biz.reportOptions"
                :key="item.value"
                :label="commonI18n(item)"
                :value="item.value">
               </el-option>
      ...
      <el-form ref="defaultForm.params" :inline="true" :model="defaultForm.params" label-width="80px">
        <div v-for="(item) in cols" :key="item.id" style="display:inline">
          ...
          >
           ...
           >
              <el-select v-model="defaultForm.params[item.field]" @change="$forceUpdate()" :placeholder="$t('options')">
                <el-option
                  v-for="item in initSelect[item.field]"
                  :key="item.value"
                  :label="item.label"
  ...
  methods: {
    ...
    actionReport(id,load=false){
          ...
          if(this.exportColumn.fields.length==0)
             {
                    this.$message({
                    type: 'info',
                    message: that.$t('common.notExportItem')
                  });
                  return
             }
      ...
      requestData(){
        ...
        // if(sqlArgs.length>0){
        //     Object.assign(args,{where:' where ' +where.join(" and ")},{args:sqlArgs})
        // }
        if(sqlArgs.length>0){
          Object.assign(args,{args:sqlArgs})
        }
