<template>
    ...
    <div class="filter-container">
            ...
            <el-button type="primary"     class="filter-item"  @click="query()" >
             ...
             <el-button type="warning"   class="filter-item" @click="add"  v-ele-show="{btnCode:'add_menu_grp_info'}">
                 {{ $t("common.btn.add") }}
             </el-button>
       </div>
       <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="showDetails" >
        ...
        <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
          <template slot-scope="scope">
            <i class="el-icon-edit"  @click="edit(scope.row)"  v-ele-show="{btnCode:'job_m_add'}"></i>
            &nbsp;
            <i class="el-icon-edit" title="权限"  @click="authorization(scope.row)"  v-ele-show="{btnCode:'batch_job_role'}"></i>
            &nbsp;
            <i class="el-icon-delete"  @click="del(scope.$index,scope.row)"  v-ele-show="{btnCode:'Job_Delete'}"></i>
          </template>
        </el-table-column>
      </el-table>
  ...
  import {roleItem,saveRoleGrpRole,roleAll} from "@/api/roles";
  ...
  export default {
    ...
    data() {
      const  _initdata ={
        ...
        cols: [
                  ...
                  { field: 'jobNo', title: '职位编码', align: "left",query:true,queryType:'input',editReadonly:true,set:true,require:true   },
                  { field: 'jobName', title: '职位名称', align: "left",query:true,queryType:'input',set:true,require:true   },
                  { field: 'jobDesc', title: '职位描述', align: "left",query:true,queryType:'input',set:true,require:true   },
    ...
    methods: {
      ...
      authorization(row){
        ...
        const _data = {url:'/role/guest/query-grp-role-job',params:{data:row.id}}
        this.$store.dispatch('fmsCommon/actionAuthPost',_data)
        .then(data => {
          data.forEach(item=>{
      ...
      this.checkedroleData.forEach(item=>{
        for(let c in this.roleData){
          if(this.roleData[c].id ===item){
            ...
            //若全部取消，则置为#
        if(data.length == 0){
               data.push({roleId:'#',jobId:this.myRowId})
          }
      const _data = {url:'/job/batch-job-role',params:{data:data}}
        this.$store.dispatch('fmsCommon/actionAuthPost',_data)
        .then(data => {
            this.checkedroleData=[]
            this.getList()
