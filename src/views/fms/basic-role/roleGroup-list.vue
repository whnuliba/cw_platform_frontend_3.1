<template>
    <div class="app-container rolegroup-list">
    <div class="filter-container">
            ...
            <el-button type="primary"     class="filter-item"  @click="query()" >
             ...
             <el-button type="warning"   class="filter-item" @click="add"  v-ele-show="{btnCode:'role_group_add'}">
                 {{ $t("common.btn.add") }}
             </el-button>
       </div>
       <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="showDetails" >
        ...
        <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="80">
          <template slot-scope="scope">
            <i class="el-icon-edit"  @click="edit(scope.row)"  v-ele-show="{btnCode:'role_group_add'}"></i>
            &nbsp;
            <i class="el-icon-edit" title="权限"  @click="authorization(scope.row)"  v-ele-show="{btnCode:'role_group_add_role'}"></i>
            &nbsp;
            <i class="el-icon-delete"  @click="del(scope.$index,scope.row)"  v-ele-show="{btnCode:'role_group_delete'}"></i>
          </template>
        </el-table-column>
      </el-table>
  ...
  import {roleItem,saveRoleGrpRole} from "@/api/roles";
  ...
  export default {
    ...
    data() {
      const  _initdata ={
        ...
        initSelect:{
           ...
           scope:[{label:'共享',value:'1'},{label:'私有',value:'0'}],
        },
        defaultForm: {
          row:{},
          params:{
        ...
        cols: [
                  { field: 'createDate', title: '创建日期', align: "left",editReadonly:true,set:false },
                  { field: 'createUser', title: '创建人', align: "left",set:false   },
                  { field: 'groupNo', title: '组编码', align: "left",query:true,queryType:'input',editReadonly:true,set:true,require:true   },
                  { field: 'groupName', title: '组名称', align: "left",query:true,queryType:'input',set:true,require:true   },
                  { field: 'roleMaxUser', title: '基数限定', align: "left",query:true,queryType:'input',set:true,require:true   },
                  { field: 'groupDesc', title: '组描述', align: "left",query:true,queryType:'input',set:true,require:true   },
    ...
    methods: {
      ...
      authorization(row){
        ...
        this.roleGrpId = row.id
        this.checkedroleData=[]
        const _data = {url:'/role/query-grp-role',params:{data:row.id}}
        this.$store.dispatch('fmsCommon/actionAuthPost',_data)
        .then(data => {
          data.forEach(item=>{
    ...
    SubmitAuthorization(){
      ...
      this.checkedroleData.forEach(item=>{
        let obj={
          roleId:item,
          groupId:this.roleGrpId
        }
        data.push(obj);
      })
      //若全部取消，则置为#
      if(data.length == 0){
         data.push({roleId:'#',groupId:this.roleGrpId})
      }
      saveRoleGrpRole({data}).then(res=>{
