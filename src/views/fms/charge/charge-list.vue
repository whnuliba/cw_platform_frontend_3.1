<template>
  <div class="app-container">
    ...
    <el-dialog title="编辑" v-el-drag-dialog :visible.sync="dialogFormVisible" width="700px" >
     ...
     <!--显示明细-->
     <el-dialog :title="'['+cellBarCode+']明细'" v-el-drag-dialog :visible.sync="chargeDetailsVisible" >
            <component  :is="dtsOfswitch" ref="chargeDetailsList" :chargeDetailsList="chargeDetailsList"> </component >
             <div slot="footer" class="dialog-footer" >
            <el-button @click="chargeDetailsVisible = false">  {{ $t("common.btn.cancel") }}</el-button>
           </div>
     </el-dialog>
...
import chargeDetailsList from './components/charge-details'
...
export default {
  name: 'chargeList',
  components: { Pagination,chargeDetailsList,ColTableInit },
 directives: { waves, elDragDialog },
  ...
  data() {
    const  _initdata ={
      ...
      chargeDetailsList:'',
      chargeDetailsVisible:false,
      cellBarCode:'',
      dtsOfswitch:'chargeDetailsList',
      //列显示呈现 -》 组件所需name,需唯一
      table_mark:"FMS_CHARGE_LIST",
      queryParams:{
        requestData:{ }
        },
      initSelect:{
  ...
  methods: {
    ...
    getList() {
      ...
      const _data = {url:'/CwChargeProcessController/list',params:this.queryParams}  // /chargeresult/getchargeresult
    ...
    //   })
      ...
      this.chargeDetailsList=row.id
      this.chargeDetailsVisible=true
