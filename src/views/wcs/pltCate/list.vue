  ...
  <div class="app-container">
        <div class="filter-container">
          ...
          <el-button type="info"     class="filter-item" @click="reset" >
           ...
           <el-button type="warning" v-ele-show="{btnCode:'wcs_pltCateAdd'}"  class="filter-item" @click="add" >
               {{ $t("common.btn.add") }}
           </el-button>
        </div>
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="showDetails" >
      ...
      <el-table-column align="left" :label="$t('common.oper')" fixed="right" width="200">
        <template slot-scope="scope">
          <el-button type="primary" size="small" v-ele-show="{btnCode:'wcs_pltCateAdd'}"  icon="el-icon-edit" @click="edit(scope.row)">
             {{ $t("common.btn.edit") }}
          </el-button>
           &nbsp;
           <el-button type="warning" v-ele-show="{btnCode:'wcs_pltCateDel'}"   size="small" icon="el-icon-edit" @click="del(scope.$index,scope.row)">
              {{ $t("common.btn.delete") }}
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="dialogtitle" v-el-drag-dialog :visible.sync="dialogFormVisible" width="400px" >
        ...
        <div v-for="(item) in cols" :key="item.id" style="display:inline">
          ...
          >
           ...
           <el-form-item  v-if="item.set && item.queryType==='select'" :label="commonI18n(item)"
            :rules="[{ required: item.required, message: commonI18n(item)+$t('const.cannotNull'), trigger: 'blur' }]"
           >
              <el-select clearable  v-model="defaultForm.params[item.field]" @change="$forceUpdate()" :placeholder="$t('const.pleaseSelect')"  style="width:90%">
                <el-option
                  v-for="item in initSelect[item.field]"
                  :key="item.value"
                  :label="item.label"
      ...
      <div slot="footer" class="dialog-footer" >
        ...
        <el-button type="primary" v-if="defaultForm.params.pltCateId==='' || defaultForm.params.pltCateId===null" @click="save('defaultForm.params')">  {{ $t("common.btn.save") }}</el-button>
        <el-button type="primary" v-if="defaultForm.params.pltCateId!='' && defaultForm.params.pltCateId!=null"  @click="submit('defaultForm.params')">  {{ $t("common.btn.submit") }}</el-button>
      </div>
    </el-dialog>
  </div>
 ...
 directives: { waves, elDragDialog },
  ...
  data() {
    ...
    //   total: 0,
      ...
      cols: [
                { field: 'pltCateId', title: '托盘类型Id', minWidth: 100,hide:true, align: "center" },
                { field: 'pltCateCode', title: '类型编号', minWidth: 100, align: "center",queryType:'input',set:true,required:true },
                { field: 'pltCateName', title: '类型名称', minWidth: 100, align: "center",query:false,queryType:'input',set:true,required:true },
                { field: 'pltFunc', title: '托盘功能', minWidth: 100, align: "center" ,queryType:'select',set:true,required:true},
                { field: 'capacity', title: '容量', minWidth: 100, align: "center",queryType:'input',set:true,required:true},
                { field: 'ctrlCode', title: '调度编号', align: 'center',query:false,queryType:'input',set:true,required:true},
                { field: 'pltRules', title: '条码规则', align: 'center' ,queryType:'input',set:true,required:false},
                // {field: 'sortNum', title: '序号', hide:true,align: 'center',query:false,queryType:'input',set:true,filter:true,required:true},
                { field: 'useState', title: '使用标志', align: 'center' ,queryType:'select',set:true,required:true},
       ]
    }
  ...
  methods: {
    ...
    getList() {
      ...
      const _data = {url:'/api/Common/GetPltCates',params:{}}
  ...
  submit(formName){
       const _data = {url:'/api/Common/AddPltCates',params:{
  ...
  query(){
   ...
   save(formName){
      ...
      let _data = {url:'/api/Common/AddPltCates',params:{data:this.defaultForm.params}}
         this.$refs[formName].validate((valid) => {
          if (valid) {
              this.$store.dispatch('fmsCommon/actionWcsPost',_data)
              .then(data => {
   ...
   del(index,row){
      this.$confirm(i18n.t("common.tip_info.delete_info"), i18n.t("common.tip_info.tips"), {
          ...
          type: 'warning'
                ...
                const _data = {url:'/api/Common/DelPltCates',params:{data:row.pltCateId}}
                this.$store.dispatch('fmsCommon/actionWcsPost',_data)
                .then(data => {
