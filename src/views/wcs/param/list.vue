  ...
  <div class="app-container">
        <div class="filter-container">
          ...
          <el-button type="info"     class="filter-item" @click="reset" >
           ...
           <el-button type="warning" v-ele-show="{btnCode:'wcs_paramAdd'}"  class="filter-item" @click="add" >
               {{ $t("common.btn.add") }}
           </el-button>
        </div>
     <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%"  @row-dblclick="showDetails" >
      ...
      <el-table-column align="center" :label="$t('common.oper')" fixed="right" width="200">
        <template slot-scope="scope">
          <el-button type="primary" size="small" v-ele-show="{btnCode:'wcs_paramUpdate'}"  icon="el-icon-edit" @click="edit(scope.row)">
             {{ $t("common.btn.edit") }}
          </el-button>
           &nbsp;
           <el-button type="warning" v-ele-show="{btnCode:'wcs_paramDel'}"   size="small" icon="el-icon-edit" @click="del(scope.$index,scope.row)">
              {{ $t("common.btn.delete") }}
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="dialogtitle" v-el-drag-dialog :visible.sync="dialogFormVisible" width="600px" >
      <el-form ref="defaultForm.params" :inline="false" :model="defaultForm.params" label-width="150px">
        <div v-for="(item) in cols" :key="item.id" style="display:inline">
          ...
          >
             <el-input  v-model="defaultForm.params[item.field]"  :disabled="!((difDiag && item.isEdit) || (!difDiag))"  style="width:80%" />
          </el-form-item>
           <el-form-item  v-if="item.set && item.queryType==='select' " :label="commonI18n(item)"
            :rules="[{ required: item.required, message: commonI18n(item)+$t('const.cannotNull'), trigger: 'blur' }]"
           >
              <el-select clearable  v-model="defaultForm.params[item.field]" @change="$forceUpdate()" :disabled="!((difDiag && item.isEdit) || (!difDiag))" :placeholder="$t('const.pleaseSelect')"  width='200px'>
                <el-option
                  v-for="item in initSelect[item.field]"
                  :key="item.value"
                  :label="item.label"
      ...
      <div slot="footer" class="dialog-footer" >
        ...
        <el-button type="primary" v-if="defaultForm.params.paramCode==='' || defaultForm.params.paramCode===null" @click="save('defaultForm.params')">  {{ $t("common.btn.save") }}</el-button>
        <el-button type="primary" v-if="defaultForm.params.paramCode!='' && defaultForm.params.paramCode!=null"  @click="submit('defaultForm.params')">  {{ $t("common.btn.submit") }}</el-button>
      </div>
    </el-dialog>
  </div>
 ...
 directives: { waves, elDragDialog },
  ...
  data() {
    const  _initdata ={
      ...
      cols: [
                { field: 'paramCateId', title: '参数类型',en_title:'paramType', minWidth: 100,query:true,queryType:'select',isEdit:false, set:true,required:true, align: "center" },
                { field: 'paramCode', title: '参数编号', minWidth: 100, align: "center",isEdit:false, query:true,queryType:'input',set:true,required:true },
                { field: 'paramName', title: '参数名称', minWidth: 100, align: "center",isEdit:false, query:false,queryType:'input',set:true,required:true },
                { field: 'paramValue', title: '参数值', align: 'center',isEdit:true,query:false,queryType:'input',set:true,required:true},
  ...
  methods: {
    ...
    getList() {
      ...
      const _data = {url:'/api/Common/GetParamApi',params:this.queryParams}
  ...
  submit(formName){
       const _data = {url:'/api/Common/UpdateParamsApi',params:{
  ...
  query(){
   ...
   selectOptions(){
       //获取系统参数类型
      let _data = {url:'/api/Common/GetpcsParamCates',params:{}}
      this.$store.dispatch('fmsCommon/actionWcsPost',_data)
            .then(data => {
                this.initSelect.paramCateId = data.map(c=>{
                  return {
                    label:c.paramCateName,
                    value:c.paramCateId
                  }
                })
            }).catch(err => {
   ...
   save(formName){
      ...
      let _data = {url:'/api/Common/AddParamsApi',params:{data:this.defaultForm.params}}
         this.$refs[formName].validate((valid) => {
          if (valid) {
              this.$store.dispatch('fmsCommon/actionWcsPost',_data)
              .then(data => {
   ...
   del(index,row){
      this.$confirm(i18n.t("common.tip_info.delete_info"), i18n.t("common.tip_info.tips"), {
          ...
          type: 'warning'
                ...
                const _data = {url:'/api/Common/DelPcsParamApi',params:{data:row.paramCode}}
                this.$store.dispatch('fmsCommon/actionWcsPost',_data)
                .then(data => {
