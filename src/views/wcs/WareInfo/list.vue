<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button type="warning" v-ele-show="{btnCode:'AddWareInfo'}" class="filter-item" @click="add">
        {{ $t('common.btn.add') }}
      </el-button>
    </div>
    ...
    <el-dialog :title="dialogtitle" v-el-drag-dialog :visible.sync="dialogFormVisible" width="800px">
        <el-form :label-position="labelPosition" label-width="120px" :model="addParam">
          <el-form-item :label="commonI18n(getItem(cols,'wareCode'))">
            <el-input v-model="addParam.wareCode"></el-input>
          </el-form-item>
          <el-form-item :label="commonI18n(getItem(cols,'wareName'))">
            <el-input v-model="addParam.wareName"></el-input>
          </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false" v-preventReClick>{{$t('common.btn.cancel')}}</el-button>
        <el-button type="primary" @click="addParamData" v-preventReClick>{{$t('common.btn.submit')}}</el-button>
      </div>
    </el-dialog>
  ...
  export default {
    ...
    data() {
      const _initdata = {
        ...
        listQuery: {},
        cols: [
          { field: 'wareCode', title: '仓库编号', minWidth: 100, align: 'center' },
          { field: 'wareName', title: '仓库名称', minWidth: 100 },
    ...
    methods: {
      ...
      addParamData(){
        ...
        const _data = { url: '/api/LocInfo/AddWareInfo', params: json }
        this.$store.dispatch('fmsCommon/actionWcsPost', _data)
          .then(data => {
            this.$message({
      ...
      del(index, row) {
          ...
          cancelButtonText: i18n.t("common.btn.cancel"),
            ...
            console.log(row)
            this.listLoading = true;
            const _data = { url: '/api/LocInfo/DelWareInfo', params: {data:row.wareId} }
