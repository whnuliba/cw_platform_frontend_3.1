{
    mounted(){
            //需要挂载后才能添加监听 - 没有卸载，导致监听混乱
         /* this.$nextTick(() => {
             this.$watch(() => this.$store.getters.language, (newVal, oldVal) => {
                  if (newVal != oldVal) {
                    this.vueContext['CwCtrlList1'].$children[0].cols = [
    ...
    cols: [
      ...
      { field: 'createTime', title: '创建日期', align: "center", query: false, queryType: 'input', set: false },
      { field: 'createUser', title: '创建人', align: "center", query: false, queryType: 'input', set: false },
      { field: 'status', title: '状态', align: "center", query: true, queryType: 'select', set: false, jsonSelData: true, status: [{ label: this.commonI18n('common.todo.end'), value: 2 }, { label: this.commonI18n('common.todo.rejected'), value: 4 }, { label: this.commonI18n('common.todo.agree'), value: 6 }, { label: this.commonI18n('common.todo.pending'), value: 7 }] },
      //
      { field: 'uri', title: '地址模板', align: "center", query: false, queryType: 'input', set: false },
      ...
      toolColumns: [
        {
          ...
          buttons: [
            ...
            // },
            {
              text: 'common.todo.approval', background: '', size: 'mini', show: function (row) {
                return row.status == 7 ? true : false
              }, callback: function (e, self, row, selected, vueContext) {
                let _this = this
                vueContext['CwDialogPanel1'].modelTitle = self.commonI18n('common.ProcessFlowList.editProcess')
                vueContext['CwDialogPanel1'].dialogVisible = true
                vueContext['CwCtrlList1'].dataObtainedByEditor['rowTableData'] = row
                vueContext['CwTree1'].dataSource = [];
        
                vueContext['CwSelect1'].options = [
                  { label: self.commonI18n('common.todo.agree'), value: 6 },
                  { label: self.commonI18n('common.todo.reject'), value: 4 }
                ]
                ...
                this.$store.dispatch('fmsCommon/actionFmsPost', _dataTop).then(data => {
                  ...
                  this.$store.dispatch('fmsCommon/actionFmsPost', _data).then(response => {
                    ...
                    for (let i = 0; i < data.length; i++) {
                      const jsonTemp = {} 
                      const st = (data[i].status == 2 || data[i].status == null) ? '' : '_'+self.commonI18n('common.processFlowTmpList.jumpStation')
                      jsonTemp['id'] = data[i].id
                      jsonTemp['label'] = self.commonI18n('[工序0') + data[i].flowSort + ']_' + data[i].processCode + '_' + self.commonI18n(data[i].processName) + st
