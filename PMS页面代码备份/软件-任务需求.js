{
 ...
 url:{
    deleteUri:'/cw-pms-app/pms-soft-demand/deleteById',
    editUri:'/cw-pms-app/pms-soft-demand/updateById',  
    addUri:'/cw-pms-app/pms-soft-demand/createBaseComponent',  
    queryUri:'/cw-pms-app/pms-soft-demand/queryPage', 
    batchAddUri:'/cw-pms-app/pms-soft-demand/batchCreateBaseComponent',
    fileUpload:'/cw-pms-app/pms-soft-demand/uploadFile',
    fileDownload:'/cw-pms-app/pms-soft-demand/downloadFile',
    removeFile:'/cw-pms-app/pms-soft-demand/removeFile',      
 },  
 customToolBar:{
           show:true,
           showFilter:true,
           ...
           buttons:[
		   		...
		   		{text:'附件信息',icon:'el-icon-folder-opened',permCode:undefined,background:'',size:'mini',hideText:false,callback:function(e,self){
               ...
               {text:'日志',icon:'el-icon-edit-outline',permCode:undefined,background:'',size:'mini',hideText:false,callback:function(e,self){
				  ...
				  let id =user.userInfo.id
					  ...
					  if(row.TASK_STATE==='RUNING' && row.COMPLETE_USER_ID!==id ){
                ...
                {text:'完成',icon:'el-icon-success',permCode:undefined,background:'',size:'mini',hideText:false,callback:function(e,self){
				  ...
				  let id =user.userInfo.id
					 let row =self.boxSelectValue[0] 
					if(row.TASK_STATE==='ASSIGN'  ){
						self.$message.error('当前任务未指派，你不可以完成！')
					   return
					}
					  if(row.TASK_STATE==='RUNING' && row.COMPLETE_USER_ID!==id ){
				   ...
				   if(row.TASK_STATE==='RUNING' && row.TASK_STATE==='ASSIGN'){
               ...
               {text:'查询',icon:'el-icon-search',permCode:'pms-work-task_query',background:'',size:'mini',hideText:false,callback:function(e,self){
                   self.fieldFormQuery()
                },
                style:{
                float: 'left'
              ...
              {text:'编辑',icon:'el-icon-edit',background:'',permCode:'pms-work-task_edit',size:'mini',hideText:false,callback:function(e,self){
				  ...
				  if(row.TASK_STATE!=='CREATE' || row.COMPLETE_USER_ID!==id ){
 ...
 formModes:[
   {
       ...
       forms:[
            ...
            { field: 'DESCRIPTION', title: '需求描述', minWidth: 100, align: "center", queryType: 'textarea',style:{width:'600px'}, set: true, require: false, query: false, hide: false },
            ...
            { field: 'ATTACHMENT1', title: '附件', minWidth: 100, align: "center", queryType: 'attachment',style:{width:'600px'}, set: true, require: false, query: false, hide: false,
              fileLimit:10,
			  httpRequest:function(item){
				item.name=item.file.name
			   // this.fileList.push(item)
				let names = this.iformFile['completeDialog'].fileList['ATTACHMENT1'].map(c=>c.name);
      ...
      buttons:[
            {text:'确认',hideText:false,icon:'el-icon-success',permCode:undefined,background:'',size:'max',callback:function(e,row,form){
                      ...
                      let op = {url:'/cw-pms-app/pms-soft-demand/complete',params:fa}
       ...
       forms:[
			...
			{ field: 'ATTACHMENT1', title: '附件1', minWidth: 100, align: "center", queryType: 'attachment', set: true, require: false, query: false, hide: false,
			  ...
			  httpRequest:function(item){
				item.name=item.file.name
			   // this.fileList.push(item)
				let names =this.iformFile['workLogDialog'].fileList['ATTACHMENT1'].map(c=>c.name);
				if(names.indexOf(item.file.name)==-1)
				{
...
attachmentModels:[
{
 ...
 cols: [
            ...
            { field: 'CREATE_USER', title: '创建认', minWidth: 100, align: "center", queryType: 'daterange', set: false, require: false, query: false, hide: false},

            { field: 'PLAN_START_TIME', title: '开始时间', minWidth: 150, align: "center", queryType: 'datetime', set: true, require: true, query: false, hide: false},
            { field: 'PLAN_END_TIME', title: '结束时间', minWidth: 150, align: "center", queryType: 'datetime', set: true, require: true, query: false, hide: false},
			{ field: 'DEMAND_CODE', title: '需求编码', minWidth: 150, align: "center", queryType: 'input', set: false, require: false, query: false, hide: false },   
       ...
       queryType: 'select',selUri:'/cw-pms-app/app/constant/work-task-states',TASK_STATE:['name','value'], set: false, require: true, query: true, hide: false },
            ...
            { field: 'COMPLETE_USER', title: '指派给', minWidth: 100, align: "center", queryType: 'input',charNum:20, set: false, require: false, query: false, hide: false },
   			{ field: 'COMPLETE_USER_ID', title: '指派给', minWidth: 100,align: 'center',multiple:true,charNum:20,orgType:300,selectRange:[300], queryType: 'user',userChange:function(result,form,item){
             
              let {name,code,id} = result
              form['COMPLETE_USER'] = name
              
            }, set: true,formItemShow:function(form){
            ...
            }, require: true, query: false, hide: true },
            ...
            { field: 'DEMAND_HOME', title: '提出类型', minWidth: 100, align: "center", queryType: 'select', set: true, selUri:'/cw-pms-app/app/constant/demand-homes',DEMAND_HOME:['name','value'],require: false, query: false, hide: false },
            ...
            { field: 'ATTACHMENT3', title: '附件3', minWidth: 100, align: "center", queryType: 'file', set: false, require: false, query: false, hide: true },
      		{ field: 'DESCRIPTION', title: '需求描述', minWidth: 100, align: "center", queryType: 'textarea',style:{width:'600px'}, set: true, require: false, query: false, hide: false },
            { field: 'WORK_CONTENT', title: '需求内容', minWidth: 150, align: "center", queryType: 'textarea',style:{width:'600px'}, set: false, require: false, query: false,charNum:5, hide: true },
                              ...
                              {text:'指派',icon:'el-icon-right',background:'',size:'mini',
                               ...
                               ,callback: function(e,self,row,selected){ 
									  ...
									  type: 'warning'
										   ...
										   let op = {url:'/cw-pms-app/pms-soft-demand/assign',params:{data:row.ID}}
										   this.actionPost(op).then((data)=>{
												   self.$message({
													type: 'success',
                               ...
                               ,callback: function(e,self,row,selected){ 
							           this.$confirm(this.i18n("开始后必须在指定时间完成，否则会影响你的KPI"), this.i18n("common.tip_info.tips"), {
										  ...
										  type: 'warning'
										   ...
										   let op = {url:'/cw-pms-app/pms-soft-demand/startTask',params:{data:row}}
										   this.actionPost(op).then((data)=>{
												   self.$message({
													type: 'success',
													message: 'change success!=>' + data
